			...
			if (maxcount > 0)
			{
				for (int i = 0;i < maxcount; ++i)
				{
					...
					if (buildinglv > maxlv)
					{
						...
						if (maxlv >= m_targetlv)
						{
							myPlayer->doEvent(WCH_JXJAdvanceTask,MAKECREPARAM(m_this->getTaskID(),0));
							break;
						}
					}
				}
	...
	if (playerData)
	{
		...
		if (braindata)
		{
			...
			if (rollcardcount > m_oldrollcount)
			{
				...
				playerData->doEvent(WCH_JXJAdvanceTask, MAKECREPARAM(m_this->getTaskID(), TS_One));
			}
		}
	}
}
			...
			if ((curstep) > 1)
			{
				...
				if(NewguideStepTab->queryOneRecord(0,crArgumentParser::appItoa(curguideid),Step_stepindex,crArgumentParser::appItoa((curstep) - 1),Step_record) >= 0)
				{
					...
					if(scene)
					{
						...
						if(item)
						{
							item->doEvent(WCH_JXJBuildingTaskEffect);
						}
					}
				}
			}
						...
						else
						{
							...
							if(scene)
							{
								...
								if(item)
								{
									...
									item->doEvent(WCH_JXJBuildingTaskEffect,MAKECREPARAM(1,NULL));
...
{//提取需要修改的玩家数据并进行修改
	...
	if(!playerdatachangelistQueryResultVec.empty())
	{
			...
			++itr )
		{
			...
			{
				mainrole = playerData->getPlayerGameData()->getMainRole();
				data = playerData->getPlayerGameData()->getDataClass();
				CRCore::ScopedLock<crData> lock(*data);
				mainroledata = mainrole->getDataClass();
				//data->excHandle(MAKECREPARAM(WCH_LockData,1));
				if (changetype == T_ActiveSuperEquip)
				{
					mainrole->doEvent(WCH_JXJActiveSuperEquip);
				}
				else if (changetype == T_Giftgold)
				{
					moneydata.first = count;
					moneydata.second = "GM修改礼金";
					playerData->getPlayerGameData()->doEvent(WCH_JXJRecvGiftgold, MAKECREPARAM(&moneydata, NULL));
				}
				else if (changetype == T_GroupID)
				{
					m_this->doEvent(WCH_JXJKictOutGroupMemberByGM, MAKECREPARAM(playerid,NULL));
				}
				else if (changetype == T_Item)
				{
					if (count > 0)
					{
						...
						playerData->getPlayerGameData()->doEvent(WCH_JXJRecvRewardItems,MAKECREPARAM(&rewardItems,0));
					}
					else if (count < 0)
					{
						...
						playerData->getPlayerGameData()->doEvent(WCH_JXJRecvDeductItems,MAKECREPARAM(&deductitemmap,0));
					}
				}
...
void crJXJScenarioTalkMethod::operator()(crHandle &handle)
{
	...
	if (ScenarioTalkTab.valid() && talkidindex >= 0 && canvas.valid())
	{
		...
		if (canvasdata.valid() && !records.empty())
		{
			...
			canvas->doEvent(WCH_JXJJXJShowScenarioTalk,MAKECREPARAM(&canvasname,m_scenarioValue));
		}
	}
}
/////////////////////////////////////////
...
void crJXJShowScenarioTalkMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_canvasname = *(std::string *)LOCREPARAM(param64);
			m_scenarioValue = HICREPARAM(param64);
		}
		else
		{
			m_canvasname.clear();
...
void crJXJGetConsortiaNameMethod::inputParam(int i, void *param)
{
	...
	case 2:
		if(param)
		{
			...
			m_consortianame = (std::string*)(HICREPARAM(param64));
		}
		else
		{
			m_playerid = 0;
...
void crJXJUIMarketExchangeResMethod::operator()(crHandle &handle)
{
	...
	if (m_this && playerGameData)
	{
		...
		if (canvas.valid() && marketcanvas.valid())
		{
			...
			{
				playerGameData->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(2104, NULL));//资源购买后超过府库上线
			}
		}
	}
}
		...
		if (m_netType == GameServer)
		{
			...
			if (playerData.valid())
			{
				...
				if (success == 1)
				{
					...
					{
					case T_Food:
						m_this->doEvent(WCH_JXJRecvFood, MAKECREPARAM(-sellrescount, NULL));
						resname1 = "粮食";
						break;
					case T_Wood:
						m_this->doEvent(WCH_JXJRecvWood, MAKECREPARAM(-sellrescount, NULL));
						resname1 = "木材";
						break;
					case T_Iron:
						m_this->doEvent(WCH_JXJRecvIron, MAKECREPARAM(-sellrescount, NULL));
						resname1 = "铁矿";
						break;
					case T_Horse:
						m_this->doEvent(WCH_JXJRecvHorse, MAKECREPARAM(-sellrescount, NULL));
						resname1 = "马匹";
						break;
					default:
						break;
					...
					{
					case T_Food:
						m_this->doEvent(WCH_JXJRecvFood, MAKECREPARAM(getrescount, NULL));
						resname2 = "粮食";
						break;
					case T_Wood:
						m_this->doEvent(WCH_JXJRecvWood, MAKECREPARAM(getrescount, NULL));
						resname2 = "木材";
						break;
					case T_Iron:
						m_this->doEvent(WCH_JXJRecvIron, MAKECREPARAM(getrescount, NULL));
						resname2 = "铁矿";
						break;
					case T_Horse:
						m_this->doEvent(WCH_JXJRecvHorse, MAKECREPARAM(getrescount, NULL));
						resname2 = "马匹";
						break;
					default:
						break;
						...
						do
						{
							...
							if (success == 0)
							{
								...
								for (int i = 0; i < 3; ++i)
								{
									...
									{
									case T_Exp:
										m_this->getMainRole()->doEvent(WCH_JXJRecvExperience, MAKECREPARAM(itemid[i][0], NULL));
										break;
									case T_VIPExp:
										{
											int maxvalue = atoi(crGlobalHandle::gData()->gGameGlobalValue(WCHDATA_JXJMaxCycleVipExpReward, viplv).c_str());
											m_this->doEvent(WCH_JXJAddVipExp, MAKECREPARAM(CRCore::minimum(itemid[i][0] * (cycleCount + 1),maxvalue), NULL));
										}
										break;
									case T_Copper:
										m_this->doEvent(WCH_JXJRecvCoppercash, MAKECREPARAM(itemid[i][0], NULL));
										break;
									case T_Exploit:
										m_this->getMainRole()->doEvent(WCH_JXJRecvExploit, MAKECREPARAM(itemid[i][0], NULL));
										break;
									case T_Giftgold:
...
void crJXJLingquFengluMethod::operator()(crHandle &handle)
{
	...
	if (myPlayer && netConductor)
	{
		...
		if (flag == 0)
		{
			...
			myPlayer->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(2108, NULL));//
	}
}
/////////////////////////////////////////
//
	...
	if (m_this && m_this->getMainRole() && m_this->getMainRole()->getDataClass() && m_stream.valid())
	{
		if (m_netType == GameServer)
		{
			...
			if (playerData.valid() && m_this->getMainRole() && m_this->getMainRole()->getDataClass())
			{
							...
							for (crTableIO::DataVec::iterator itr = records.begin(); itr != records.end(); ++itr)
							{
								...
								if (count > 0)
								{
									...
									else
									{
										...
										{
										case T_Copper:
											m_this->doEvent(WCH_JXJRecvCoppercash, MAKECREPARAM(count, NULL));
											break;
										case T_Food:
											m_this->doEvent(WCH_JXJRecvFood, MAKECREPARAM(count, NULL));
											datatype = 0;
											break;
										case T_Wood:
											m_this->doEvent(WCH_JXJRecvWood, MAKECREPARAM(count, NULL));
											datatype = 0;
											break;
										case T_Iron:
											m_this->doEvent(WCH_JXJRecvIron, MAKECREPARAM(count, NULL));
											datatype = 0;
											break;
										case T_Horse:
											m_this->doEvent(WCH_JXJRecvHorse, MAKECREPARAM(count, NULL));
											datatype = 0;
											break;
										case T_Exp:
											m_this->getMainRole()->doEvent(WCH_JXJRecvExperience, MAKECREPARAM(count, NULL));
											datatype = 0;
											break;
										case T_Achievement:
											m_this->getMainRole()->doEvent(WCH_JXJRecvAchievement, MAKECREPARAM(count, NULL));
											datatype = 0;
											break;
										case T_Giftgold:
											moneydata.first = count;
											...
											break;
										case T_Exploit:
											m_this->getMainRole()->doEvent(WCH_JXJRecvExploit, MAKECREPARAM(count, NULL));
											datatype = 0;
											break;
										case T_Contribute:
											m_this->doEvent(WCH_JXJRecvCountryContribute, MAKECREPARAM(count, NULL));
											datatype = 0;
											break;
										default:
											break;
				...
				if (success == 1)
				{
					m_this->doEvent(WCH_JXJRecvGetChengChiEarnings,MAKECREPARAM(NULL,GameServer));
				...
				if (success == 1)
				{
					data->excHandle(MAKECREPARAM(WCH_LockData, 1));
					m_this->doEvent(WCH_JXJRecvRewardItems, MAKECREPARAM(&rewardItems, 0));
					m_this->doEvent(WCH_JXJRecvHuoyuedu, MAKECREPARAM(HYD_GovpostMoney, 1));
					data->excHandle(MAKECREPARAM(WCH_LockData, 0));
				}
	...
	if (!gmmailQueryResultVec.empty())
	{
			...
			++itr)
		{
			...
			else
			{
				...
				m_this->doEvent(WCH_JXJSystermSendMailToPlayer);
			}
		}
	}
	...
	if (m_this && playerGameData)
	{
		...
		if (canvas.valid() && resaddbuildingcanvas.valid() && needviplvindex >= 0 && buildinglvindex >= 0 && priceindex >= 0)
		{
			...
			if (ResAddBuildingTab->queryOneRecord(0, crArgumentParser::appItoa(buildingid), buildinglvindex, crArgumentParser::appItoa(buildinglv + 1), ResAddBuildingrecord) >= 0)
			{
				...
				{
					playerGameData->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(2111, NULL));//VIP不足
				}
			}
			else
			{
				playerGameData->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(2110, NULL));//建筑已升至最高级
			}
		}
	}
}
...
/////////////////////////////////////////
//
//crJXJUIResAddBuildingCanvasUpdateMethod
//
/////////////////////////////////////////
crJXJUIResAddBuildingCanvasUpdateMethod::crJXJUIResAddBuildingCanvasUpdateMethod()
{
}
crJXJUIResAddBuildingCanvasUpdateMethod::crJXJUIResAddBuildingCanvasUpdateMethod(const crJXJUIResAddBuildingCanvasUpdateMethod& handle) :
	crMethod(handle)
{
	m_canvasname = handle.m_canvasname;
	for (int i = 0; i < 9; ++i)
...
void crJXJUIResAddBuildingCanvasUpdateMethod::inputParam(int i, void *param)
{
}
void crJXJUIResAddBuildingCanvasUpdateMethod::addParam(int i, const std::string& str)
{
	switch (i)
	{
	case 0:
...
void crJXJUIResAddBuildingCanvasUpdateMethod::operator()(crHandle &handle)
{
	void *param;
	crPlayerGameData *playerGameData = crMyPlayerData::getInstance()->getPlayerGameData();
	ref_ptr<crCanvasNode> canvas = crFilterRenderManager::getInstance()->findCanvas(m_canvasname);
...
void crJXJBuyResAddBuildingTimeMethod::operator()(crHandle &handle)
{
	...
	if (netConductor)
	{
		...
		if (buildingid > 0)
		{
			...
			playerGameData->doEvent(WCH_JXJCheckLijinGold, MAKECREPARAM(&price, NULL));
			if (price == 0)
			{
				data->getParam(WCHDATA_JXJExtraProduceMap, param);
				ExtraProduceMap *extraProduceMap = (ExtraProduceMap *)param;
			...
			{
				playerGameData->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(tipsid, NULL));//
			}
		}
	}
}
