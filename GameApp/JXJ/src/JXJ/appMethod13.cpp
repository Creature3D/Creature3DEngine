		...
		if(m_netType == GameServer)
		{
			...
			if(playerData.valid() && mainRole && mainRole->getDataClass())
			{
				...
				do 
				{
					...
					m_this->doEvent(WCH_JXJNewGetAwardsByActiveCode,MAKECREPARAM(&activeCodeData,&logRet));
					...
					else if(_status == 0)
					{
						...
						if (huodongTab->queryOneRecord(0,crArgumentParser::appItoa(zhoukaID),record) >= 0)
						{
							...
							if (lijinVec.size() >= 7)
							{
								...
								data->inputParam(WCHDATA_JXJZhouKaGetStatus,&zhoukastatus);
								data->excHandle(MAKECREPARAM(WCH_LockData,0));
								m_this->doEvent(WCH_JXJRecvUpdateZhouKaStatus,MAKECREPARAM(NULL,m_netType));
								logRet = 1;
							}
		...
		if (canvas.valid())
		{
			...
			if (canvasdata && memberList.valid())
			{
				...
				if (curId != playerGameData->getPlayerID())
				{
					...
					{
						playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(5002,NULL));
					}
				}
				else
				...
				if(groupLevel < maxGroupLevel)
				{
					if (groupUpdateTab.valid() && groupUpdateTab->queryOneRecord(0,crArgumentParser::appItoa(groupLevel),record) >= 0)
					{
						...
						if (funds >= needfunds)
						{
							...
							m_this->doEvent(WCH_JXJRecvQueryMyGroupInfo,MAKECREPARAM(NULL,GameServer));
						}
						else
						{
							ret = 1;	//资金不足
...
void crJXJRenMingCanvasUpdateMethod::operator()( crHandle &handle )
{
	...
	if (myPlayer && myPlayer->getDataClass() &&mygroupcanvas.valid())
	{
		...
		if (canvasdata)
		{
			...
			{
				ref_ptr<crTableIO> chengchitab = crGlobalHandle::gData()->gGlobalTable(WCHDATA_JXJChengChiTab);
				crTableIO::DataVec recordVec, recordVecTmp;
				...
				for (MyAllocatedCantonVec::iterator itr = groupCantonVec->begin();
					itr != groupCantonVec->end();
					++itr)
				{
					chengchitab->queryRecords(cantonIdx, crArgumentParser::appItoa(*itr), recordVecTmp);
					if (recordVecTmp.empty()) continue;
					for (crTableIO::DataVec::iterator ditr = recordVecTmp.begin();
						 ditr != recordVecTmp.end();
						++ditr)
					{
						recordVec.push_back(*ditr);
					}
				}
...
void crJXJRecvGroupContributionMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_groupcontribution = LOCREPARAM(param64);
		}
		else
		{
			m_groupcontribution = 0;
...
void crJXJClientRecvGroupContributionMethod::operator()(crHandle &handle)
{
	if(m_stream.valid())
	{
		if(m_netType == GameClient_Game)
		{
			...
			if(extra!=0)
				crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_GroupContribute,extra));	
		}
	}
}
...
void JXJ::crJXJRefreshGroupCantonSpecialtyMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_groupid = (int *)(LOCREPARAM(param64));
			m_playerid = (int *)(HICREPARAM(param64));
		}
		else
		{
			m_groupid = NULL;
		...
		if (mainRoleData && gsBrainData)
		{
			...
			if(groupdata.valid())
			{
				...
				if (chengchiProTab.valid() && juntuanProTab.valid())
				{
					...
					int playerlvIdx = chengchiProTab->getTitleIndex("军团等级");
					...
					if (comsitr != membermap->end() && comsitr->second.valid())
					{
						...
						if (ffcmitr != fenFengedChengChiMap->end())
						{
							...
							for (crTableIO::DataVec::iterator dvitr = recordVec.begin();dvitr !=recordVec.end() && _index <6; ++dvitr)
							{
								...
								if (groupLv >= LvVec[0] && groupLv <= LvVec[1])
								{
						...
						playerlvIdx = juntuanProTab->getTitleIndex("军团等级");
				...
				if(mygroupid > 0)
				{
					...
					if (groupdata.valid())
					{
						...
						if (comsitr != membermap->end() && comsitr->second.valid())
						{
							...
							if (comsitr->second->isGroupcantonspecialtyVecempty() || isrefresh)
							{
								if (isrefresh)
								{
									...
									if (totalprice > 0)
									{
										...
										{
											m_this->doEvent(WCH_JXJRefreshGroupCantonSpecialty,MAKECREPARAM(&mygroupid,&playerid));
							...
							m_this->doEvent(WCH_JXJRecvUpdateCantonSpecialty,MAKECREPARAM(NULL,GameServer));
						}
					}
					gsBrainData->excHandle(MAKECREPARAM(WCH_LockData,0));
				}
	...
	m_telephoneGet(handle.m_telephoneGet)
{
	for (unsigned char i = 0; i < 7; ++ i)
	{
		...
		m_canGet[i] = handle.m_canGet[i];
		m_canGetBackBoard[i] = handle.m_canGetBackBoard[i];
		if (i < 5)
		{
...
void crJXJ7DayHuodongCanvasUpdateMethod::operator()( crHandle &handle )
{
	if (m_this)
	{
		...
		ref_ptr<crStaticTextWidgetNode > canGet[7] = { NULL };
		ref_ptr<crNode > canGetBackBoard[7] = { NULL };
		...
		for (unsigned char i = 0; i < 7; ++ i)
		{
			...
			canGet[i] = dynamic_cast<crStaticTextWidgetNode *>(m_this->getWidget(m_canGet[i]));
			canGetBackBoard[i] = m_this->getChildNode(m_canGetBackBoard[i]);
			if (title_sw[i].valid())
			{
			...
			if (canGet[i].valid())
			{
				canGet[i]->setVisiable(false);
			}
			if (canGetBackBoard[i].valid())
			{
				canGetBackBoard[i]->setVisiable(false);
			}
			if (i < 5)
			{
		...
		do 
		{
			...
			for (unsigned char i = curPage; i < vecSize && j < 7; ++ i, ++ j)
			{
				...
				{
					if (title[j].valid()) title[j]->setString(huoRecord[titlexianshiIdx]);
					...
					if (canGet[j].valid())
					{
						...
						if (mainRole.valid() && mainRole->getDataClass())
						{
							...
							roleData->getParam(WCHDATA_JXJNewHuodongMap, param);
							NewHuodongMap *huodongMap = (NewHuodongMap *)param;
							NewHuodongMap::iterator hitr = huodongMap->find(huodongVec[i]);
							if (hitr != huodongMap->end())
							{
								if (!hitr->second->getComplete())
								{
									if (hitr->second->isCanAdvance() && huodongID != Activation_CodeActivity)
									{
										canGet[j]->setString(std::string("1"));
										canGet[j]->setVisiable(true);
										if (canGetBackBoard[i].valid())
										{
											canGetBackBoard[i]->setVisiable(true);
										}
									}
								}
							}
...
void crJXJUpdate7DayHuoDongMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_type = LOCREPARAM(param64);
			m_chapterid = HICREPARAM(param64);
		}
		else
		{
			m_chapterid = -1;
...
{
	if (m_this && m_this->getMainRole() && m_this->getMainRole()->getDataClass())
	{
			void *param;
			int playerid = m_this->getPlayerID();
			crData *mainroledata = m_this->getMainRole()->getDataClass();
				...
				for(crTableIO::DataVec::iterator siter = huodongIDVec.begin(); siter!=huodongIDVec.end();++siter)
				{
					...
					if(itr != huodongMap.end())
					{
						...
						{	
							if (m_type == 0)
							{
								itr->second->doEvent(WCH_JXJUpdate7DayHuoDong,MAKECREPARAM(NULL,m_this->getPlayerID()));
							}
							...
							else if(m_type >= 3 && m_type < 5)
							{
								...
								if (m_type == 3)
								{
									itr->second->doEvent(WCH_JXJUpdate7DayHuoDong,MAKECREPARAM(killedcounts,m_this->getPlayerID()));
								}
								else if(m_type == 4)
								{
									itr->second->doEvent(WCH_JXJUpdate7DayHuoDong,MAKECREPARAM(deadcounts,m_this->getPlayerID()));
								}
							}
						}
					}
	...
	if(itemData)
	{
		...
		if(*itemState!=IS_Dead)
		{
			...
			if (progresscanvas.valid() && progresscanvas->getDataClass())
			{
				...
				canvasdata->inputParam(WCHDATA_JXJTouchInterval, &time);
	...
	if (headerCanvas.valid() && trunkCanvas.valid())
	{
		...
		//{
			...
			{//神器将卡说明
				...
				//int id = atoi(name.c_str()) - 1;
				std::string cardnodename = m_cardimg + name;
				crStaticTextWidgetNode *textNode = dynamic_cast<crStaticTextWidgetNode *>(m_this);
				if (textNode)
				{
					notice = textNode->getUTF8String();
				}
				crImageBoxWidgetNode *imageBox = dynamic_cast<crImageBoxWidgetNode *>(m_this->getParentCanvas()->getWidget(cardnodename));
				if (imageBox)
				{
					ref_ptr<crTableIO> itemTab = crGlobalHandle::gData()->gGlobalTable(WCHDATA_JXJItemTab);
					...
					int nameindex = itemTab->getTitleIndex("name");
					crTableIO::StrVec itemrecord;
					if (itemTab->queryOneRecord(jkiconindex, imageBox->getImageName(), itemrecord) >= 0)
					{
						titlestr = itemrecord[nameindex] + "神器效果";
					}
				}
			}
		...
		float posx = (tips.m_max[0]-tips.m_min[0])*0.5;
		float posy = (tips.m_max[1]-tips.m_min[1])*0.5;
		crVector3 pos(iconbox.center()[0],iconbox.m_max[1],0.0f);
		/////////////////////////////////////////
		crVector3 pos2(iconbox.center()[0] ,iconbox.m_min[1],0.0f);
		pos = pos * mat;
		pos2 = pos2 * mat;
		float posx2,posy2;
		/////////////////////////////////////////                             
		...
		if(mainRole && mainRole->getDataClass())
		{
			...
			for (; itr != huodongMap->end(); ++ itr)
			{
				...
				if(itr->second->getActivation()==1 || (itr->second->getComplete() && itr->second->isIntimeCheck()))
				{
					...
					{
						if(atoi(record[displayid].c_str()) == Huodong_Huikui)
						{//充值回馈
...
void crJXJNewHuiKuiCanvasUpdateMethod::addParam( int i, const std::string& str )
{
	...
	case 126:
	case 127:
		m_superEquipTitle[i - 124] = str;
		break;
	case 128:
	case 129:
	case 130:
	case 131:
		m_superEquipText[i - 128] = str;
		break;
	case 132:
	case 133:
	case 134:
	case 135:
		m_superEquipButton[i - 132] = str;
		break;
	case 136:
		m_pagesw = str;
		break;
...
void crJXJNewHuiKuiCanvasUpdateMethod::setTitleText(int tiaojian1,int tiaojian2, int title, int neirong,int cishu, const CREncapsulation::crTableIO::StrVec &record, crHypertextWidgetNode * hyp, CRUI::crStaticTextWidgetNode *text)
{
	...
	else if(type.compare(std::string("今日充值")) == 0 || type.compare(std::string("多日充值")) == 0 || type.compare(std::string("神器充值")) == 0)
	{
		if (strVec.size() >= 2)
		{
			std::vector<float>v_i;
		...
		//CRCore::notify(CRCore::ALWAYS) << type <<":"<< strVec.size()<<" text:"<< text << std::endl;
		if (text && strVec.size() >= 3 && type.compare(std::string("神器充值")) == 0)
		{
			char buf[64];
			sprintf_s(buf, "神器升级每级额外获得%s点注灵点,该效果可轮回继承。\0", strVec[2].c_str());
...
void crJXJNewHuiKuiCanvasUpdateMethod::operator()( crHandle &handle )
{
	...
	if (myPlayer && myPlayer->getDataClass())
	{
		...
		ref_ptr<crHypertextWidgetNode> superEquipTitle[4];
		ref_ptr<crStaticTextWidgetNode> superEquipText[4];
		ref_ptr<crButtonWidgetNode> superEquipButton[4];
		...
		for (unsigned char i = 0; i < 18; ++i)
		{
			...
			if (i < 4)
			{
				superEquipTitle[i] = dynamic_cast<crHypertextWidgetNode *>(m_this->getWidget(m_superEquipTitle[i]));
				if (superEquipTitle[i].valid())
				{
					superEquipTitle[i]->clearText();
				}
				superEquipText[i] = dynamic_cast<crStaticTextWidgetNode *>(m_this->getWidget(m_superEquipText[i]));
				//CRCore::notify(CRCore::ALWAYS) << m_superEquipText[i] << ":" << superEquipText[i].get()<< std::endl;
				superEquipButton[i] = dynamic_cast<crButtonWidgetNode *>(m_this->getWidget(m_superEquipButton[i]));
			}
			icon[i] = dynamic_cast<crImageBoxWidgetNode *>(m_this->getWidget(m_icon[i]));
			if (icon[i].valid())
			{
			...
			do 
			{
				...
				for (HuiKuiHuoDongMap::iterator hkmitr = HKhuodongmap.begin(); hkmitr != HKhuodongmap.end() && _index < 10; ++hkmitr,++_index)
				{
					if (!hkmitr->second.empty())
					{
						...
						if (hdIdx == _index)
						{
							if (hkmitr->first == "神器充值")
							{
								//CRCore::notify(CRCore::ALWAYS) <<4<<" "<<hkmitr->first <<" pagesw="<<m_pagesw<< std::endl;
								pagesize = 4;
								if (pagesw.valid())
							...
							{
								//CRCore::notify(CRCore::ALWAYS) << 3 << " " << hkmitr->first << " pagesw=" << m_pagesw << std::endl;
								pagesize = 3;
								if (pagesw.valid())
							...
							numIdx = 0;
							int val = 0;
							if (pagesw->getActiveSwitchSet() != 1)
							{
								if (scroll.valid())
								{
									if (num <= pagesize)
									...
									{
										scroll->setEnable(true);
										scroll->setRange(0, num - pagesize);
										scroll->setLineValue(1.0f);
										scroll->setPageValue(pagesize);
									}
									val = scroll->getValue();
								}
							}
								...
								if (numIdx >= val && iconIdx < pagesize)
								{
									...
									if (huodongTab->queryOneRecord(0,crArgumentParser::appItoa(huodongid),huodongrecord) >= 0)
									{
										...
										if (titleIdx >= 0 && contentIdx >= 0)
										{
											...
											if (hkmitr->first == "神器充值")
											{
												if (superEquipTitle[iconIdx].valid())
												{
													setTitleText(tiaojian1Idx, tiaojian2Idx, titleIdx, contentIdx, circlesIdx, huodongrecord, superEquipTitle[iconIdx].get(), superEquipText[iconIdx].get());
												}
											}
											else
											{
											...
											else if (hdtitle.compare(std::string("神器充值")) == 0)
											{
												if (superEquipButton[iconIdx].valid())
												{
													roleData->getParam(WCHDATA_JXJSuperEquipLvupPoint, param);
													unsigned char superEquipLvupPoint = *(unsigned char *)param;
													int awardIdx = huodongTab->getTitleIndex("奖励1");
													crVector2i superAward;
													crArgumentParser::appAtoVec(huodongrecord[awardIdx], superAward);

													superEquipButton[iconIdx]->setVisiable(true);
													superEquipButton[iconIdx]->setEnable(false);
													crData* uiData = superEquipButton[iconIdx]->getDataClass();
													if (uiData)
													{
														int gold = atoi(huodongrecord[tiaojian1Idx].c_str());
														uiData->inputParam(WCHDATA_JXJUIID, &gold);
													}
													if (superEquipLvupPoint < superAward[1] && hdmitr != huodongMap->end())
													{
														if (hdmitr->second->isActivation() && !hdmitr->second->isCanAdvance())
														{
															superEquipButton[iconIdx]->setEnable(true);
														}
													}
												}
											}
...
void JXJ::crJXJRecvClientNewHuiKuiGetAwardsMethod::operator()( crHandle &handle )
{
	if(m_this && m_stream.valid())
	{
		...
		else if(m_netType == GameClient_Game)
		{
			...
			{
			case -1:
				m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(2036, NULL));//元宝不足
				break;
			default:
				break;
			}
