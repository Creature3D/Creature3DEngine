...
void crJXJNotInSightMethod::operator()(crHandle &handle)
{
	...
	if(m_this && m_stream.valid())
	{
		if (m_netType == GameClient_Game &&crRunGameHandle::getInstance()->isInBattle())
		{
			...
			if (!camera1.valid())
				return;
			camera1->getAttachedNode()->doEvent(WCH_JXJNotInSight2MsgData, MAKECREPARAM(&pos,NULL));
...
void crJXJVIPRevenuesBonusMethod::inputParam(int i, void* param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_cashPtr = (int*)(HICREPARAM(param64));
			m_cash1Ptr = (int*)(LOCREPARAM(param64));
		}
		break;
	}
}
...
void crJXJAdjustToushicheBulletSpeedMethod::inputParam(int i, void* param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_firer = (crInstanceItem*)(LOCREPARAM(param64));
		}
		break;
	}
}
...
void crJXJShowCityInfoMethod::operator()(crHandle &handle)
{
	...
	if(chengchitab->queryOneRecord(buttonnameindex, btName, record)>=0)
	{
		...
		if (guishu_juntuan)
		{
			...
			if (cityMapData)
			{
				cityMapData->excHandle(MAKECREPARAM(WCH_LockData,1));
				...
				cityMapData->excHandle(MAKECREPARAM(WCH_LockData,0));
			}
