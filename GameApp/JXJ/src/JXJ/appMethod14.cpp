...
void crJXJBattleBuffMethod::operator ()(crHandle &handle)
{
	...
	{
		playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(9100,NULL));//犒赏失败
	}
}
		...
		if (resaddbuildingcanvas.valid() && needviplvindex >= 0 && buildinglvindex >= 0 && priceindex >= 0)
		{
			...
			if (ResAddBuildingTab->queryOneRecord(0, crArgumentParser::appItoa(buildingid), buildinglvindex, crArgumentParser::appItoa(buildinglv + 1), ResAddBuildingrecord) >= 0)
			{
				...
				if (viplv >= needviplv)
				{
					...
					int tempprice = price;
					playerGameData->doEvent(WCH_JXJCheckLijinGold, MAKECREPARAM(&tempprice, NULL));
					if (tempprice == 0)
					{
						conditionfull = true;
					}
	...
	if(m_this && m_stream.valid())
	{
		...
		if(m_netType == GameServer)
		{
			...
			if(playerGameServerPlayerData.valid())
			{
				...
				serverBrainData->excHandle(MAKECREPARAM(WCH_LockData,1));
				serverBrainData->getParam(WCHDATA_JXJServerCycleTimer, param);
				float cycleTimer = *(float *)param;
				serverBrainData->excHandle(MAKECREPARAM(WCH_LockData,0));
			...
			if(playerData.valid())
			{
				...
				if (hasitem)
				{
					...
					if(dataObject)
					{
						...
						//dataObject->doEvent(MAKEINT64(WCH_JXJUseBagItem,GameServer),MAKECREPARAM(m_this,&param));
...
void crJXJZhengZhanShopCanvasUpdateMethod::operator()( crHandle &handle )
{
	if (m_this)
	{
		...
		ref_ptr<crStaticTextWidgetNode > count[12] = {NULL};
		...
		if (scroll.valid())
		{
			if (cRow > 12)
			{
				scroll->setRange(0, cRow - 12);
				scroll->setLineValue(6);
				scroll->setPageValue(12);
		...
		for (unsigned char i = 0; i < 12; ++ i) 
		{
			...
			if(rowid < cRow)
			{
				std::string counts = itemtab->getData(rowid,shuLiang);
				std::string needLv = itemtab->getData(rowid,dengJi);
				std::string price = itemtab->getData(rowid,danJia);
				int itemid = atoi(itemtab->getData(rowid,itemidin).c_str());
				unsigned char buyshangxian = atoi(itemtab->getData(rowid,goumaishangxian).c_str());
				...
				if (imageBox[i].valid())
				{
					uidata = imageBox[i]->getDataClass();
					if (uidata)
					{
						uidata->inputParam(WCHDATA_JXJUIID, &itemid);
					}
					if (buytop>0)
					{
						imageBox[i]->setVisiable(true);
						imageBox[i]->setImageName(itemtab->getData(rowid,4));
					}
					else
					{
						imageBox[i]->setVisiable(false);
				...
				if (gouMaiBtn[i].valid())
				{
					uidata = gouMaiBtn[i]->getDataClass();
					if (uidata)
					{
						uidata->inputParam(WCHDATA_JXJUIID, &itemid);
					}
...
void crJXJRecvZhengZhanShopBuyMethod::addParam(int i, const std::string& str)
{
	...
	case 2:
		...
		break;
	case 3:
		crArgumentParser::appAtoVec(str, m_task);
		break;
	default:
		break;
	}
	...
	if(m_this && m_stream.valid())
	{
		...
		else if(m_netType == GameClient_Game)
		{
			...
			/*	ref_ptr<crCanvasNode> animatinoCanvas = crFilterRenderManager::getInstance()->findCanvas(m_animation);
				...
				if (m_task[0] > 0)
				{
					myPlayer->doEvent(WCH_JXJAdvanceTask, MAKECREPARAM(m_task[0], m_task[1]));
				}
			} 
			else
			{
...
void crJXJRecvZhengZanQuanMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_zhengzhanquan = LOCREPARAM(param64);
		}
		else
		{
			m_zhengzhanquan = 0;
	...
	if(m_stream.valid())
	{
		if(m_netType == GameClient_Game)
		{
			...
			if(data)
			{
				...
				if(extra!=0)
					crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_Zhanquan,extra));
...
void crJXJRecvGuoZhanJiFenMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_guozhanjifen = LOCREPARAM(param64);
		}
		else
		{
			m_guozhanjifen = NULL;
...
void crJXJClientRecvGuoZhanJiFenMethod::operator()(crHandle &handle)
{
	if (m_this && m_stream.valid())
	{
		if (m_netType == GameClient_Game)
		{
			...
			if(extra!=0)
				crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_GuoZhanJiFen,extra));
	...
	if(scene)
	{
		...
		if (data)
		{
			...
			if(zhengtingitem.valid() && zhengtingitem->getDataClass())
			{
				...
				//zhengtingitem->doEvent(WCH_GetHP,MAKECREPARAM(&maxhp,NULL));
...
void crJXJActiveTaskAfterCountryFoundedMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_isFirstWeek = (bool )(LOCREPARAM(param64));
			m_shiliID = (unsigned char )(HICREPARAM(param64));
		}
		else
		{
			m_isFirstWeek = false;
...
void crJXJtreasonbuffCanvasUpdateMethod::operator()(crHandle &handle)
{
	...
	if(playerData)
	{
		...
		myPlayer->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&isselectedCountrylose,&selectedshili));
		...
		else
		{
			...
			myPlayer->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&isLoseCountry,&myShiliid));
			if (isLoseCountry)
				*treasonbuffTimer = 0;
...
void crJXJRecvClearTreasonBuffTimeMethod::operator()(crHandle &handle)
{
	...
	if(m_netType == GameServer)
	{
		...
		if(playerData)
		{
			...
			int needgold = 10; //atoi(crGlobalHandle::gData()->gGameGlobalValue(WCHDATA_JXJChangeShiLiNeedGold,vipLevel).c_str());
			playerData->getParam(WCHDATA_JXJGoldingot,param);
			int goldingot = *(int*)param;
			playerData->getParam(WCHDATA_JXJGiftGold,param);
			int giftgold = *(int*)param;
			...
			else
			{
				...
				*buffertime = 0;
				...
				m_this->doEvent(WCH_JXJChangeShili,MAKECREPARAM(&shiliid,NULL));
				_case = 1;
	...
	{
		//提示元宝不足
		m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(2002, NULL));
	}
}
...
void crJXJCheckInTreasonPunishTimeMethod::operator()(crHandle &handle)
{
	...
	if (myPlayer && myPlayer->getDataClass())
	{
		...
		{
			cancelTask = true;
			myPlayer->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(9160, NULL));
		}
	}
...
void crJXJRecvCityInBAattleMethod::operator()(crHandle &handle)
{
	...
	if(m_netType == GameServer)
	{
		...
		if (dat && data)
		{
			...
			dat->excHandle(MAKECREPARAM(WCH_LockData,1));
			...
			if (playerid == chengxiangid)
				...
				*guanzhi = 0;
			myGuanzhi = *guanzhi;
			dat->excHandle(MAKECREPARAM(WCH_LockData,0));
		}
	...
	else if(m_netType == GameClient_Game)
	{
		...
		crPlayerGameData *playerData = crMyPlayerData::getInstance()->getPlayerGameData();
		if (!playerData && !playerData->getDataClass()) return;
		crData *data = playerData->getDataClass();
		data->inputParam(WCHDATA_JXJPlayerGuanZhi,&myGuanzhi);
		if (_case != 0 && playerData->getMainRole() && playerData->getMainRole()->getDataClass())
		{
			crData *roledata = playerData->getMainRole()->getDataClass();
			roledata->getParam(WCHDATA_Level, param);
			unsigned char characterlevel = *(unsigned char *)param;
			...
			int battlelv_1 = battlelv;
			if (tianshu >= IncreaseDate && tianshu < maxBattlelv - IncreaseDate)
				battlelv_1 = battlelv + tianshu - IncreaseDate + 1;
...
void crJXJGuoJiaZiJinJiangLiMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_dead = (crInstanceItem*)(LOCREPARAM(param64));
			m_fire = (crInstanceItem*)HICREPARAM(param64);
		}
		else
		{
			m_dead = NULL;
...
void crJXJGuoJiaZiJinJiangLiMethod::operator()(crHandle &handle)
{
	...
	if (m_fire.valid())
	{
		crData *fireData = m_fire->getDataClass();
		void *param;
		unsigned char shiliid = 0;
		fireData->getParam(WCHDATA_Camp, param);
		shiliid = *(unsigned char *)param;

		int playerid = m_fire->getPlayerID();
		_crInt64 ownid = m_fire->getOwnerID();
		if (ownid != 0L)
		{
			playerid = LOINT64(ownid);
		}
		...
		if (vec3[2] > 0)
		{
			...
			for (int i=0; i<size; i++)
			{
				...
				if (gsplayerData.valid())
				{
					...
					{
						playerdata->excHandle(MAKECREPARAM(WCH_LockData,1));
						gsplayerData->getPlayerGameData()->doEvent(WCH_JXJRecvPlayerRewardInfo,MAKECREPARAM(reward_info.get(),NULL));
						playerdata->excHandle(MAKECREPARAM(WCH_LockData,0));
					}
				}
			}
		...
		if (playerData && playerData->getDataClass())
		{
			...
			if (characterlevel >= openlv)
			{
				if (getAward == (unsigned char)-1) 
				{
					...
					playerData->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&isLose,&shiliid));
...
void crJXJCityHaveDequeUpdateMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_chengchiid = LOINT32(LOCREPARAM(param64));
			m_type = HIINT32(LOCREPARAM(param64));
			m_attactshili = LOINT16(LOINT32(HICREPARAM(param64)));
			m_defenedshili = HIINT16(LOINT32(HICREPARAM(param64)));
		}
		else
		{
			m_stream = NULL;
...
void crJXJBattleBaseNoticeUpdataMethod::operator ()(crHandle &handle)
{
	if (m_this)
	{
		...
		if (inputnotice.valid())
		{
			...
			for (std::deque<std::string>::iterator itr = tonggaoqueue->begin(); itr != tonggaoqueue->end(); itr++)
			{
				std::string text;
...
void crJXJBattleBehaveFrameMethod::operator ()(crHandle &handle)
{
	if (m_this && m_this->getDataClass())
	{
		...
		cavasdata->excHandle(MAKECREPARAM(WCH_LockData,1));
		...
		cavasdata->excHandle(MAKECREPARAM(WCH_LockData,0));
	}
}
...
void crJXJBattleShowNoticeCanvasMethod::operator ()(crHandle &handle)
{
	...
	if(showCanvas.valid() && noticeCanvas.valid())
	{
		...
		noticeCanvas->setMatrix(crMatrix::translate(bbox.xMax() - canvas_box.xLength()*0.5f, bbox.yMax() + canvas_box.yLength()*0.5f - crDisplaySettings::instance()->getUIPixelMeter()*crDisplaySettings::instance()->getUIHeightScale() / crDisplaySettings::instance()->getUIScaleFactor(), 0.0f));
...
void crJXJCountryOfficialLeaveMethod::inputParam(int i, void *param)
{
	...
	case 2:
		if(param)
		{
			...
			m_playerGameData = (crPlayerGameData *)HICREPARAM(param64);
		}
		break;
	}
}
	...
	if(m_this && m_stream.valid())
	{
		if(m_netType == GameClient_Game)
		{
			...
			if (rankcanvas.valid() && rankcanvas->getDataClass())
			{
				...
				{
					unsigned char points = m_stream->_readUChar();
					short count = m_stream->_readShort();
					int playerid = m_stream->_readInt();
					std::string name = m_stream->_readString();
					data.playerid = playerid;
					...
					if(iter != pointsrankmap->end())
					{
						...
						pointsrankmap->insert(std::make_pair((int)(points/**5*/),data));

					if (myid == playerid)
					{
						myinfo[3] += (int)count;
						myinfo[5] += (int)(points/**5*/);
					}
					(*countrypoints) += (int)(points/**5*/);
				}
				...
				else if(_case == 4) //杀死小怪更新积分排行榜
				{
					...
					if(iter != pointsrankmap->end())
					{
						...
						pointsrankmap->insert(std::make_pair((int)(points/**10*/),data));

					if (myid == playerid)
					{
						myinfo[5] += (int)(points/**10*/);
					}
					(*countrypoints) += (int)(points/**10*/);
				}
...
void crJXJButtonGetJiFenAwardMethod::operator()(crHandle &handle)
{
	...
	if(netConductor)
	{
		...
		if (playergamedata.valid() && playergamedata->getDataClass())
		{
			...
			{
				playergamedata->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(9179, NULL)); //领取次数不够
			}
			else
			{
				ref_ptr<crStreamBuf> stream = new crStreamBuf;
...
void crJXJRecvButtonGetJiFenAwardMethod::operator()(crHandle &handle)
{
	if(m_stream.valid() && m_this)
	{
		...
		else if (m_netType == GameClient_Game)
		{
			...
			if (_case == -2)
			{
				m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(9178, NULL)); //元宝不够
			}
			else if (_case == -1)
			{
				m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(9179, NULL)); //领取次数不够
			}
			else if (_case == -0)
			{
				m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(9180, NULL)); //没有完成活动
			}
			else if (_case == 1)
			{
				ref_ptr<crData> data = m_this->getDataClass();
...
void crJXJServerDisposeShowPraiseMethod::operator()(crHandle &handle)
{
	if(m_stream.valid() && m_this)
	{
		...
		else if(m_netType == GameClient_Game)
		{
			...
			if (_case == 2)
			{
				m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(9184, NULL)); //玩家被赞次数已满
			}
			else if (_case == 4)
			{
				m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(9216, NULL)); //玩家不在先线
			}
			else if (_case == 1)
			{
...
void crJXJpraiseLinkClickMethod::operator()(crHandle &handle)
{
	...
	{
		crMyPlayerData::getInstance()->getPlayerGameData()->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(9215, NULL)); //只能赞一次
	}
}
...
void crJXJWorldFuBenStatusUpdataMethod::operator ()(crHandle &handle)
{
	...
	if (m_this && m_this->getDataClass() && canvas.valid() && canvas->getDataClass())
	{
		...
		statusdata->excHandle(MAKECREPARAM(WCH_LockData,1));
		...
		statusdata->excHandle(MAKECREPARAM(WCH_LockData,0));
	}
}
		...
		if (m_netType == GameServer)
		{
			...
			if (m_this->getMainRole() && m_this->getMainRole()->getDataClass() && m_this->getDataClass())
			{
				...
				*count = *count + playernpcdead[2];
				...
				TroopsAchieveMap * bingzhonginfomap = (std::map<int,CRCore::crVector3i> *)param;
				TroopsAchieveMap::iterator bingzhongitr;

				TroopsAchieveMap::iterator troopitr = troopsinfomap.begin();
				...
				//playerData->getParam(WCHDATA_JXJTroopsAchieveInfo,param);
				//TroopsAchieveMap bingzhongmap = *(TroopsAchieveMap *)param;
				playerData->getParam(WCHDATA_JXJKillEachTroopsCount,param);
		...
		else if(m_netType == GameClient_Game)
		{
			if (m_this->getMainRole() && m_this->getMainRole()->getDataClass() && m_this->getDataClass())
			{
				...
				*ckilldoorboss = m_stream->_readVec4s();
				...
				playerData->getParam(WCHDATA_JXJTroopsAchieveInfo,param);
				TroopsAchieveMap * bingzhonginfomap = (TroopsAchieveMap *)param;
				TroopsAchieveMap::iterator bingzhongitr;
...
crJXJDetroyOppositeCountryMethod::crJXJDetroyOppositeCountryMethod(const crJXJDetroyOppositeCountryMethod& handle):
	...
	m_achieveid6(handle.m_achieveid6)
{
}
...
void crJXJDetroyOppositeCountryMethod::addParam(int i, const std::string& str)
{
	...
	case 4:
		...
		break;
	case 5:
		crArgumentParser::appAtoVec(str, m_achieveid6);
		break;
	}
}
void crJXJDetroyOppositeCountryMethod::operator()(crHandle &handle)
{
	...
	if(myPlayer)
	{
		...
		if((achieveid- m_achieveid1) == (shiliid-c_startShiliID)) return;
		...
		if (canvas.valid() && canvas->getDataClass())
		{
			...
			else if (achieveid == m_achieveid5 || achieveid == m_achieveid6[0])
			{
				bool isLoseCountry = false;
				myPlayer->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&isLoseCountry,&shiliid));
				if(!isLoseCountry)
				{
					...
					{
						isLoseCountry = false;
						myPlayer->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&isLoseCountry,&othershili));
						if(isLoseCountry)
						{
							step++;
						}
					...
					if (step == 2 && achieveid == m_achieveid6[0])
					{//天数校验
						//playerdata->getParam(WCHDATA_JXJGameStartedDays, param);
						//unsigned short startDays = *(unsigned short *)param;
						int tianshu = 0;
						crGlobalHandle::getInstance()->doEvent(WCH_JXJGetStartedDays, MAKECREPARAM(&tianshu, NULL));
						if (tianshu<=m_achieveid6[1])
							step++;
					}
					m_this->setStep(step);
				}
...
void crJXJDetroyOppositeCountrCheckMethod::operator()(crHandle &handle)
{
	if(m_player && m_player->getDataClass())
	{
		...
		if((achieveid- m_achieveid1) == (myshiliid-c_startShiliID)) return;
		...
		else if(achieveid == m_achieveid5 || achieveid == m_achieveid6[0])
		{
			...
			if(!isLoseCountry)
			{
				...
				{
					isLoseCountry = false;
					crServerBrainHandle::getInstance()->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&isLoseCountry,&othershili));
					if(isLoseCountry)
					{
						step++;
					}
	...
	if(m_this && m_stream.valid())
	{
		if(m_netType == GameServer)
		{
			...
			if (playerData.valid() &&  m_this->getMainRole() && m_this->getMainRole()->getDataClass())
			{
				...
				//if(code == 0)
				//{
					std::string logdata = "成就商店兑换：(code,id)"+crArgumentParser::appVectoa(crVector2i(code,id));
		...
		else if(m_netType == GameClient_Game)
		{
			...
			case 0:
				...
				break;
			case -1://成就点数不足
				m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(1117, NULL));
				break;
			case -2://等级不足
				m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(1118, NULL));
				break;
			case -3://需要完成相关的成就
				m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(1119, NULL));
				break;
			case -4://需要完成相关的成就
				m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(1120, NULL));
				break;
			}
		}
	}
...
void crJXJGuoZhanAchieveMethod::operator()(crHandle &handle)
{
	...
	if(myPlayer && myPlayer->getDataClass())
	{
		...
		if (accomplish_table->queryOneRecord(0,crArgumentParser::appItoa((int)achieveid),record) >=0)
		{
			...
			if (notevec[1] > 0)
			{
				if (notevec[1] == WCHDATA_JXJTroopsAchieveInfo)
				{
					playergdata->lock();
					playergdata->getParam(notevec[1], param);
					TroopsAchieveMap* troopsinfomap = (TroopsAchieveMap *)param;
					if (notevec[2] < 100)
					{//兵种等级
						ref_ptr<crTableIO> troopsTab = crGlobalHandle::gData()->gGlobalTable(WCHDATA_JXJTroopsTab);
						int lvindex = troopsTab->getTitleIndex("兵种等级");
						crTableIO::StrVec troopsrecord;
						for (TroopsAchieveMap::iterator itr = troopsinfomap->begin();
							itr != troopsinfomap->end();
							++itr)
						{
							if (itr->second[notevec[3]] > 0)
							{
								//CRCore::notify(CRCore::FATAL) << "id=" << itr->first << " Vec=" << itr->second << " nowstep=" << nowstep << std::endl;
								if (troopsTab->queryOneRecord(0, crArgumentParser::appItoa(itr->first), troopsrecord) >= 0)
								{
									if (atoi(troopsrecord[lvindex].c_str()) >= notevec[2])
									{
										nowstep += itr->second[notevec[3]];
										//CRCore::notify(CRCore::FATAL) <<"成就id="<< achieveid <<"nowstep="<<nowstep<<" 加量="<<itr->second[notevec[3]]<< std::endl;
									}
								}
							}
						}
					}
					else if (notevec[2] >= 20001)
					{//兵种
						TroopsAchieveMap::iterator troopinfoitr = troopsinfomap->find(notevec[2]);
					...
					{//兵种等级
							...
							++itr)
						{
							if (itr->second[notevec[3]] >= 0)
							{
								if (troopsTab->queryOneRecord(0, crArgumentParser::appItoa(itr->first), troopsrecord) >= 0)
								{
									if (atoi(troopsrecord[lvindex].c_str()) >= 3)
									{
										nowstep += itr->second[notevec[3]];
									}
								}
...
void crJXJRecvAchievePointMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_achievepoint = LOCREPARAM(param64);
		}
		else
		{
			m_achievepoint = 0;
...
void crJXJUIAchieveShopCommodityTipsMethod::operator()(crHandle &handle)
{
	...
	if(tipsCanvas.valid() && playerData && infocanvas.valid() && parentcanas.valid())
	{
		ref_ptr<crTableIO> shoptab = crGlobalHandle::gData()->gGlobalTable(m_tabid);//WCHDATA_JXJAchieveShopTab
		//int cRow = shoptab->getRowCount();
		//ref_ptr<crScrollBarWidgetNode>  scroll = dynamic_cast<crScrollBarWidgetNode *>(parentcanas->getWidget(m_canvas));
		//int val = 0;
		int itemtype = 0;
		...
		int id = *(int *)param;
		crTableIO::StrVec record;
		if (shoptab->queryOneRecord(0, crArgumentParser::appItoa(id), record)>=0)
		{
			int typein = shoptab->getTitleIndex("类型");
			int countin = shoptab->getTitleIndex("数量");
			itemtype = atoi(record[typein].c_str());
			shulingstr = record[countin];
		}
...
void JXJ::crJXJXinShouGuoZhanJieSuanMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_playerid = (int)LOCREPARAM(param64);
			m_fubenid = (int)HICREPARAM(param64);
		}
		else
		{
			m_playerid = 0;
		...
		if (m_this && gameServer && m_this->getDataClass())
		{
			...
			if(playerData.valid() && playerData->getPlayerGameData() && playerData->getPlayerGameData()->getDataClass())
			{
				...
				if (fubenrewardtab->queryOneRecord(0,crArgumentParser::appItoa(m_fubenid),record) >= 0)
				{
					...
					if (mainRole)
					{
						mainRole->doEvent(WCH_JXJRecvExperience,MAKECREPARAM(experencec,NULL));
						mainRole->doEvent(WCH_JXJRecvExploit,MAKECREPARAM(exploitc,NULL));
	...
	case 17:
		{
			...
			m_strCardImage[i - 3][0] = strVec[0];
			m_strCardImage[i - 3][1] = strVec[1];
		}
		break;
	case 18:
	case 19:
	...
	case 68:
		{
			std::vector<std::string> strVec;
			crArgumentParser::appAtoVec(str, strVec);
			m_strItemImage[i - 49][0] = strVec[0];
			m_strItemImage[i - 49][1] = strVec[1];
		}
		break;
	case 69:
	case 70:	
...
void crJXJLunHuiCanvasUpdateNewMethod::operator()(crHandle &handle)
{
	...
	if(radioGroup.valid() && mainRole && itemtab.valid())
	{
		...
		if (select == 0 || select == 1)
		{
			...
			ref_ptr<crStaticTextWidgetNode> vipexpText = dynamic_cast<crStaticTextWidgetNode *>(m_this->getWidget(m_vipexp));
			ref_ptr<crStaticTextWidgetNode> totalrechargeText = dynamic_cast<crStaticTextWidgetNode *>(m_this->getWidget(m_totalrecharge));
			...
			if(playerStore.valid())
			{
				...
				if(playerStoreData)
				{
					if (select == 0)
					{
						...
						playerStoreData->getParam(WCHDATA_VipExp, param);
						int vipexp = *(int *)param;
						if (vipexpText.valid())
						{
							vipexpText->setString(crArgumentParser::appItoa(vipexp));
						}
					}
					else if (select == 1)
					{
						...
						if (totalrechargeText.valid())
							totalrechargeText->setString(crArgumentParser::appItoa(totalmoney));
					}
				}
			}
		...
		else if(radioGroup->getSelect() == 2)
		{
			...
			ref_ptr<crRadioGroupWidgetNode> cardPage = dynamic_cast<crRadioGroupWidgetNode *>(m_this->getWidget(m_cardPage));
			...
			crCheckBoxWidgetNode *checkCard[CycleCardOnePageCount] = { NULL };
			crImageBoxWidgetNode *imageCard[CycleCardOnePageCount] = {NULL};
			crMultiSwitch * cardQulitySw[CycleCardOnePageCount] = { NULL };
			crMultiSwitch * cardstatusSw[CycleCardOnePageCount] = { NULL };
			for (int i = 0; i<CycleCardOnePageCount; i++)
			{
				checkCard[i] = dynamic_cast<crCheckBoxWidgetNode *>(m_this->getWidget(m_strCardImage[i][0]));
				imageCard[i] = dynamic_cast<crImageBoxWidgetNode *>(m_this->getWidget(m_strCardImage[i][1]));
			...
			if(playerStore.valid())
			{
				...
				if(playerStoreData)
				{
					...
					//{
					//	if (itr->valid())
					//		CRCore::notify(CRCore::ALWAYS) << "cardid=" << (*itr)->getItemID() << ",count=" << (*itr)->getItemCount() << std::endl;
					//}
					int cardid;
					for (int i = 0; i<CycleCardOnePageCount; i++)
					{
						...
						if(imageCard[i])
						{
							if(cardid<cardVec->size())
							{
								imageCard[i]->setVisiable(true);
								
								if(itemtab.valid() && (*cardVec)[cardid].valid() && \
									itemtab->queryOneRecord(0,crArgumentParser::appItoa((*cardVec)[cardid]->getItemID()),itemrecord)>=0)
								{
									...
									if(cardstatusSw[i])
									{
										...
										if (1 == (*cardVec)[cardid]->getItemHasCycle())
										{
											cardstatusSw[i]->setActiveSwitchSet(2);
											if (checkCard[i])
											{
												checkCard[i]->setEnable(true);
											}
										}
										else
			...
			if(!cardMap->empty())
			{
				for (ItemOrCardMap::iterator iter=cardMap->begin(); iter!=cardMap->end(); iter++)
				{
					int index = iter->first-offset;
					if (index >= 0 && index < CycleCardOnePageCount)
					{
		...
		else if(radioGroup->getSelect() == 3)
		{
			...
			ref_ptr<crRadioGroupWidgetNode> itemPage = dynamic_cast<crRadioGroupWidgetNode *>(m_this->getWidget(m_itemPage));
			int page = itemPage->getSelect();
			...
			crCheckBoxWidgetNode *checkItem[CycleItemOnePageCount] = { NULL };
			crImageBoxWidgetNode *imageItem[CycleItemOnePageCount] = {NULL};
			crMultiSwitch * itemstatusSw[CycleItemOnePageCount] = { NULL };
			for (int i = 0; i<CycleItemOnePageCount; i++)
			{
				checkItem[i] = dynamic_cast<crCheckBoxWidgetNode *>(m_this->getWidget(m_strItemImage[i][0]));
				itemstatusSw[i] = dynamic_cast<crMultiSwitch *>(m_this->getChildNode(m_strItemStatus[i]));
				imageItem[i] = dynamic_cast<crImageBoxWidgetNode *>(m_this->getWidget(m_strItemImage[i][1]));
				if (checkItem[i])
				{
					checkItem[i]->setEnable(false);
				}
				if(imageItem[i])
				{
			...
			if(playerStore.valid())
			{
				...
				if(playerStoreData)
				{
					...
					//{
					//	if(itr->valid())
					//		CRCore::notify(CRCore::ALWAYS) << "itemid=" << (*itr)->getItemID() << ",count=" << (*itr)->getItemCount() << std::endl;
					//}
					int itemid;
					for (int i = 0; i<CycleItemOnePageCount; i++)
					{
						...
						if(imageItem[i])
						{
							if(itemid<itemVec->size())
							{
								imageItem[i]->setVisiable(true);
								
								if(itemtab.valid() && (*itemVec)[itemid].valid() && \
									itemtab->queryOneRecord(0,crArgumentParser::appItoa((*itemVec)[itemid]->getItemID()),itemrecord)>=0)
								{
									//int qualityIndex = itemtab->getTitleIndex("品质");
									...
									if(itemstatusSw[i])
									{
										...
										if (1 == (*itemVec)[itemid]->getItemHasCycle())
										{
											itemstatusSw[i]->setActiveSwitchSet(2);
											if (checkItem[i])
											{
												checkItem[i]->setEnable(true);
											}
										}
										else
			...
			if(!itemMap->empty())
			{
				...
				{
					int index = iter->first - offset;
					if (index >= 0 && index < CycleItemOnePageCount)
					{
						int itemid = iter->second->getItemID();
						if (itemtab.valid() && itemtab->queryOneRecord(0, crArgumentParser::appItoa(itemid), itemrecord) >= 0)
						{
...
//////////////////////////////////////////////////////////////////////////
//
//crJXJUILunHuiPageChangeMethod
//切页变化
//
//////////////////////////////////////////////////////////////////////////
crJXJUILunHuiPageChangeMethod::crJXJUILunHuiPageChangeMethod()
{}
crJXJUILunHuiPageChangeMethod::crJXJUILunHuiPageChangeMethod(const crJXJUILunHuiPageChangeMethod& handle)
{
}
void crJXJUILunHuiPageChangeMethod::inputParam(int i, void *param)
{
	switch (i)
	{
	case 1:
...
void crJXJUILunHuiPageChangeMethod::addParam(int i, const std::string& str)
{
	switch (i)
	{
	case 0:
...
void crJXJUILunHuiPageChangeMethod::operator()(crHandle &handle)
{
	crCanvasNode *canvas = m_this->getParentCanvas();
	ref_ptr<crCheckBoxWidgetNode> cardCheck[CycleCardOnePageCount] = { NULL };
	for (int i = 0; i < CycleCardOnePageCount; i++)
	{
		cardCheck[i] = dynamic_cast<crCheckBoxWidgetNode *>(canvas->getWidget(m_card[i]));
		if (cardCheck[i].valid() && cardCheck[i]->getSelect())
		{
			cardCheck[i]->select(false);
		}
	}
	ref_ptr<crCheckBoxWidgetNode> itemCheck[CycleItemOnePageCount] = { NULL };
	for (int i = 0; i < CycleItemOnePageCount; i++)
	{
		itemCheck[i] = dynamic_cast<crCheckBoxWidgetNode *>(canvas->getWidget(m_item[i]));
		if (itemCheck[i].valid() && itemCheck[i]->getSelect())
		{
			itemCheck[i]->select(false);
		}
	}
}
//////////////////////////////////////////////////////////////////////////
...
m_strRadioGroup(handle.m_strRadioGroup),
m_type(handle.m_type)
{
...
void crJXJUIQuChuLunHuiThingMethod::operator()(crHandle &handle)
{
	...
	if(myPlayer)
	{
		...
		unsigned char viplv = *(unsigned char *)param;
		int openlv = atoi(crGlobalHandle::gData()->gGameGlobalValue(WCHDATA_JXJPlayerLunHuiTiQuDengJi, viplv).c_str());
		...
		if(characterlevel >= openlv)
		{
			...
			if (radioGroup.valid())
			{
				unsigned char select = radioGroup->getSelect();
				ref_ptr<crStreamBuf> stream = new crStreamBuf;
				stream->createBuf(24);
				...
				{//选择提取的将卡
					...
					if (m_type == 1)
					{
						...
						for (std::set<char>::iterator itr = IDSet.begin();
							itr != IDSet.end();
							++itr )
						{
							stream->_writeChar(*itr);
						}
					}
					data->getParam(WCHDATA_JXJShiliID, param);
					unsigned char shiliid = *(unsigned char *)param;
					...
					if (playerStore.valid())
					{
						...
						if (playerStoreData)
						{
							int foundCount = 0;//查找是否有异国将卡
							if (!m_msgCanvas.empty())
							{
								ref_ptr<crTableIO>itemtab = crGlobalHandle::gData()->gGlobalTable(WCHDATA_JXJItemTab);
								crTableIO::StrVec record;
								...
								int itemid;
								for (JXJStoreItemBagVec::iterator itr = cardVec->begin();
									itr != cardVec->end(); i++, itr++)
								{
									if (!IDSet.empty() && IDSet.find(i) == IDSet.end())
										continue;
									if (itr->valid() && (*itr)->getItemHasCycle() == 1)
									{
										...
										if (itemtab->queryOneRecord(0, crArgumentParser::appItoa(itemid), record) >= 0)
										{
											...
											if (jkshili != 2 && jkshili != 20 && jkshili != shiliid && jkshili != 21 && jkshili != 22 && jkshili != 23/*&& jkshili != 21+shiliid-c_startShiliID*/)
											{
												foundCount++;// = true;
												//break;
											}
							...
							{//存在异国将卡，二次确认对话框
								...
								if (msgCanvas.valid())
								{
									...
									int mode = 2;
									noticeHandle->inputParam(WCHDATA_NoticeMode, &mode);
									noticeHandle->inputParam(WCHDATA_NoticeParam1, &(crArgumentParser::appItoa(foundCount)));
									int needGold = atoi(crGlobalHandle::gData()->gGameGlobalValue(WCHDATA_JXJDistillForeignCardCost, viplv).c_str()) * foundCount;
									noticeHandle->inputParam(WCHDATA_NoticeParam2, &(crArgumentParser::appItoa(needGold)));
									...
									//ref_ptr<crTableIO> textTab = crGlobalHandle::gData()->gGlobalTable(WCHDATA_JXJCodeTextTab);
									//crTableIO::StrVec record;
									//if (textTab->queryOneRecord(0, crArgumentParser::appItoa(texid), record) >= 0)
									//{
										...
										cfg_script.Pop();
										cfg_script.Pop();
										ref_ptr<crHypertextWidgetNode>  msg = dynamic_cast<crHypertextWidgetNode *>(msgCanvas->getWidget(m_msgBox));
										if (msg.valid())
											msg->setHypertext(cfg_script);
										crData *dlgData = msgCanvas->getDataClass();
										int commandtype = CDP_Widget;
				...
				{//选择提取的装备
					...
					if (m_type == 1)
					{
						...
						for (int i = 0; i < CycleItemOnePageCount; i++)
						{
							...
							if (itemCheck[i].valid() && itemCheck[i]->getSelect())
							{
								IDVec.push_back(i+offset);
							}
						}
						if (IDVec.empty())
							return;
						stream->_writeChar(IDVec.size());
						for (int i = 0; i < IDVec.size(); i++)
						{
							stream->_writeChar(IDVec[i]);
						}
					}
				}
				crNetConductor *netConductor = crNetContainer::getInstance()->getDynamicNetConductor(GameClient_Game);
...
void crJXJRecvQuChuLunHuiTingMethod::operator ()(crHandle &handle)
{
	if(m_this)
	{
		if(m_netType == GameServer)
		{
			...
			{
				int code = 0;
				m_this->doEvent(WCH_JXJGetPlayerGameStore,MAKECREPARAM(m_stream.get(),&code));
				if(code==0)
					stream->_writeChar(1);
				else//元宝不足
					stream->_writeChar(-2);
	...
	if (m_this)
	{
		...
		if (Canvas.valid())
		{
			...
			if (boxname.find("XLunhui_Card") != std::string::npos)
			{
				...
				if (index<CycleCardOnePageCount && cardoritemMap && tempcardvec)
				{
			...
			else if (boxname.find("XLunhui_Icon") != std::string::npos)
			{
				...
				if (index<CycleItemOnePageCount && cardoritemMap && cardoritembagvec)
				{
					ItemOrCardMap::iterator iter = cardoritemMap->find((char)index);
					if (iter != cardoritemMap->end() && iter->second.valid())
					{
						ref_ptr<crBagItemData> temp = iter->second;
						canvasData->excHandle(MAKECREPARAM(WCH_LockData,1));
						cardoritemMap->erase(iter);
						canvasData->excHandle(MAKECREPARAM(WCH_LockData,0));
			...
			if (name.valid())
			{
				...
				if (m_index == 0)
				{
					...
					{//
						...
						int s = t % 60;
						char buf[64];
						sprintf(buf, "%.1f折倒计时%d:%d:%d\0", (float)(curDiscount)*0.1f, hour, minute, s);
						str1 = buf;
					}
				}
				else
