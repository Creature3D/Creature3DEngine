...
void crJXJMyZhoujunManagerCanvasUpdateMethod::operator()(crHandle &handle)
{
	...
	if(playerGameData && netConductor && myCountryCanvas &&cityMapCanvas && mygroupcanvas)
	{
		...
		int myGroupCantonId1 = -1;
		int myGroupCantonId2 = -1;
		if (!myAllocatedCantonVec->empty())
		{
			myGroupCantonId1 = (*myAllocatedCantonVec)[0];
			if (myAllocatedCantonVec->size() > 1)
			{
				myGroupCantonId2 = (*myAllocatedCantonVec)[1];
			}
		}
		...
		for(int i = 0; i < 20 && zItr != zhoujunMap->end(); i++, ++ zItr)
		{
			if(cantontab->queryOneRecord(0,crArgumentParser::appItoa(zItr->first),record)>=0)
			{
				...
				if (newCantonId == myGroupCantonId1 || newCantonId == myGroupCantonId2)
				{
	...
	if(m_this && m_stream.valid())
	{
		...
		else if(m_netType == GameClient_Game)
		{
			...
			if (canvas.valid()&& canvas->getDataClass() && canvas_1.valid() && canvas_1->getDataClass() && canvas_2.valid() && canvas_2->getDataClass())
			{
				...
				myCountryData->excHandle(MAKECREPARAM(WCH_LockData,1));
				...
				myCountryData->excHandle(MAKECREPARAM(WCH_LockData,0));
			...
			if (playerData.valid())
			{
				...
				if(shiliid >= c_startShiliID)
				{
					...
					if(shili && shili->getDataClass())
					{
						...
						//shilidata->excHandle(MAKECREPARAM(WCH_BuildPlayerStream,shiliStream.get()));
	...
	if(m_this && m_stream.valid())
	{
		if(m_netType == GameServer)
		{
			...
			if (playerData.valid() && data && mainroledata)
			{
								...
								if(records.size())
								{
									if (atoi((*records.rbegin())[moneyneedindex].c_str()) > *gongxian)
									{
										...
										else
										{
											...
											int cost = coppercost;
											m_this->doEvent(WCH_JXJDeductCopperCash,MAKECREPARAM(&cost,NULL));
											//*mycopper -= coppercost;
				...
				if (success == 1)
				{
					m_this->doEvent(WCH_JXJRecvHuoyuedu, MAKECREPARAM(HYD_BuildCountry, 1));
				}
				///游戏日志
		...
		else if (m_netType == GameClient_Game)
		{
			...
			if (success != 1)
			{
				if (tipindex !=5001)
				{
					m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(tipindex,NULL));
				}
				else
				{
					crData *data = m_this->getDataClass();
...
//				cantionid,groupid,op
		...
		else if(m_netType == GameClient_Game)
		{
			...
			{
				m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(4008,NULL));
			}
			canvasdata->excHandle(MAKECREPARAM(WCH_LockData,0));
		}
	}
	...
	for (int i = 0; i < 3; ++ i)
	{
		...
		{//建国完成
			...
			if (size >= 1)
			{
				...
				crServerBrainHandle::getInstance()->doEvent(WCH_GameLog,MAKECREPARAM(id,&gamelog));
...
void JXJ::crJXJQueryPeopleInfoListMethod::operator()( crHandle &handle )
{
	...
	if (msgid)
	{
		...
		if (playerGameData)
		{
			playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(msgid,NULL));
		}
	}
}
...
void JXJ::crJXJRecvPeopleInfoListMethod::operator()( crHandle &handle )
{
	do 
	{
		...
		void *param;
		countryData->excHandle(MAKECREPARAM(WCH_LockData,1));
		...
		countryData->excHandle(MAKECREPARAM(WCH_LockData,0));
						...
						if (pmItr != peopleMap->end() && pmItr->second.valid())
						{
							if (pmItr->second->getSmallAcc())
							{
								...
								if(textTab.valid() && textTab->queryOneRecord(0,crArgumentParser::appItoa(textid),textrecord) >= 0)
								{
									...
									crServerBrainHandle::getInstance()->doEvent(WCH_JXJRecvSystemMail,MAKECREPARAM(pMailStream.get(),NULL));
								}
							}
						}
					}
									...
									do 
									{
										...
										if (moneydata.first == 0)
										{
											...
											if (idx < 3)
											{
												...
												m_this->doEvent(WCH_JXJDeductResource,MAKECREPARAM(&giftGoldData,NULL));
	...
	case 2:
		{
			...
			{
				CREPARAM& param64 = *(CREPARAM*)param;
				m_gmData = (crJXJCountryVoteGMData *)(LOCREPARAM(param64));
			}
			else
			{
				m_gmData = NULL;
...
void JXJ::crJXJCountryCantonMapRefreshMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_chengchiId = LOCREPARAM(param64);
...
void JXJ::crJXJCountryCantonMapRefreshMethod::operator()( crHandle &handle )
{
	if (m_this && m_this->getDataClass() && m_chengchiId && m_shiliId)
	{
		...
		if (idx >= 0)
		{
			...
			shiliData[idx]->excHandle(MAKECREPARAM(WCH_LockData,1));
			...
			shiliData[idx]->excHandle(MAKECREPARAM(WCH_LockData,0));
		}
		else idx = 0;
	...
	if (m_this && m_this->getDataClass())
	{
		...
		for (PlayerIDVec::iterator pvItr = playerIDVec.begin(); pvItr != playerIDVec.end(); ++ pvItr)
		{
			...
			if (playerData.valid() && playerData->getPlayerGameData())
			{
				...
				playerGameData->doEvent(WCH_JXJUpdateRankingData/*	,MAKECREPARAM(playerData.get(),NULL)*/);
			}
		}
	}
}
...
void JXJ::crJXJReduceRTMpMethod::inputParam( int i, void *param )
{
	...
	case 2:
		if(param)
		{
			...
			m_reduce = *(float *)LOCREPARAM(param64);
...
void JXJ::crJXJReduceRTMpMethod::operator()( crHandle &handle )
{
	...
	{
		//float mp = 0.0f;
		//m_this->doEvent(WCH_GetMP,MAKECREPARAM(&mp,NULL));
...
void JXJ::crJXJRecvRedeceRTMpMethod::operator()( crHandle &handle )
{
	if(m_stream.valid() && crRunGameHandle::getInstance()->getRunningCode() == crRunGameHandle::NetGame)
	{
		...
		//if(reduce>0)
		//{
		//	//m_this->doEvent(WCH_JXJClientRecvDamage, MAKECREPARAM(-reduce,NULL));
		//}
	}
}
				...
				if (consortiaData)
				{
						...
						&&(nowTime - cmmItr->second->getLoginTime()) / 86400 >= 3)
					{
							...
							&& cmmItr1 != consortiaMemberMap->end() && cmmItr->second.valid() && cmmItr1->second.valid())
						{
							...
							crServerBrainHandle::getInstance()->doEvent(WCH_GameLog,MAKECREPARAM(newID,&gamelog1));
						}
			...
			case QT_PlayerData	:
				{
					...
					{
						m_this->doEvent(WCH_JXJBuildPlayerDataSaveStreamTab	,MAKECREPARAM(playerData->getPlayerGameData(),&gmID));
						...
						ipsItr != insertPlayerSet->end(); ++ ipsItr)
					{
						...
						if ( itr != peopleMap->end())
						{
							insertSql->buildUpdateSql(itr->second->getID(),itr->second->getShili(),
								...
								itr->second->getConsortiaId(),itr->second->getConsortiaName(),itr->second->getSmallAcc(),
...
void JXJ::crJXJRecvFoundCountryGetAwardMethod::operator()( crHandle &handle )
{
	if (m_this)
	{
		if (m_netType == GameServer && m_stream.valid())
		{
			...
			//m_this->doEvent(WCH_JXJNewerTaskHadAllCompleted	,MAKECREPARAM(&newerCompleted,NULL));
						...
						if (foundCountryAwardTab.valid())
						{
							...
							if (!awardType)
							{
									...
									shiliIdx,crArgumentParser::appItoa(shiliid),record) >= 0)
								{
									...
									//	m_this->doEvent(WCH_JXJAddVipExp,MAKECREPARAM(addVipExp,NULL));
									//}
								}
							}
							else
...
void JXJ::crJXJNewerTaskHadAllCompletedMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_isCompleted = (bool *)(LOCREPARAM(param64));
		}
		else
		{
			m_isCompleted = NULL;
