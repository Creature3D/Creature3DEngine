...
{//id name
	...
	else if(netType == GameClient_Game)
	{
		...
		crBrain::getInstance()->doEvent(WCH_JXJRecvSearchBattle,MAKECREPARAM(m_streamBuf.get(),NULL));
	}
}
/////////////////////////////////////////
//
...
{//
	...
	else if(netType == GameClient_Game)
	{
		...
		crBrain::getInstance()->doEvent(WCH_JXJReturnRandomName,MAKECREPARAM(m_streamBuf.get(),NULL));
	}
}
...
void crJXJGameServerCreateRoomPacket::parsePacket(const std::string &sender)
{
	...
	{
		crServerBrainHandle::getInstance()->doEvent(WCH_GameServerCreateRoomReturn,MAKECREPARAM(m_streamBuf.get(),NULL));
	}
}
...
void crJXJJuntuanCreateRoomPacket::parsePacket(const std::string &sender)
{
	...
	{
		crServerBrainHandle::getInstance()->doEvent(WCH_JXJJuntuanCreateRoom,MAKECREPARAM(m_streamBuf.get(),NULL));
	}
}
...
void crJXJBattleRemovedPacket::parsePacket(const std::string &sender)
{
	...
	{
		crServerBrainHandle::getInstance()->doEvent(WCH_JXJRecvBattleRemoved,MAKECREPARAM(m_streamBuf.get(),NULL));
	}
}
...
void crJXJBattleSettlePacket::parsePacket(const std::string &sender)
{
	...
	if(netType == GameServer)
	{
		crServerBrainHandle::getInstance()->doEvent(WCH_JXJRecvBattleSettle,MAKECREPARAM(m_streamBuf.get(),NULL));
	}
	else if(netType == GameConnectServerClient_Scene)
	{
	...
	{//
		...
		if(room)
		{
			room->doEvent(WCH_JXJRecvBattleSettle,MAKECREPARAM(m_streamBuf.get(),NULL));
		}
	}
}
...
void crJXJCountryCapitalAwardPacket::parsePacket(const std::string &sender)
{
	...
	if(netType == GameServer)
	{
		...
		crServerBrainHandle::getInstance()->doEvent(msg,MAKECREPARAM(m_streamBuf.get(),NULL));
		//crServerBrainHandle::getInstance()->doEvent(WCH_JXJRecvBattleSettle,MAKECREPARAM(m_streamBuf.get(),NULL));
	}
}
struct BattleSettleInfo : public CRCore::Referenced
{
	int m_playerid;
	int m_killcount;
	int m_deadcount;
...
void crJXJBattleReportPacket::parsePacket(const std::string &sender)
{
	...
	if (netType == GameServer)
	{
		...
		if (serverData)
		{
			...
			battleData->createBuf(roomName.size() + 23 + size*44);
			...
			std::multimap<float, ref_ptr<BattleSettleInfo>, std::greater<float>> winermap;
			std::multimap<float, ref_ptr<BattleSettleInfo>, std::greater<float>> losermap;

			ref_ptr<BattleSettleInfo> info;
			for (int i = 0; i < size; i++)
			{
				...
				info = new BattleSettleInfo;
				info->m_playerid = m_streamBuf->_readInt();
				info->m_killcount = m_streamBuf->_readInt();
				info->m_deadcount = m_streamBuf->_readInt();
				...
				info->m_experince = m_streamBuf->_readFloat();
				info->m_exploit = m_streamBuf->_readFloat();
				info->m_playername = m_streamBuf->_readString();
				info->m_buduicount = m_streamBuf->_readInt();
				//info.push_back(crArgumentParser::appItoa(m_streamBuf->_readInt()));
			...
			int index = 1;
			int my_index = -1;
			for (std::multimap<float, ref_ptr<BattleSettleInfo>, std::greater<float>>::iterator itr = winermap.begin(); itr != winermap.end(); itr++)
			{
				battleReportInfo->getPlayerVec().push_back(itr->second->m_playerid/*atoi((itr->second)[0].c_str())*/);

				battleData->_writeInt(itr->second->m_killcount/*atoi((itr->second)[1].c_str())*/);//4 killcount
				battleData->_writeInt(itr->second->m_deadcount/*atoi((itr->second)[2].c_str())*/);//4 deadCount
				...
				if(battleRewardTable->queryOneRecord(0,crArgumentParser::appItoa(0),record_reward_base)>=0 && battleRewardTable->queryOneRecord(0,crArgumentParser::appItoa(my_index),record_reward_extra)>=0)
					battleData->_writeFloat(float(atoi(record_reward_base[exploitindex].c_str()))+float(atoi(record_reward_extra[exploitindex].c_str()))+ itr->first+itr->second->m_exploit);

				battleData->_writeFloat(itr->second->m_experince/*float(atoi((itr->second)[3].c_str()))*/);//4 experience
				battleData->_writeString(itr->second->m_playername/*(itr->second)[4]*/);//20+4
				battleData->_writeInt(itr->second->m_buduicount/*atoi((itr->second)[5].c_str())*/);//部队数

				index++;
			}

			index = 1;
			for (std::multimap<float, ref_ptr<BattleSettleInfo>, std::greater<float>>::iterator itr = losermap.begin(); itr != losermap.end(); itr++)
			{
				battleReportInfo->getPlayerVec().push_back(itr->second->m_playerid/*atoi((itr->second)[0].c_str())*/);
...
void crJXJFubenSettlePacket::parsePacket(const std::string &sender)
{
	...
	if(netType == GameServer)
	{
		crServerBrainHandle::getInstance()->doEvent(WCH_JXJRecvFubenSettle,MAKECREPARAM(m_streamBuf.get(),NULL));
	}
	//else if(netType == GameConnectServerClient_Scene)
	//{
	//	int recvid = m_streamBuf->_readInt();
	...
	//	if(room)
	//	{
			//room->doEvent(WCH_JXJRecvFubenSettle,MAKECREPARAM(m_streamBuf.get(),NULL));
	//	}
	//}
}
...
//	if(netType == GameServer)
//	{
//		crServerBrainHandle::getInstance()->doEvent(WCH_JXJRecvPassSettle,MAKECREPARAM(m_streamBuf.get(),NULL));
//	}
//	else if(netType == GameConnectServerClient_Scene)
//	{
//		int recvid = m_streamBuf->_readInt();
...
//		if(room)
//		{
//			room->doEvent(WCH_JXJRecvPassSettle,MAKECREPARAM(m_streamBuf.get(),NULL));
//		}
//	}
//}
/////////////////////////////////////////
...
void crJXJBattleTotalArmyCountPacket::parsePacket(const std::string &sender)
{
	...
	{//
		...
		if(room)
		{
			room->doEvent(WCH_JXJRecvBattleArmyCount,MAKECREPARAM(m_streamBuf.get(),NULL));
		}
	}
}
/////////////////////////////////////////
				...
				if(roomplayer)
				{
					...
					if(chengchiTab->queryOneRecord(0,crArgumentParser::appItoa(chengchiid),record)>=0)
					{
							...
							++itr )
						{
							...
							if(id>0)
								role->doEvent(WCH_ExtraHandle,MAKECREPARAM(MAKEINT32(id,-1),NULL));
							if(rbuff>0)
								role->doEvent(WCH_ExtraHandle,MAKECREPARAM(MAKEINT32(rbuff,-1),NULL));
						}
...
void crJXJCampAddArmyInfoPacket::parsePacket(const std::string &sender)
{
	...
	else if(netType == GameServer)
	{
		...
		if(playerData.valid() && playerData->getPlayerGameData())
		{
			...
			playerGameData->doEvent(WCH_JXJBattleRecruitCheck,MAKECREPARAM(&recruitTroops,&recruitCheck));
			...
			{//补兵成功
				...
				{//扣除元宝
					...
					playerGameData->doEvent(WCH_JXJDeductLijinGold,MAKECREPARAM(&moneydata,&needdeductgold));
					//元宝复活计数递增;
					(*goldRecruitCount)++;
				}
...
//			//
//			int code = 0;
//			playerData->getPlayerGameData()->doEvent(MAKEINT64(WCH_JXJVIPExtend,WCH_JXJRecvAddArmy),MAKECREPARAM(&code,NULL));
...
//				addArmyPlayerData->excHandle(MAKECREPARAM(WCH_LockData,1));
//				//此处需要获取VIP等级
//				addArmyPlayerData->getParam(WCHDATA_JXJVipLv,param);
...
//							playerData->getPlayerGameData()->doEvent(WCH_JXJRecruitCheck,MAKECREPARAM((*itr)[1],&recruitCheck));
//
...
//					itr++;
//				}
//				addArmyPlayerData->excHandle(MAKECREPARAM(WCH_LockData,0));
//
//				//发包给客户端资源扣除
...
//					playerData->getPlayerGameData()->doEvent(WCH_JXJDeductLijinGold,MAKECREPARAM(&moneydata,&needdeductgold));
//
//					////元宝扣除增加成长值
//					//addArmyPlayerData->getParam(WCHDATA_JXJVipLv,param);
//					//unsigned char vipLevel = *(unsigned char *)param;
//					//int goldToVipExpFactor = atoi(crGlobalHandle::gData()->gGameGlobalValue(WCHDATA_JXJGoldToVipExpFactor, vipLevel).c_str());
//					//playerData->getPlayerGameData()->doEvent(WCH_JXJAddVipExp,MAKECREPARAM(goldneed * goldToVipExpFactor,NULL));
//					//发包
//					stream->createBuf(2+addArmyInfo.size()*8);
//					stream->_writeChar(_case);
//					stream->_writeChar(addArmyInfo.size());
				...
				if(playerGameData && playerGameData->getDataClass())
				{
					...
					for (;itr!=formationVec->end();itr++)
					{
						...
						if (rtCount <= 0)
						{
							...
							roleData->excHandle(MAKECREPARAM(WCH_InitData, itr_role->second.get()));
							scene->itemRelive(itr_role->second.get());
							if (id > 0)
								itr_role->second->doEvent(WCH_ExtraHandle, MAKECREPARAM(MAKEINT32(id,-1), NULL));
							if (rbuff > 0)
								itr_role->second->doEvent(WCH_ExtraHandle, MAKECREPARAM(MAKEINT32(rbuff,-1), NULL));
						}
					}
					data->excHandle(MAKECREPARAM(WCH_LockData,0));
				}
			...
			if(playerGameData.valid())
			{
				...
				for (;itr_role!=myRoles.end();itr_role++)
				{
					...
					if (rtCount <= 0)
					{
						...
						float dt = 0.1f;
						itr_role->second.first->doEvent(WCH_ChangeActState, MAKECREPARAM(ACT_Idle, &dt));
					}
...
void crJXJDropedItemPacket::parsePacket(const std::string &sender)
{
	...
	if(netType == GameServer)
	{
		crServerBrainHandle::getInstance()->doEvent(WCH_JXJRecvSceneItemDrop,MAKECREPARAM(m_streamBuf.get(),NULL));
	}
	else if(netType == GameConnectServerClient_Scene)
	{
		int recvid = m_streamBuf->_readInt();
...
{//
	...
	else if(netType == GameServer)
	{
		...
		if(senderPlayerData.valid() && senderPlayerData->getPlayerGameData() && senderPlayerData->getPlayerGameData()->getDataClass())
		{
			...
			if(chatmode == ChatInWorld)
			{
				...
				playerGameData->doEvent(WCH_JXJDeductCopperCash,MAKECREPARAM(&chatprice,NULL));
				if (chatprice == 0)
				{
	...
	else if(netType == GameClient_Game)
	{
		...
		//	//m_streamBuf->seekBegin();
		//	//m_streamBuf->setBufSize(bufSize);
		//	crMyPlayerData::getInstance()->getPlayerGameData()->doEvent(WCH_JXJRecvPingMsg,MAKECREPARAM(m_streamBuf.get(),GameClient_Game));
		//}
		//else
		{
			if(crRunGameHandle::getInstance()->isInManor() || crRunGameHandle::getInstance()->isInBattle())
				crMyPlayerData::getInstance()->getPlayerGameData()->doEvent(WCH_JXJRecvGameChat,MAKECREPARAM(m_streamBuf.get(),NULL));
		}
	}	
}
			...
			if(playerData.valid() && playerData->getPlayerGameData() && playerData->getPlayerGameData()->getDataClass())
			{
				...
				if(room.valid())
				{
					...
					if (rbuff = 1)
					{
						...
						//	}
						//	(*itemptr)->doEvent(WCH_ExtraHandle,MAKECREPARAM(MAKEINT32(id,-1),NULL));
						//}
						id = JXJExt_BattleBuff_1;
						crSceneServerPlayerData::RoleMap &roleMap = playerData->getRoleMap();
						crSceneServerPlayerData::RoleMap::iterator itr_role = roleMap.begin();
...
void crJXJPingPacket::parsePacket(const std::string &sender)
{
	...
	else if(netType == GameClient_Game)
	{
		...
		{
			crMyPlayerData::getInstance()->getPlayerGameData()->doEvent(WCH_JXJRecvPingRet,MAKECREPARAM(m_streamBuf.get(),GameClient_Game));
		}
			...
			else
			{
				...
				//{
					...
					if(!loginGameQueryResultVec.empty())
					{
						...
						{
							CRCore::notify(CRCore::ALWAYS) << "无法查询到登录信息,playerid=" << playerid << std::endl;
						}
					}
					accountSession->releaseQuery();
					accountdb->endSession(accountSession.get());
					...
					else
					{
						...
						{//
							...
							{//断线重连
								...
								connectedPlayerData->getPlayerGameData()->doEvent(MAKEINT64(WCH_PlayerLogin,GameServer),MAKECREPARAM(NULL,connectedPlayerData.get()));
								//游戏登陆成功
								loginCode = 1;
								crJXJLoginGamePacket packet;
								...
								netManager->sendPacket(sender,packet);
								crServerBrainHandle::getInstance()->doEvent(MAKEINT64(WCH_PlayerLogin,GameServer),MAKECREPARAM(connectedPlayerData.get(),NULL));
							}
							else
							{
...
void JXJ::crJXJMonitorPingPacket::parsePacket( const std::string &sender )
{
	...
	else if(netType == GameClient_Game)
	{
		...
		if (playerData)
		{
			playerData->doEvent(WCH_JXJCountMonitorPacket, MAKECREPARAM(m_streamBuf.get(),netType));
		}
	}
}
