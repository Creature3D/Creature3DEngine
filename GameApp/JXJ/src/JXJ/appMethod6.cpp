						...
						if (canvasdata)
						{
							...
							if ((*itembagvec)[seletindex] != NULL && itemtab->queryOneRecord(0,crArgumentParser::appItoa((*itembagvec)[seletindex]->getItemID()),record) >= 0)
							{
								...
								else
								{
									...
									playerData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(tipindex,NULL));//失败
								}
							}
							else
							{
	...
	if(m_this && playerData)
	{
		...
		if (canvas.valid() && gongfangcanvas.valid())
		{
			...
			if (editbox.valid())
			{
				...
				{
					cancelTask = true;
					playerData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2055,NULL));//失败
				}
			}
		}
	}
...
void crJXJUISmeltingMethod::operator()(crHandle &handle)
{
	...
	if(m_this && playerGameData)
	{
		...
		if (canvas.valid())
		{
			...
			if (tipindex > 0)
			{
				playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(tipindex,NULL));
			}
			else if (tipindex == 0)
			{
				int needbagcount = 1;
			...
			{//
				...
				if (itemtab.valid() && attrid > 0 && colorid > 0 && updateindex > 0)
				{
					...
					if (success == 1)
					{
						...
						{
							m_this->doEvent(WCH_JXJDeductCopperCash,MAKECREPARAM(&price[costtype],NULL));
							if (price[costtype] == 0)
							{
								success = 1;
							}
		...
		else if(m_netType == GameClient_Game)
		{
			...
			if (success > 0)
			{
				...
				if (gongfangcanvas.valid())
				{
					...
					if (gongfangdata)
					{
						gongfangdata->excHandle(MAKECREPARAM(WCH_LockData,1));
						gongfangdata->getParam(WCHDATA_JXJSmelterItemData,param);
						crBagItemData *bagitemdata = (crBagItemData *)param;
						bagitemdata->setItemID(0);
						bagitemdata->setItemCount(0);
						bagitemdata->setEquipMagic(0);
						gongfangdata->excHandle(MAKECREPARAM(WCH_LockData,0));
					}
				}
				crData *data = m_this -> getDataClass();
				data->excHandle(MAKECREPARAM(WCH_LockData,1));
			...
			if (success == 1)
			{
				...
				if (successdonghua.valid())
				{
					...
					if (successinput.valid())
					{
						...
						m_this->doEvent(WCH_JXJCreateItemRefineTips,MAKECREPARAM(&newitem,&outscript));
...
void crJXJEquipRefineMethod::operator()(crHandle &handle)
{
	...
	if(myPlayer && netConductor)
	{
		...
		if(canvas.valid())
		{
			...
			//	{
			//		canrefine = false;
			//		myPlayer->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2058,NULL));
			//	}
			//}
		...
		if(m_netType == GameServer)
		{
			...
			{//
					...
					case RefineType_Gold:
						...
						{//高级精炼打折
							...
							if (timer > 0)
							{
								...
								float discount = *(unsigned char *)param;
								discount = price2i[0] * discount * 0.01f;
								price = discount + lockneedgold;//
							}
							else
							{
				...
				if(success>0)
				{
					...
					//增加活跃度/////////////////////////////////
					m_this->doEvent(WCH_JXJRecvHuoyuedu, MAKECREPARAM(HYD_RefineEquip, 1));
					///////////////////////////////////////////////
					///游戏日志
	...
	if(m_this && m_stream.valid())
	{
		if(m_netType == GameClient_Game)
		{
			...
			if (data)
			{
				...
				if (extra != 0)
					crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_FreeRefine, extra));
...
//			curpage = *(int *)param;
			...
			if (equipdata)
			{
				...
				{//炉子中的装备
					...
					playerData->doEvent(WCH_JXJCreateItemRefineTips,MAKECREPARAM(&item,&outscript));
					...
					if (aftermagic != 0 )
					{
						...
						playerData->doEvent(WCH_JXJCreateItemRefineTips,MAKECREPARAM(&item2,&outscript2));
			...
			if(goldinput.valid())
			{
				...
				{//高级精炼打折
					...
					if (timer > 0)
					{
						...
						float discount = *(unsigned char *)param;
						discount = price2i[0] * discount * 0.01f;
						//CRCore::notify(CRCore::ALWAYS) << "discount = " << discount <<"price2i[0]="<< price2i[0]<<" xx="<<(float)(price2i[0])*discount << std::endl;
						totalneedgold = discount + addprice;//
					}
					else
					{
		...
		if (itemid > 0 && itemcount > 0)
		{
			...
			if (needbagcount == 0)
			{
				...
				if (itemTab->queryOneRecord(0,crArgumentParser::appItoa(itemid),record) >= 0 && shoppriceindex > 0)
				{
					...
					playerGameData->doEvent(WCH_JXJCheckLijinGold,MAKECREPARAM(&needgoldtemp,NULL));
					if (needgoldtemp == 0)
					{
					//	data->getParam(WCHDATA_JXJGoldCostHintInt,param);
				...
				{
					playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2047,NULL));
				}
			}
			else
			{
...
crJXJCheckHuoDongBtnMethod::crJXJCheckHuoDongBtnMethod(const crJXJCheckHuoDongBtnMethod& handle):
	...
	m_zixuanlibaoInput(handle.m_zixuanlibaoInput),
	m_zhoukaInput(handle.m_zhoukaInput)
{
...
void crJXJCheckHuoDongBtnMethod::operator()(crHandle &handle)
{
	...
	if(myPlayer)
	{
		...
		ref_ptr<crStaticTextWidgetNode> zixuanlibaoInput = dynamic_cast<crStaticTextWidgetNode *>(m_this->getWidget(m_zixuanlibaoInput));
		ref_ptr<crStaticTextWidgetNode> zhoukaInput = dynamic_cast<crStaticTextWidgetNode *>(m_this->getWidget(m_zhoukaInput));
			...
			if (getAward == (unsigned char)-1)
			{
				...
				if (tianshu >= 0 && tianshu < IncreaseDate)
				{
					...
					else
					{
						...
						myPlayer->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&isLose,&shiliid));
						if(isLose)
						{
				...
				do 
				{
					...
					if (zixuanhuodongID > 0)
					{
						...
						else
						{
							...
							NewHuodongMap *huodongMap = (NewHuodongMap *)param;
							NewHuodongMap::iterator itr = huodongMap->find(zixuanhuodongID);
							if (itr != huodongMap->end())
							{
								ref_ptr<crJXJNewHuodong> zixuanhuodong = itr->second;
								if (zixuanhuodong->isActivation() && zixuanhuodong->isCanAdvance() && !zixuanhuodong->getComplete())
								{
									if (Conditions > 0)
									{
										linqucishu = (zixuanhuodong->getStep() - zixuanhuodong->getStepCount()) / Conditions + 1;
									}
									int shengyucishu = zixuanhuodong->getCircles();
									linqucishu = linqucishu < shengyucishu ? linqucishu : shengyucishu;
								}

							}
							roledata->excHandle(MAKECREPARAM(WCH_LockData, 0));
							if (zixuanlibaoInput.valid())
							{
								zixuanlibaoInput->setVisiable(linqucishu>0);
								zixuanlibaoInput->setString(crArgumentParser::appItoa(linqucishu));
							}
						}
					}
					else
		...
		if (zhoukaBtn.valid())
		{
			...
			{
				if (zhoukaInput.valid())
				{
					zhoukaInput->setVisiable(false);
				}
				do 
				{
								...
								else
								{
									...
									if (zhoukaInput.valid())
									{
										if (zhoukadayIdx >= 0 && zhoukadayIdx < 7)
										{
											if (((zhoukastatus >> zhoukadayIdx) & 1) == 0)
											{
												zhoukaInput->setString(std::string("1"));
												zhoukaInput->setVisiable(true);
											}
										}
									}
								}
		...
		if (junTuanHuodongBtn.valid())
		{
			...
			int startDays = 0;
			crGlobalHandle::getInstance()->doEvent(WCH_JXJGetStartedDays, MAKECREPARAM(&startDays, NULL));
...
void crJXJFormationGetLeaderPowerMethod::operator()(crHandle &handle)
{
	...
	*m_output = 0;
	...
	if (role)
	{
		...
		role->doEvent(WCH_JXJGetTongShuai,MAKECREPARAM(&totletongshuai,NULL));
...
void crJXJChangePlayerDataMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_needChange = (int *)(LOCREPARAM(param64));
			m_type = (unsigned char )(HICREPARAM(param64));
		}
		else
		{
			m_needChange = NULL;
		...
		if(m_netType == GameClient_Game)
		{
			...
			if (data)
			{
				...
				if(extra!=0 && (crRunGameHandle::getInstance()->isInManor() || crRunGameHandle::getInstance()->isInBattle()))
					crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(thingtype,extra));
			}
		}
	}
}
...
void crJXJRecvDeductItemsMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_deductItems = (DeductItemMap *)(LOCREPARAM(param64));
			m_type = (unsigned char *)(HICREPARAM(param64));
		}
		else
		{
	...
	if(m_this && m_stream.valid())
	{
		if(m_netType == GameClient_Game)
		{
			...
			if (data)
			{
				...
				if (extra != 0)
					crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_FreeRollCard, extra));
...
void crJXJTroopsTechUpdateMethod::operator()(crHandle &handle)
{
	...
	if(myPlayer && netConductor)
	{
		...
		int govpostIndex = TroopsTechLvTab->getTitleIndex("官职");
		int itemIndex = TroopsTechLvTab->getTitleIndex("消耗道具");
		crData *roleData = myPlayer->getMainRole()->getDataClass();
			...
			++itr)
		{
			...
			do 
			{
				...
				if (TroopsTechLvTab->queryOneRecord(0,crArgumentParser::appItoa(troopstechid),lvIndex,crArgumentParser::appItoa(troopstechlv),recordlv) >= 0)
				{
					...
					else if (govpost < atoi(recordlv[govpostIndex].c_str()))
					{
						//官职不足
						lvcolor = colorgray;
						success = false;
					}
					else if (!recordlv[itemIndex].empty())
					{
						crVector2i require;
						crArgumentParser::appAtoVec(recordlv[itemIndex], require);
						int hascount = 0;
						data->getParam(WCHDATA_JXJItemBagVec, param);
						JXJItemBagVec *itembagvec = (JXJItemBagVec *)param;
						for (JXJItemBagVec::iterator itr = itembagvec->begin();
				...
				myPlayer->doEvent(WCH_JXJCheckCopperCash,MAKECREPARAM(&needcopper,NULL));
				if (needcopper != 0)
				{
					//铜钱不足
					lvcolor = colorred;
...
bool crJXJTroopsTechTipsMethod::setInfo()
{
	...
	if(tipsCanvas.valid() && canvas.valid() && playerData && playerData->getDataClass())
	{
		...
		//crMultiSwitch *cardlvsw = dynamic_cast<crMultiSwitch *>(tipsCanvas->getChildNode(m_cardlvsw));
		int troopstechid = 0;
		crTableIO::StrVec record,needtechrecord,needtechlvrecord,techlvrecord,troopsrecord,nexttechlvrecord,govpostrecord;
		ref_ptr<crTableIO>TroopsTab = crGlobalHandle::gData()->gGlobalTable(WCHDATA_JXJTroopsTab);
		ref_ptr<crTableIO>TroopsTechTab = crGlobalHandle::gData()->gGlobalTable(WCHDATA_JXJTroopsTechTab);
		ref_ptr<crTableIO>TroopsTechLvTab = crGlobalHandle::gData()->gGlobalTable(WCHDATA_JXJTroopsTechLvTab);
		...
		std::string needgovpoststr = "需求官职：";
			...
			TroopsTechTab->queryOneRecord(buttonnameIndex,btnname,armytypeIndex,crArgumentParser::appItoa(troopstype),record) >= 0)
		{
			...
			if (troopstechid > 0)
			{
				...
				if (TroopsTechLvTab->queryOneRecord(0,crArgumentParser::appItoa(troopstechid),lvIndex,crArgumentParser::appItoa(techlv),techlvrecord) >= 0)
				{
					...
					needgovpost = atoi(techlvrecord[govpostIndex].c_str());
					if (needcopper > 0)
					{
					...
					if (needgovpost > 0 && govpostTab->queryOneRecord(0, techlvrecord[govpostIndex], govpostrecord) >= 0)
					{
						needgovpoststr += govpostrecord[1];
					}
					...
					if (needgovpost > 0)
					{
						...
						cfg_script.Add("DrawMode", "TEXT");
						cfg_script.Add("CharacterSize", 16);
						cfg_script.Add("Text", needgovpoststr + "\n");
						cfg_script.Pop();
					}
					//消耗道具
					if (!techlvrecord[itemIndex].empty())
					{
						crVector2i require;
						crArgumentParser::appAtoVec(techlvrecord[itemIndex], require);
						if (require != crVector2i(0, 0))
						{
							int hascount = 0;
							data->getParam(WCHDATA_JXJItemBagVec, param);
							...
							if (itemTab->queryOneRecord(0, crArgumentParser::appItoa(require[0]), itemrecord) >= 0)
							{
								v_i.clear();
								cfg_script.Add("Content");
								cfg_script.Push("Content", ++content);
								...
								cfg_script.Add("DrawMode", "TEXT");
								cfg_script.Add("CharacterSize", 16);
								std::string str = std::string("需求")+itemrecord[nameid] + "：" + crArgumentParser::appItoa(require[1]);
...
void crJXJRecvAddBlackNameMethod::operator()(crHandle &handle)
{
	if(m_this && m_stream.valid())
	{
		...
		else if(m_netType == GameClient_Game)
		{
			...
			m_this->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(tipsindex,NULL));//黑名单已满
		}
	}
}
/////////////////////////////////////////
...
void crJXJDeleteBlackNameMethod::operator()(crHandle &handle)
{
	...
	if(myPlayer && netConductor)
	{
		...
		{
			myPlayer->doEvent(WCH_JXJUIShowTipsCanvas, MAKECREPARAM(2085,NULL));//黑名单中不存在该帐号
		}
		data->excHandle(MAKECREPARAM(WCH_LockData,0));
	}
}
		...
		if (data)
		{
			...
			if (techid>0)
			{
				...
				if(techtab->queryOneRecord(0,crArgumentParser::appItoa(techid),record) >= 0)
				{
					...
					{
						playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(tipsindex,NULL));//
					}
				}
			}
		}
...
{	
	...
	if (canvas.valid() && canvas->getDataClass())
	{
		...
		if (canset)
		{
			...
			int totallead = 0;
			player->doEvent(WCH_JXJFormationGetLeaderPower,MAKECREPARAM(&totallead,NULL));
	...
	if (canvas.valid() && canvas->getDataClass())
	{
		...
		for (int index = 0; index < tempformation->size(); index++)
		{
			...
			if ((*tempformation)[index]->getCount() != 0)
			{
				...
				myPlayer->doEvent(WCH_JXJCalculatePower,MAKECREPARAM(&formation4i,&power));
			}
...
void crJXJCalculatePowerMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_troopinfo = *(crVector4i*)(LOCREPARAM(param64));//id,count,jiangka1,jiangka2
			m_output = (crVector2i*)(HICREPARAM(param64));
		}
		break;
	}
}
	...
	if(m_troopinfo[0]>0)
	{
		...
		if (troopsTab->queryOneRecord(0,crArgumentParser::appItoa(m_troopinfo[0]),record) >= 0 && scriptindex > 0)
		{
			...
			if(roleData)
			{
				...
				role->doEvent(WCH_JXJGetRoleFightingPower,MAKECREPARAM(&output,formationinfo.get()));
				(*m_output)[0] = output;
...
void crJXJRecvGMChangePlayerDataMethod::operator()(crHandle &handle)
{
	if(m_this && m_stream.valid())
	{
		if(m_netType == GameServer)
		{
			...
			m_this->doEvent(WCH_JXJGMGetPlayerData, MAKECREPARAM(gameserverplayerdata.get(),playerid));
			int changeid = 0;
			int changecount = 0;
			unsigned char goldtype = 0;
			int *curcountint = 0;
	...
	if(m_this &&playerGameData)
	{
		...
		if (viplv >= needviplv)
		{
			...
			if (needgoldtemp == 0)
			{
				...
				{
					playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2008,NULL));//背包不足
				}
			}
			else
			{
		...
		{
			playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2101,NULL));//VIP等级不足
		}
	}
}
/////////////////////////////////////////
	...
	if(m_this && m_stream.valid())
	{
		if(m_netType == GameServer)
		{
			...
			if (playerData.valid())
			{
				...
				m_this->doEvent(WCH_JXJCheckBagEmptyBag,MAKECREPARAM(&needbagnum,NULL));
				if (needbagnum != 0)
				{//背包不足
					success = -1;
				}
						...
						++itr)
					{
							...
							++vitr)
						{
							if (playerid == (*vitr)->getPlayerID())
							{
								...
								m_this->doEvent(WCH_JXJCheckLijinGold,MAKECREPARAM(&needgoldtemp,NULL));
								if (needgoldtemp != 0)
								{//元宝不足
									success = -2;
									break;
				...
				if (success == 0)
				{
					...
					*junling -= needjunling * remainrushcount * 100;
					...
					for (int i = 0; i < remainrushcount; ++i)
					{
						...
						m_this->doEvent(WCH_JXJFubenRushReward,MAKECREPARAM(&fubenid,NULL));	
					}
					data->inputParam(WCHDATA_JXJFubenOrChengchiType,NULL);
					data->inputParam(WCHDATA_JXJFubenOrChengchiType,NULL);
					data->excHandle(MAKECREPARAM(WCH_LockData, 0));
					//增加活跃度/////////////////////////////////
					m_this->doEvent(WCH_JXJRecvHuoyuedu, MAKECREPARAM(HYD_FubenRush, remainrushcount));
					///////////////////////////////////////////////
				}
				else
				{
...
void crJXJUIAutoUseBagItemMethod::operator()(crHandle &handle)
{
	...
	if(braindata && playerData)
	{
		...
		if(remaincount > 0)
		{
			...
			{
				playerData->doEvent(MAKEINT64(WCH_JXJUseBagItem,NULL), MAKECREPARAM(itemid,selectid));
			}
		}
	}
}
