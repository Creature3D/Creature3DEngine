		...
		if(netConductor && canvas.valid())
		{
			...
			if( grouplist.valid() && data)
			{
				...
				if (mygroupid > 0)
				{
					...
					else playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(4062,NULL));
				}
			}
		}
	}
			...
			if (success)
			{
				...
				if (canvas.valid())
				{
					...
					if (canvasdata)
					{
						...
						crJXJGroupInfo *groupinfo = (crJXJGroupInfo *)param;
						groupinfo->setDismissTime(dismissTime);
						m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(3033,NULL));//修改成功
...
void JXJ::crJXJRecvAddGroupMemberMethod::operator()( crHandle & handle )
{
	if(m_this && m_stream.valid())
	{
		...
		else if(m_netType == GameClient_Game)
		{
			...
			m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(msgid,NULL));
		}
	}
}
		...
		if (ifcansave)
		{
			...
			if(netConductor && canvas.valid())
			{
				...
				if( editwidget.valid())
				{
					...
					{
						int success = 1;
						playerGameData->doEvent(WCH_JXJCheckInvalidWords,MAKECREPARAM(&success,&newGroupName));
						if (success == 1)
						{
							playerGameData->doEvent(WCH_JXJCheckNameLimit,MAKECREPARAM(&success,&newGroupName));
							if (success == 1)
							{
		...
		if(m_netType == GameServer)
		{
			...
			if (data && playerData.valid() )
			{
				...
				do 
				{
					...
					int validcheck = 1;
					m_this->doEvent(WCH_JXJCheckInvalidWords,MAKECREPARAM(&success,&newGroupName));
					if (validcheck == 1)
					{
						m_this->doEvent(WCH_JXJCheckNameLimit,MAKECREPARAM(&success,&newGroupName));
						if (validcheck == 1)
						{
			...
			case JXJGroupChangeNameRetVal_suc :
				{
					...
					m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(3030,NULL));//修改成功
				}
				break;
			case JXJGroupChangeNameRetVal_failed_repreat:
				m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(3031,NULL));//修改失败
				break;
			case JXJGroupChangeNameRetVal_failed_noYuanBao:
				m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(3032,NULL));
				break;
			case JXJGroupChangeNameRetVal_failed_empty:
				m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(1099,NULL));
				break;
			case JXJGroupChangeNameRetVal_failed_size:
				m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2023,NULL));
				break;
		...
		if (ifcansave)
		{
			...
			if(netConductor && canvas.valid())
			{
				...
				if( editwidget.valid() && data)
				{
					...
					{
						playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(5018,NULL));
					}
				}
			}
		}
		...
		else if (m_netType == GameClient_Game)
		{
			...
			if (success)
			{
				...
				if (groupCanvas.valid() && groupCanvas->getDataClass())
				{
					...
					groupCanvas->doEvent(WCH_UI_UpdateData);
					m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2026,true));//修改成功
				}
			}
			else
			{
...
void JXJ::crJXJUIGroupContributeMethod::operator()( crHandle &handle )
{
	...
	if (contributeTab->queryOneRecord(0,crArgumentParser::appItoa(contributeType),record) >= 0)
	{
		...
		switch (contributeType)
		{
			...
			{
			int msgid = 3026;
			player->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(msgid,NULL));
			}
			}
			break;*/
		case JXJGroupContributeType_yuanbao1:
			...
			if (playerData.valid())
			{
				...
				if (mygroupid > 0 && contributeTab->queryOneRecord(0,crArgumentParser::appItoa(contributeType),record) >= 0)
				{
					...
					do 
					{
						...
						//			m_this->doEvent(WCH_JXJDeductCopperCash,MAKECREPARAM(&copperCost,NULL));
						...
						//			m_this->doEvent(WCH_JXJDeductResource,MAKECREPARAM(&foodData,NULL));
						//			m_this->doEvent(WCH_JXJDeductResource,MAKECREPARAM(&woodData,NULL));
						//			m_this->doEvent(WCH_JXJDeductResource,MAKECREPARAM(&ironData,NULL));
						//			m_this->doEvent(WCH_JXJDeductResource,MAKECREPARAM(&horseData,NULL));
					...
					{
						// 增加活跃度
						m_this->doEvent(WCH_JXJRecvHuoyuedu, MAKECREPARAM(HYD_GroupContribution, 1));
							...
							case JXJGroupStartFuBenType_yuanbao:
								{
									...
									//		crServerBrainHandle::getInstance()->doEvent(WCH_JXJGroupFundsUpgrade,MAKECREPARAM(&testCost,mygroupid));
									//		if (testCost == -1)
									//		{
			...
			if (mygroupcanvas.valid())
			{
				...
				if (canvasdata)
				{
					...
					case JXJGroupRemoveChengChiGroupRetVal_failed_isGuard:
						{
							m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(4064,NULL));
						}
						break;
					...
					case JXJGroupFenFengChengChiRetVal_suc:
						{
							...
							mygroupcanvas->doEvent(WCH_UI_UpdateData);
							m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2028,NULL));
						}
						break;
					default:
						break;
...
void JXJ::crJXJUIChengChiGroupUpdateMethod::operator()( crHandle &handle )
{
	...
	if (playerGameData && m_this)
	{
		...
		if (data)
		{
			...
			ref_ptr<crStaticTextWidgetNode> chengchiname[4];
			ref_ptr<crStaticTextWidgetNode> num[8];
			...
			for (unsigned char i = 0;i<8;++i)
			{
				...
				if (i < 4)
				{
					...
					if (chengchiname[i].valid())
					{
						...
						//CRCore::notify(CRCore::FATAL) << m_chengchiname[i] << std::endl;
					}
					//else
					//{
					//	CRCore::notify(CRCore::FATAL) << "错误：" <<m_chengchiname[i] << std::endl;
					//}
				}
			}
			...
			if (canvasdata)
			{
				...
				if (!groupCantonVec->empty())
				{
					...
					if (groupCantonVec->size() > 1)
					{
						cantonId = (*groupCantonVec)[1];
						if (cantontab->queryOneRecord(0, crArgumentParser::appItoa(cantonId), record) >= 0)
						{
							if (chengchiname[1].valid()) chengchiname[1]->setString(record[nameIdx]);
							isfenfenged = true;
						}
					}
				}
				...
				if (!myApplyCantonVec->empty())
				{
					for (MyApplyCantonVec::iterator macvitr = myApplyCantonVec->begin();macvitr != myApplyCantonVec->end() && i<4/*3*/;++macvitr)
					{
						int cantonId = *macvitr;
						if (cantontab->queryOneRecord(0,crArgumentParser::appItoa(cantonId),record)>=0)
						{
							if (i<4/*3*/ && chengchiname[i].valid())
							{
...
void JXJ::crJXJGameServerDayZeroMethod::operator()( crHandle &handle )
{
	if (m_this && m_this->getDataClass())
	{
		...
		int startedDays = 0;
		crServerBrainHandle::getInstance()->doEvent(WCH_JXJGetStartedDays, MAKECREPARAM(&startedDays, NULL));
		...
		if (sTm.tm_hour == 0 && sTm.tm_min == 0 && interval >= 60)
		{
			...
			crArgumentParser::appAtoVec(crGlobalHandle::gData()->gGameGlobalValue(WCHDATA_JXJCountryStrengthParam, 0), paramsVec);
			float A = paramsVec[0];//登录人数 300
			float B = paramsVec[1];//城池数量差 300
			float C = paramsVec[2];//世界等级排名 200
			float D = paramsVec[3];//本国当日占领城池数量/三国当日占领城池数量 200
			float E = paramsVec[4];//城池数量差>0时取值0.3
			float F = paramsVec[5];//比例值0.5
			float G = paramsVec[6];//比例值0.4
			float H = paramsVec[7];//比例值0.25
			...
			for( int i = 0; i < 3; ++ i)
			{
				if(alllanded <= 0 && allconquered <= 0)
					guolizhi[i] = (chavecitynew[i]-chavecityold[i]>0? E :0.0)*B + cDengji[i]*C/(limitcount>0?(float)limitcount:1.0);
				else if(alllanded <= 0)
					guolizhi[i] = ((float)(cConqueredcitynew[i]*D))/allconquered + (chavecitynew[i]-chavecityold[i]>0? E :0.0)*B + cDengji[i]*C/(limitcount>0?(float)limitcount:1.0);
				else if(allconquered <= 0)
					guolizhi[i] = ((float)(clandedplayer[i]*A))/alllanded + (chavecitynew[i]-chavecityold[i]>0? E :0.0)*B + cDengji[i]*C/(limitcount>0?(float)limitcount:1.0);
				else
					guolizhi[i] = ((float)(clandedplayer[i]*A))/alllanded + ((float)(cConqueredcitynew[i]*D))/allconquered + (chavecitynew[i]-chavecityold[i]>0? E :0.0)*B + cDengji[i]*C/(limitcount>0?(float)limitcount:1.0);
			}

			shiliid = 10;
			bool isLoseCountry = false;
			m_this->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&isLoseCountry,&shiliid));
			if(isLoseCountry)
				guolizhi[0] = 0;
				...
				if(shiliData)
				{
					...
					*countnew = 0;
					...
					{//开服前三天不统计强弱国
						...
						CRCore::notify(CRCore::ALWAYS) << tmp<<" 强弱国判定数据:"<<shili->getID()<< std::endl;
		...
		if (gsBrainData)
		{
			...
			if (!(dismissGroupMap->empty()) && gameServer && dbConductor) 
			{
				...
				for (; dgmIt != dismissGroupMap->end(); ++ dgmIt)
				{
					...
					GameLogData gamelog(Log_DimissGroup,logdata);
					crServerBrainHandle::getInstance()->doEvent(WCH_GameLog,MAKECREPARAM(dgmIt->first,&gamelog));
									...
									case T_Wood:
										{
											...
											m_this->doEvent(WCH_JXJDeductResource,MAKECREPARAM(&resourceData,NULL));
										}
										break;
									case T_Copper:
										{
									...
									case T_Exploit:
										{
											...
											if (mainRole)
											{
												mainRole->doEvent(WCH_JXJRecvExploit,MAKECREPARAM(newExploit,NULL));
											}
										}
										break;
									case T_Giftgold:
									...
									case T_Contribute:
										{
											...
											m_this->doEvent(WCH_JXJRecvCountryContribute, MAKECREPARAM(countrycontribute, NULL));
										}
										break;
									case T_GroupContribute:
										{
											int groupcontribut = (int)(gccemitr->second);
											m_this->doEvent(WCH_JXJRecvGroupContribution,MAKECREPARAM(groupcontribut,NULL));
										}
										break;
									case T_Zhanquan:
										{
											int zhanquan = (int)(gccemitr->second);
											m_this->doEvent(WCH_JXJRecvZhengZanQuan,MAKECREPARAM(zhanquan,NULL));
										}
										break;
									case T_JiangHun:
										{
											int jianghun = (int)(gccemitr->second);
											m_this->doEvent(WCH_JXJRecvCardPieces,MAKECREPARAM(jianghun,NULL));
										}
										break;
									case T_AchievePoint:
										{
											int achievepoint = (int)(gccemitr->second);
											m_this->doEvent(WCH_JXJRecvAchievePoint,MAKECREPARAM(&achievepoint,NULL));
										}
										break;
									default:
										break;
...
void JXJ::crJXJDeductResourceMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_resourceData = (ResourceChangeData  *)(LOCREPARAM(param64));
		}
		else
		{
			m_resourceData= NULL;
			...
			if (data)
			{
				...
				if (msg)
				{
					...
					if(extra!=0)
					{
						...
						crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(resourceType,extra));
					}
				}
			}
		}
				...
				if (shiliid >= c_startShiliID)
				{
					...
					if (gsBrainData)
					{
						...
						do 
						{
							...
							//if (!isSyn)
							{
								crServerBrainHandle::getInstance()->doEvent(WCH_JXJSYNChengChiToCanton);
								isSyn = true;
...
void JXJ::crJXJPutIntoGroupChatMsgQueueMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_msgPtr = (JXJGroupChatMsgBase *)LOCREPARAM(param64);
			m_groupId = HICREPARAM(param64);
		}
		else
		{
			m_msgPtr = NULL;
...
void JXJ::crJXJGroupFundsUpgradeMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_funds = (int *)LOCREPARAM(param64);
			m_groupId = HICREPARAM(param64);
		}
		else
		{
...
void JXJ::crJXJJunTuanFuBenSettleMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_fubenPlayerInfo = (FuBenPlayerInfo *)LOCREPARAM(param64);
			m_circle = *(float *)HICREPARAM(param64);
	...
	if (m_this && m_stream.valid())
	{
		if (m_netType == GameServer)
		{
			...
			do 
			{
				...
				crServerBrainHandle::getInstance()->doEvent(WCH_JXJGroupFundsUpgrade,MAKECREPARAM(&testFunds,mygroupid));
				cmmIt->second->clearJunTuanFuBenInfo();
			...
			if(exp!=0) m_this->getMainRole()->doEvent(WCH_JXJRecvExperience,MAKECREPARAM(exp,NULL));
			/*if(exploit!=0) m_this->getMainRole()->doEvent(WCH_JXJRecvExploit,MAKECREPARAM(exploit,NULL));
			if(countryContribute!=0) m_this->doEvent(WCH_JXJRecvCountryContribute,MAKECREPARAM(countryContribute,NULL));*/
			if(groupContribute != 0) m_this->doEvent(WCH_JXJRecvGroupContribution,MAKECREPARAM(groupContribute,NULL));
		...
		else if (m_netType == GameClient_Game)
		{
				...
				&& m_this->getMainRole()->getDataClass())
			{
				...
				if (/*canvas.valid() && canvas->getDataClass() && */playerData)
				{
					...
					if(extra!=0)
						crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_GroupFunds,extra));	
					//int exp = m_stream->_readInt();
					...
					//crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_Achievement,zhanGong));
					//crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_Exploit,exploit));
					//crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_Contribute,countryContribute));
			...
			case JXJGroupFuBenDoubleRet_failed_timesLimit:
				{
					...
					m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(4073,NULL));
				}
				break;
			default:
				break;
			...
			if (invalidwordsize <=  wtext.length())
			{
				...
				if(ipos != std::string::npos)
				{
					if(crGlobalHandle::isClient())
						m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(4052,NULL));
					*m_success = 0;
					break;
				}
			}
			...
			if (shili && shili->getDataClass())
			{
				...
				if (itr != peopleMap->end())
				{
						...
						&& m_this->getMainRole()->getDataClass())
					{
						...
						mainRoleData->getParam(WCHDATA_JXJSuperEquipLvupPoint, param);
						unsigned char lvpoint = *(unsigned char *)param;
						itr->second->setSuperEquipLvupPoint(lvpoint);
						...
						{
							SuperEquipExpVec expvec;
							expvec.resize(SuperEquipCount);
							for (int i = 0; i<SuperEquipCount; i++)
							{
								expvec[i] = (*superEquipDataVec)[i]->getSuperEquipExp();
							}
							itr->second->setSuperEquipExpVec(expvec);
						}
					}
		...
		if (myGroup->getLv() <= maxGroupLevel)
		{
			...
			if (fubenTab.valid() )
			{
				...
				if (fuBenCurPage + m_idx - 1 < dataVec.size())
				{
					...
					{
						playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(4070,NULL));
					}
				}
			}
		}
...
crJXJClearTargetExtraMethod::crJXJClearTargetExtraMethod(const crJXJClearTargetExtraMethod& handle):
	m_exceptBufTypeSet(handle.m_exceptBufTypeSet),
	crMethod(handle)
{
}
...
void crJXJClearTargetExtraMethod::operator()(crHandle &handle)
{
	if(m_hitParam.valid() && m_hitParam->m_fireItem.valid() && m_hitParam->m_hitItem.valid())
	{
		...
		if(param)
		{
			...
			crTableIO::StrVec record;
			int isNegativeIdx = bufferAttrTab->getTitleIndex("Buf类型");
				...
				itr != extraIDMap->end(); )
			{
				...
				if(bufferAttrTab.valid() && bufferAttrTab->queryOneRecord(0,extraValue,record)>=0)
				{
					if (m_exceptBufTypeSet.find(atoi(record[isNegativeIdx].c_str())) != m_exceptBufTypeSet.end())
					{
						if(crGlobalHandle::isClient())
						{
			...
			if (sceneServerConductor)
			{
				...
				//{
					if(m_hitParam->m_hitItem->getItemtype() == crInstanceItem::Role)
					{
						...
						ref_ptr<crStreamBuf> stream = new crStreamBuf;
						stream->createBuf(5);
						stream->_writeChar(m_exceptBufTypeSet.size());
						for (std::set<int>::iterator itr = m_exceptBufTypeSet.begin(); itr != m_exceptBufTypeSet.end(); ++itr)
						{
						...
						m_hitParam->m_hitItem->sendPacketToItemNeighbor(clearExtraPacket);
					}
				//}
			}
		}
				...
				if (sceneServerConductor)
				{
					...
					//{
						if (item->getItemtype() == crInstanceItem::Role)
						{
							...
							//item->sendPacketToItemNeighbor(clearCDPacket);
						}
					//}
				}
			}
...
//			//		hmItr->second->doEvent(WCH_UPDATE,MAKECREPARAM(myPlayer,NULL));
//			//	}
//			//}
					...
					if(itr->second->isCanAdvance() && !itr->second->getComplete())
					{
						if (huodongTab->queryOneRecord(0,crArgumentParser::appItoa(huodongID),record) >= 0)
						{
							...
							//{
								...
								//	m_this->doEvent(WCH_JXJGetAwardsByActiveCode,MAKECREPARAM(&activeCodeData,&logRet));
...
void JXJ::crJXJRecvClearTargetExtraMethod::operator()( crHandle &handle )
{
	...
	if(param && m_stream.get())
	{
		...
		std::set<int>exceptBufTypeSet;
		for (int i = 0; i<count; i++)
		{
			exceptBufTypeSet.insert(m_stream->_readChar());
		}
		crData *extraData = (crData *)param;
		crHandle *handle;
		extraData->excHandle(MAKECREPARAM(WCH_LockData,1));
			...
			itr != extraIDMap->end(); )
		{
			...
			{
				if (exceptBufTypeSet.find(atoi(record[isNegativeIdx].c_str())) != exceptBufTypeSet.end())
				{
					if(crGlobalHandle::isClient())
					{
						handle = extraData->getHandle(MAKEINT64(WCH_DoExtra,itr->first));
...
//				{
//					itr->second->doEvent(WCH_JXJHongdongSynSetp,MAKECREPARAM(m_this,NULL));
//				}
//			}
//			roleData->excHandle(MAKECREPARAM(WCH_LockData,0));
//		}
...
void crJXJCheckHuodongSingleValueChangeMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_playerGameData = (CRNetApp::crPlayerGameData *)(LOCREPARAM(param64));
		}
		else
		{
			m_playerGameData = NULL;
...
void crJXJShiliOccupyCityCountMethod::operator()( crHandle &handle )
{
	...
	do 
	{
		...
		//serverData->excHandle(MAKECREPARAM(WCH_LockData,1));//mainrole 和 serverdata互锁，ChengchiMap不会变动，可以不用锁
				...
				if (quanxian>0 && quanxian<JXJGMPermission_newer)
				{
					...
					if (otherID)
					{
						...
						case JXJGMOperation_delNoChat:	
							...
							//			crServerBrainHandle::getInstance()->doEvent(WCH_GameLog,MAKECREPARAM(otherID,&gamelog));
							//		}
							//	}
					...
					else
					{
						if (op == JXJGMOperation_kickAllPlayerOff)
						{
							...
							if (JXJGMOperationRet_suc == retVal)
							{
								...
								GameLogData gamelog(Log_GMKickOffPlayer,logdata);
								crServerBrainHandle::getInstance()->doEvent(WCH_GameLog,MAKECREPARAM(playerData->getPlayerID(),&gamelog));
							}
						}
						else
...
void JXJ::crJXJGetAwardsByActiveCodeMethod::inputParam( int i, void *param )
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_activeCodeData = (ActiveCodeData *)(LOCREPARAM(param64));
			m_retVal = (char *)(HICREPARAM(param64));
		}
		else
		{
...
//						activeCodeData.m_activeCode = activeCode;
//						activeCodeData.m_huodongID = huodongID;
//						m_this->doEvent(WCH_JXJGetAwardsByActiveCode	,MAKECREPARAM(&activeCodeData,&retVal));
...
void JXJ::crJXJExtraCardAddRTMPMethod::operator()( crHandle &handle )
{
	...
	if(m_timer>=0.0f)
	{
		...
		if(m_timer<0.0f)
		{
			...
			m_this->doEvent(MAKEINT64(WCH_JXJCureRTMp,0),MAKECREPARAM(&m_extraValue,NULL));
			thisData->excHandle(MAKECREPARAM(WCH_LockData,0));
		}
	}
}
