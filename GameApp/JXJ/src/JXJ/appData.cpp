...
m_onlinetimer(0),
m_blackShopRefreshTimer(0),
m_autoBuilding(false),
m_buildCountryTimes(-1),
m_superEquipCoin(0),
m_refineDiscountTimer(0),
m_refineDiscount(0)
{
...
m_blackShopRefreshTimer(data.m_blackShopRefreshTimer),
...
m_blackgoldshop(data.m_blackgoldshop),
m_superEquipCoin(data.m_superEquipCoin),
m_refineDiscountTimer(data.m_refineDiscountTimer),
m_refineDiscount(data.m_refineDiscount)
{
...
void crJXJPlayerData::addParam(int i, const std::string& str)
{
	...
	case WCHDATA_JXJOnlineTimer:
		...
		break;
	case WCHDATA_JXJBlackShopRefreshTimer:
		m_blackShopRefreshTimer = atoi(relStr.c_str());
		break;
	...
	case WCHDATA_JXJForeignCard:
		...
		break;
	case WCHDATA_JXJBlackGoldShop:
		crArgumentParser::appAtoVec(relStr, m_blackgoldshop);
		break;
	case WCHDATA_JXJSuperEquipCoin:
		m_superEquipCoin = atoi(relStr.c_str());
		break;
	case WCHDATA_JXJRefineDiscountTimer:
		m_refineDiscountTimer = atoi(relStr.c_str());
		break;
	case WCHDATA_JXJRefineDiscount:
		m_refineDiscount = (unsigned char)(atoi(relStr.c_str()));
		break;
	default:
		break;
	}
}
void crJXJPlayerData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_BuildSaveStream)
	{
		...
		cfg_script.Add("Param", buf);
		sprintf(buf, "%d,%s\0", WCHDATA_JXJBlackGoldShop, crArgumentParser::appVectoa(m_blackgoldshop).c_str());
		cfg_script.Add("Param", buf);
		sprintf(buf, "%d,%d\0", WCHDATA_JXJBlackShopRefreshTimer, m_blackShopRefreshTimer);
		cfg_script.Add("Param", buf);
		sprintf(buf, "%d,%d\0", WCHDATA_JXJSuperEquipCoin, m_superEquipCoin);
		cfg_script.Add("Param", buf);
		sprintf(buf, "%d,%d\0", WCHDATA_JXJRefineDiscountTimer, m_refineDiscountTimer);
		cfg_script.Add("Param", buf);
		sprintf(buf, "%d,%d\0", WCHDATA_JXJRefineDiscount, (int)m_refineDiscount);
		cfg_script.Add("Param", buf);
...
// 			}
		...
		int blackgoldcount = m_blackgoldshop.size();
		stream->_writeChar(blackgoldcount);
		for (int i = 0; i < blackgoldcount; i++)
		{
			stream->_writeShort(m_blackgoldshop[i]);
		}
		stream->_writeInt(m_superEquipCoin);
		stream->_writeInt(m_refineDiscountTimer);
		stream->_writeUChar(m_refineDiscount);
		stream->seekBegin();
		CRNet::creCompress(stream);
	}
	else if(LOCREPARAM(msg) == WCH_BuildNetStream)
	{
		GNE::LockMutex lock( m_dataMutex );
	...
	else if(LOCREPARAM(msg) == WCH_BuildCreateStream)
	{
		GNE::LockMutex lock( m_dataMutex );
		//m_dataStream = new crStreamBuf;
		crStreamBuf *stream = (crStreamBuf *)(HICREPARAM(msg));
	...
	else if(LOCREPARAM(msg) == WCH_BuildUpdateStream)
	{
		GNE::LockMutex lock( m_dataMutex );
		//m_dataStream = new crStreamBuf;
		crStreamBuf *stream = (crStreamBuf *)(HICREPARAM(msg));
	...
	else if(LOCREPARAM(msg) == WCH_BuildSceneSaveStream)
	{
		GNE::LockMutex lock( m_dataMutex );
		//m_dataStream = new crStreamBuf;
		crStreamBuf *stream = (crStreamBuf *)(HICREPARAM(msg));
...
void crJXJPlayerData::inputParam(int i, void *param)
{
	...
	case WCHDATA_PlayerStream:
		if(param)
		{
			...
			size = dataStream->_readChar();
			m_blackgoldshop.resize(size);
			for (i = 0; i < size; i++)
			{
				m_blackgoldshop[i] = dataStream->_readShort();
			}
			m_superEquipCoin = dataStream->_readInt();
			m_refineDiscountTimer = dataStream->_readInt();
			m_refineDiscount = dataStream->_readUChar();
			//m_dataStream = NULL;
		}
		break;
	...
	case WCHDATA_JXJCountryBattleTimes:
		...
		break;
	case WCHDATA_JXJAcceptZhengZhaoEnterBroadcast:
		m_releaseName = param ? *((std::string *)param) : "";
		break;
	...
	case WCHDATA_JXJOnlineTimer:
		...
		break;
	case WCHDATA_JXJBlackShopRefreshTimer:
		m_blackShopRefreshTimer = param ? *((int *)param) : 0;
		break;
	case WCHDATA_JXJAutoBuilding:
	...
	case WCHDATA_JXJForeignCard:
		...
		break;
	case WCHDATA_JXJSuperEquipCoin:
		m_superEquipCoin = param ? *((int *)param) : 0;
		break;
	case WCHDATA_JXJRefineDiscountTimer:
		m_refineDiscountTimer = param ? *((int *)param) : 0;
		break;
	case WCHDATA_JXJRefineDiscount:
		m_refineDiscount = param ? *((unsigned char *)param) : 0;
		break;
	default:
		break;
	}
...
void crJXJPlayerData::getParam(int i, void*& param)
{
 ...
 //		param = &m_playerBuyIconVec;
	...
	case WCHDATA_JXJForeignCard:
		...
		break;
	case WCHDATA_JXJBlackGoldShop:
		param = &m_blackgoldshop;
		break;
	case WCHDATA_JXJSuperEquipCoin:
		param = &m_superEquipCoin;
		break;
	case WCHDATA_JXJRefineDiscountTimer:
		param = &m_refineDiscountTimer;
		break;
	case WCHDATA_JXJRefineDiscount:
		param = &m_refineDiscount;
		break;
	default:
		param = NULL;
		break;
...
void crJXJBrainData::excHandle(CREPARAM msg)
{
...
void crJXJBrainData::inputParam(int i, void *param)
{
	...
	case WCHDATA_JXJAddFriendName:
		m_addfriendname = param?*(std::string*)param:"";
		break;
	...
	case WCHDATA_JXJChatOneToOneReciverName:
		m_chatReciverName = param?*(std::string*)param:"";
		break;
	...
	case WCHDATA_JXJNewGuideCanvas:
		m_newguidecanvas = param?*(std::string*)param:"";
		break;
	case WCHDATA_JXJNewGuideBtn:
		m_newguidebtn = param?*(std::string*)param:"";
		break;
...
//
/////////////////////////////////////////
unsigned short SuperEquipData::levelup(unsigned char lvupPoint)
{
	crTableIO::StrVec record;
	...
	if (m_superEquipLevel < SuperEquipMaxLevel)
	{
		lv = m_superEquipLevel + 1;
		for (; lv <= SuperEquipMaxLevel; lv++)
		{
			if (superEquipTab->queryOneRecord(0,crArgumentParser::appItoa(m_superEquipID), lvindex, crArgumentParser::appItoa(lv), record) >= 0)
			{
				expneed = atoi(record[expindex].c_str());
				if (expneed > m_superEquipExp)
				{
					break;
				}
				else
		...
		lv -= 1;
		if (lv > m_superEquipLevel)
		{
			m_superEquipLevel = lv;
		}
	}
	return gainpoint;
}
...
m_superEquipLvupPoint(0),
m_expPelletBuyTimes(0)
//m_mygroupapplyid(0)
//m_soldierCount(0),
//m_combatpower(0)
{
...
m_superEquipDataVec(data.m_superEquipDataVec),
m_superEquipLvupPoint(data.m_superEquipLvupPoint),
m_expPelletBuyTimes(data.m_expPelletBuyTimes),
m_superEquipZhulingData(data.m_superEquipZhulingData)
...
void crJXJMainRoleData::addParam(int i, const std::string& str)
{
	...
	case WCHDATA_JXJQiyuCompleteCount:
		...
		break;
	case WCHDATA_JXJSuperEquipLvupPoint:
		m_superEquipLvupPoint = (unsigned char)(atoi(relStr.c_str()));
		break;
	case WCHDATA_JXJExpPelletBuyTimes:
		m_expPelletBuyTimes = (unsigned char)(atoi(relStr.c_str()));
		break;
...
void crJXJMainRoleData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_BuildSaveStream)
	{
		GNE::LockMutex lock( m_dataMutex );
		...
		cfg_script.Add("Param",buf);
		sprintf(buf, "%d,%d\0", WCHDATA_JXJSuperEquipLvupPoint, m_superEquipLvupPoint);
		cfg_script.Add("Param", buf);
		sprintf(buf, "%d,%d\0", WCHDATA_JXJExpPelletBuyTimes, m_expPelletBuyTimes);
		cfg_script.Add("Param", buf);
		...
		////////神器数据
		//SuperEquipDataVec
		if (!m_superEquipDataVec.empty())
		{
			cfg_script.Add("SuperEquipDataVec");
			cfg_script.Push("SuperEquipDataVec");
			v_str.resize(3);
			for (SuperEquipDataVec::iterator itr = m_superEquipDataVec.begin();
				itr != m_superEquipDataVec.end();
				++itr)
			{
				v_str[0] = crArgumentParser::appItoa((*itr)->m_superEquipID);
				v_str[1] = crArgumentParser::appItoa((*itr)->m_superEquipExp);
				v_str[2] = crArgumentParser::appItoa((*itr)->m_superEquipLevel);
				cfg_script.Add("Param", v_str);
			}
			cfg_script.Pop();
		}
		if (m_superEquipZhulingData.valid())
		{
			cfg_script.Add("SuperEquipZhuling");
			cfg_script.Push("SuperEquipZhuling");
			cfg_script.Add("ZhulingPoint", m_superEquipZhulingData->m_zhulingPoint);
			SuperEquipZhulinglvVec &zhulinglvVec = m_superEquipZhulingData->m_zhulinglvVec;
			for (SuperEquipZhulinglvVec::iterator pitr = zhulinglvVec.begin();
				pitr != zhulinglvVec.end();
				++pitr)
			{
				cfg_script.Add("Param", (int)(*pitr));
			}
			cfg_script.Pop();
		}
		{
	...
	else if(LOCREPARAM(msg) == WCH_BuildPlayerStream)
	{
		...
		////////神器数据
		stream->_writeUChar(m_superEquipLvupPoint);
		stream->_writeUChar(m_superEquipDataVec.size());
		for (SuperEquipDataVec::iterator itr = m_superEquipDataVec.begin();
			itr != m_superEquipDataVec.end();
			++itr)
		{
			stream->_writeUChar((*itr)->m_superEquipID);
			stream->_writeInt((*itr)->m_superEquipExp);
			stream->_writeUChar((*itr)->m_superEquipLevel);
		}
		stream->_writeUChar(m_expPelletBuyTimes);
		stream->_writeBool(m_superEquipZhulingData.valid());
		if (m_superEquipZhulingData.valid())
		{
			stream->_writeUShort(m_superEquipZhulingData->m_zhulingPoint);
			SuperEquipZhulinglvVec &zhulinglvVec = m_superEquipZhulingData->m_zhulinglvVec;
			stream->_writeUChar(zhulinglvVec.size());
			for (SuperEquipZhulinglvVec::iterator pitr = zhulinglvVec.begin();
				pitr != zhulinglvVec.end();
				++pitr)
			{
				stream->_writeUChar(*pitr);
			}
		}
		///end
		stream->seekBegin();
	...
	else if(LOCREPARAM(msg) == WCH_GameServerCheckData)
	{
		//GNE::LockMutex lock( m_dataMutex );
		crInstanceItem *item = (crInstanceItem *)(HICREPARAM(msg));
...
void crJXJMainRoleData::inputParam(int i, void *param)
{
	...
	case WCHDATA_DataStream:
		if(param)
		{
			...
			////////神器数据
			//SuperEquipDataVec
			if (cfg_script.Push("SuperEquipDataVec"))
			{
				nParam = 1;
				v_str.resize(3);
				ref_ptr<SuperEquipData> superEquipData;
				m_superEquipDataVec.clear();
				while (cfg_script.Get("Param", v_str, nParam++))
				{
					superEquipData = new SuperEquipData(atoi(v_str[0].c_str()));
					superEquipData->m_superEquipExp = atoi(v_str[1].c_str());
					superEquipData->m_superEquipLevel = atoi(v_str[2].c_str());
					m_superEquipDataVec.push_back(superEquipData);
				}
				cfg_script.Pop();
			}
			if (cfg_script.Push("SuperEquipZhuling"))
			{
				m_superEquipZhulingData = new SuperEquipZhulingData;
				if (cfg_script.Get("ZhulingPoint", int1))
				{
					m_superEquipZhulingData->m_zhulingPoint = int1;
				}
				nParam = 1;
				m_superEquipZhulingData->m_zhulinglvVec.clear();
				while (cfg_script.Get("Param", int1, nParam++))
				{
					m_superEquipZhulingData->m_zhulinglvVec.push_back(int1);
				}
				cfg_script.Pop();
			}
			//
	...
	case WCHDATA_PlayerStream:
		if(param)
		{
			...
			////////神器数据
			m_superEquipLvupPoint = dataStream->_readUChar();
			size = dataStream->_readUChar();
			ref_ptr<SuperEquipData> superEquipData;
			m_superEquipDataVec.resize(size);
			for (int i = 0; i < size; i++)
			{
				superEquipData = new SuperEquipData(dataStream->_readUChar());
				superEquipData->m_superEquipExp = dataStream->_readInt();
				superEquipData->m_superEquipLevel = dataStream->_readUChar();
				m_superEquipDataVec[i] = superEquipData;
			}
			m_expPelletBuyTimes = dataStream->_readUChar();
			if (dataStream->_readBool())
			{
				m_superEquipZhulingData = new SuperEquipZhulingData;
				m_superEquipZhulingData->m_zhulingPoint = dataStream->_readUShort();
				SuperEquipZhulinglvVec &zhulinglvVec = m_superEquipZhulingData->m_zhulinglvVec;
				int psize = dataStream->_readUChar();
				zhulinglvVec.resize(psize);
				for (int j = 0; j < psize; j++)
				{
					zhulinglvVec[j] = dataStream->_readUChar();
				}
			}
			//m_dataStream = NULL;
		}
	...
	case WCHDATA_JXJQiyuTaskID:
		...
		break;
	case WCHDATA_JXJQiyuCompleteCount:
		m_qiyuCompleteCount = param==NULL?0L:*((unsigned char*)param);
		break;
	case WCHDATA_JXJSuperEquipLvupPoint:
		m_superEquipLvupPoint = param == NULL ? 0L : *((unsigned char*)param);
		break;
	case WCHDATA_JXJExpPelletBuyTimes:
		m_expPelletBuyTimes = param == NULL ? 0L : *((unsigned char*)param);
		break;
	case WCHDATA_JXJSuperEquipZhulingData:
		m_superEquipZhulingData = (SuperEquipZhulingData*)param;
		break;
	default:
...
void crJXJMainRoleData::getParam(int i, void*& param)
{
	...
	case WCHDATA_JXJHuoyueduRewardSet:
		...
		break;
	case WCHDATA_JXJSuperEquipDataVec:
		param = &m_superEquipDataVec;
		break;
	case WCHDATA_JXJSuperEquipZhulingData:
		param = m_superEquipZhulingData.get();
		break;
	case WCHDATA_JXJAppointColumnsVec:
	...
	case WCHDATA_JXJQiyuCompleteCount:
		...
		break;
	case WCHDATA_JXJSuperEquipLvupPoint:
		param = &m_superEquipLvupPoint;
		break;
	case WCHDATA_JXJExpPelletBuyTimes:
		param = &m_expPelletBuyTimes;
		break;
	default:
...
void crJXJMetierData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_BuildSaveStream)
	{
		GNE::LockMutex lock( m_dataMutex );
	...
	case WCHDATA_GameTaskTab:
		{
			...
			if(tasktab.valid())
			{
				...
				for(int i = 0; i<rowcount; ++i)
				{
					...
					if (gameTask->getTaskStepType() == TS_Zero)
					{
						gameTask->setTaskSteps(atoi((*tasktab)(i, col++).c_str()));
					}
					else
					{
...
void crJXJRoleData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_BuildSaveStream)
	{
		//ItemMap itemMap;
	...
	else if(LOCREPARAM(msg) == WCH_BuildPlayerStream)
	{
		...
		stream->_writeShort(m_inbornAttackDamageIncrease);
		stream->_writeShort(m_inbornSkillDamageIncrease);
		...
		stream->_writeShort(m_inbornCureEffectPercent);
	...
	else if(LOCREPARAM(msg) == WCH_InitData)
	{
		crInstanceItem *item = (crInstanceItem *)(HICREPARAM(msg));
		float hp = 0.0f;
		item->doEvent(WCH_GetHP,MAKECREPARAM(&hp,NULL));
		{
			...
			//float mp = 0.0f;
			//item->doEvent(WCH_GetMP,MAKECREPARAM(&mp,NULL));
	...
	else if(LOCREPARAM(msg) == WCH_ResetExtra)
	{
		...
		m_extraSkillDamage = 0;
		m_extraAttackDamageIncrease_rd = m_extraAttackDamageIncrease;
		m_extraAttackDamageIncrease = 0;
		m_extraSkillDamageIncrease_rd = m_extraSkillDamageIncrease;
		m_extraSkillDamageIncrease = 0;
		...
		m_extraCureEffectPercent_rd = m_extraCureEffectPercent;
		m_extraCureEffectPercent = 0;
		m_gainDamageType = -1;
	}
	else
...
void crJXJRoleData::inputParam(int i, void *param)
{
	...
	case WCHDATA_PlayerStream:
		if(param)
		{
			...
			m_inbornAttackDamageIncrease = dataStream->_readShort();
			m_inbornSkillDamageIncrease = dataStream->_readShort();
			//m_dataStream = NULL;
			...
			m_inbornCureEffectPercent = dataStream->_readShort();
		}
		break;
	case WCHDATA_NetStream:
		if(param)
	...
	case WCHDATA_JXJNpcItemType:
		...
		break;
	case WCHDATA_JXJGainDamageType:
		m_gainDamageType = param ? *(char*)param : -1;
		break;
	case WCHDATA_LastUseTime:
		m_lastUseItemTime = param ? *(_crInt64 *)param : 0L;
		break;
	default:
		crRoleData::inputParam(i,param);
	}
...
void crJXJRoleData::getParam(int i, void*& param)
{
	...
	case WCHDATA_JXJDamageExtraGainMp_RD:
		...
		break;
	case WCHDATA_JXJGainDamageType:
		param = &m_gainDamageType;
		break;
	case WCHDATA_LastUseTime:
		param = &m_lastUseItemTime;
		break;
	default:
		crRoleData::getParam(i,param);
		break;
...
crJXJItemData::crJXJItemData(const crJXJItemData& data):
	...
	m_npcItemType(data.m_npcItemType),
	m_touchPlayerMap(data.m_touchPlayerMap),
	m_touchTime(data.m_touchTime)
{
}
void crJXJItemData::addParam(int i, const std::string& str)
{
	...
	case WCHDATA_JXJNpcItemType:
		...
		break;
	case WCHDATA_JXJTouchTime:
		m_touchTime = atof(relStr.c_str());
		break;
	}
}
void crJXJItemData::inputParam(int i, void *param)
{
	...
	case WCHDATA_JXJNpcItemType:
		...
		break;
	case WCHDATA_JXJTouchTime:
		m_touchTime = param == NULL ? 0.0f : *(float *)param;
		break;
	}
}
void crJXJItemData::getParam(int i, void*& param)
{
	...
	case WCHDATA_JXJNpcItemType:
		param = &m_npcItemType;
		break;
	case WCHDATA_JXJTouchPlayerMap:
		param = &m_touchPlayerMap;
		break;
	case WCHDATA_JXJTouchTime:
		param = &m_touchTime;
		break;
...
void crJXJItemData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_LockData)
	{
		if(HICREPARAM(msg))
...
void crJXJSkillData::addParam(int i, const std::string& str)
{
	...
	case WCHDATA_DamageCount:
		m_damageCount = (char)(atoi(relStr.c_str()));
		break;
...
void crJXJSkillData::excHandle(CREPARAM msg)
{
	crData::excHandle(getHandle(LOCREPARAM(msg))); 
}
void crJXJSkillData::inputParam(int i, void *param)
{
	...
	case WCHDATA_EquipExtraID:
		...
		break;
	case WCHDATA_DamageCount:
		m_damageCount = param?*(char *)param:0;
		break;
	case WCHDATA_JXJTargetOverlapMaxCount:
...
void crJXJGameServerData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_LockData)
	{
		if(HICREPARAM(msg))
	...
	else if(LOCREPARAM(msg) == WCH_BuildPlayerLoginStream)
	{
...
void crJXJSceneServerData::excHandle(CREPARAM msg)
{
...
void crJXJQueryRankingUIData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_LockData)
	{
		if(HICREPARAM(msg))
	...
	{
		crUIData::excHandle(getHandle(LOCREPARAM(msg))); 
	}
}
/////////////////////////////////////////
//
...
void crJXJBattleCameraData::getParam(int i, void*& param)
{
	...
	case WCHDATA_JXJDelayCanvasCloseTime:
		param = &m_delayTimer;
		break;
	case WCHDATA_JXJQueryItemRTHPMap:
		param = &m_queryItemRTHPMap;
		break;
	default:
...
void crJXJNewHandUIData::inputParam(int i, void *param)
{
	...
	case WCHDATA_JXJTargetCanvasName:
		m_targetCanvasName = param?*(std::string*)param:"";
		break;
	case WCHDATA_JXJTargetWidgetName:
		m_targetWidgetName = param?*(std::string*)param:"";
		break;
	}
}
...
void crJXJConsortiaData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_BuildSaveStream)
	{
		GNE::LockMutex lock( m_dataMutex );
	...
	else if(LOCREPARAM(msg) == WCH_BuildPlayerStream||LOCREPARAM(msg) == WCH_BuildNetStream)
	{
		ref_ptr<crStreamBuf> stream = (crStreamBuf *)(HICREPARAM(msg));
		//m_dataStream = new crStreamBuf;
...
void crJXJShiliData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_BuildSaveStream)
	{
		void *param;
...
crJXJPlayerStoreData::crJXJPlayerStoreData(const crJXJPlayerStoreData& data):
	...
	m_vipexp(data.m_vipexp),
	m_superEquipCycleExpVec(data.m_superEquipCycleExpVec),
	m_superEquipCycleLvupPoint(data.m_superEquipCycleLvupPoint)
{
}
void crJXJPlayerStoreData::addParam(int i, const std::string& str)
{
	...
	case WCHDATA_JXJPlayerTotalAddMoney:
		...
		break;
	case WCHDATA_JXJPlayerCycleCount:
		m_cyclecount = (unsigned short)(atoi(relStr.c_str()));
		break;
	case WCHDATA_JXJVipExp:
		m_vipexp = atoi(relStr.c_str());
		break;
 	case WCHDATA_JXJSuperEquipCycleLvupPoint:
		m_superEquipCycleLvupPoint = atoi(relStr.c_str());
 		break;
	}
}
void crJXJPlayerStoreData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_BuildSaveStream)
	{
		GNE::LockMutex lock( m_dataMutex );
		...
		sprintf(buf, "%d,%d\0", WCHDATA_JXJVipExp, m_vipexp);
		cfg_script.Add("Param", buf);
		sprintf(buf, "%d,%d\0", WCHDATA_JXJSuperEquipCycleLvupPoint, m_superEquipCycleLvupPoint);
		cfg_script.Add("Param", buf);
		/////////背包列表
		if(!m_itembagvec.empty())
		{
		...
		//////神器轮回经验
		if (!m_superEquipCycleExpVec.empty())
		{
			cfg_script.Add("SuperEquipCycleExpVec");
			cfg_script.Push("SuperEquipCycleExpVec");
			for (SuperEquipCycleExpVec::iterator itr = m_superEquipCycleExpVec.begin();
				itr != m_superEquipCycleExpVec.end();
				++itr)
			{
				cfg_script.Add("Param", *itr);
			}
	...
	else if(LOCREPARAM(msg) == WCH_BuildPlayerStream)
	{
		...
		stream->_writeUChar(m_superEquipCycleLvupPoint);
		stream->_writeUChar(m_superEquipCycleExpVec.size());
		for (SuperEquipCycleExpVec::iterator itr = m_superEquipCycleExpVec.begin();
			 itr != m_superEquipCycleExpVec.end();
			++itr)
		{
...
void crJXJPlayerStoreData::inputParam(int i, void *param)
{
	...
	case WCHDATA_JXJPlayerTotalAddMoney:
		...
		break;
	case WCHDATA_JXJPlayerCycleCount:
		m_cyclecount = param ? *((unsigned short*)param) : NULL;
		break;
	case WCHDATA_JXJVipExp:
		m_vipexp = param ? *((int*)param) : NULL;
		break;
	case WCHDATA_JXJSuperEquipCycleLvupPoint:
		m_superEquipCycleLvupPoint = param ? *((unsigned char*)param) : NULL;
		break;
	case WCHDATA_DataStream:
		if(param)
		{
...
// 			}
			...
			if(cfg_script.Push("JXJJiangkaItemBagVec"))
			{
				...
				while(cfg_script.Get("Param", v_str,nJkBagVec++))
				{
					...
					m_jiangkabagvec.push_back(NULL);
				}
				cfg_script.Pop();
			}
			/////神器轮回经验
			m_superEquipCycleExpVec.clear();
			if (cfg_script.Push("SuperEquipCycleExpVec"))
			{
				nParam = 1;
				while (cfg_script.Get("Param", int1, nParam++))
				{
					m_superEquipCycleExpVec.push_back(int1);
				}
				cfg_script.Pop();
			}
			cfg_script.Close();
		...
		if(param)
		{
			...
			for(i = 0; i<itembagnum; i++)
			{
				...
				if(bagitemcount>0)
				{
					...
					itemdata->setItemCount(bagitemcount);
					itemdata->setItemID(dataStream->_readInt());
					itemdata->setEquipMagic(dataStream->_readInt());//装备附魔
			...
			m_superEquipCycleLvupPoint = dataStream->_readUChar();
			unsigned char superEquipExpCount = dataStream->_readUChar();
			m_superEquipCycleExpVec.resize(superEquipExpCount);
			for (i = 0; i < superEquipExpCount; i++)
			{
				m_superEquipCycleExpVec[i] = dataStream->_readInt();
			}
		}
		break;
	}
...
void crJXJPlayerStoreData::getParam(int i, void*& param)
{
	...
	case WCHDATA_JXJVipExp:
		...
		break;
	case WCHDATA_JXJSuperEquipCycleLvupPoint:
		param = &m_superEquipCycleLvupPoint;
		break;
	case WCHDATA_JXJSuperEquipCycleExpVec:
		param = &m_superEquipCycleExpVec;
		break;
	case WCHDATA_JXJItemBagVec:
		param = &m_itembagvec;
		break;
			...
			if (it->second.valid())
			{
				...
				if (playerData.valid() && playerData->getPlayerGameData())
				{
					...
					if (myPlayer->getDataClass())
					{
						...
						groupid = *( int *)param;
						myPlayer->doEvent(WCH_JXJRefreshGroupCantonSpecialty,MAKECREPARAM(&groupid,&playerid));
						myPlayer->doEvent(WCH_JXJRecvUpdateCantonSpecialty,MAKECREPARAM(NULL,GameServer));
					}
				}
...
void crJXJBattleRankingUIData::excHandle(CREPARAM msg)
{
	if(LOCREPARAM(msg) == WCH_LockData)
	{
		if(HICREPARAM(msg))
...
//
//////////////////////////////////////////////////////////////////////////
crJXJUIIDData::crJXJUIIDData() :
m_id(0)
{
}

crJXJUIIDData::crJXJUIIDData(const crJXJUIIDData& data) :
m_id(data.m_id)
{
}
void crJXJUIIDData::inputParam(int i, void *param)
{
	switch (i)
	{
	case WCHDATA_JXJUIID:
		m_id = param ? *((int*)param) : NULL;
		break;
	}
}

void crJXJUIIDData::getParam(int i, void*& param)
{
	switch (i)
	{
	case WCHDATA_JXJUIID:
