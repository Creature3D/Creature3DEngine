				...
				++itr )
			{
				...
				if(item && item->getDataClass())
				{
					...
					if(itemstate == IS_Dead)
					{
						...
						itemData->excHandle(MAKECREPARAM(WCH_InitData,item));
				...
				++itr )
			{
				if ((*itr).valid() && (*itr)->getDataClass())
				{
					...
					if(itemstate != IS_Dead)
					{
						...
						rthp = *(float *)param;
						rthp *= m_factor;
						DamagePair dp(rthp,std::make_pair(ExtraDM,0));
...
void crJXJExtraGainMpPerTimerMethod::operator()(crHandle &handle)
{
	...
	if(m_timer>=0.0f)
	{
		...
		if(m_dtrec >= m_interval)
		{
			...
			{
				float addMp = m_extraValue * (float)m_overlapCount;
				m_this->doEvent(MAKEINT64(WCH_JXJCureRTMp,0),MAKECREPARAM(&addMp,NULL));
			}
			else
			{
				float addMp = -m_extraValue * (float)m_overlapCount;
				m_this->doEvent(MAKEINT64(WCH_JXJReduceRTMp,0),MAKECREPARAM(&addMp,NULL));
			}
		}
	}
}
...
void crJXJExtraReduceMpPerTimerAndRemoveExtraMethod::operator()(crHandle &handle)
{
	...
	if(m_timer>=0.0f)
	{
		...
		if(m_dtrec >= m_interval)
		{
			...
			float reduceMp = m_extraValue * (float)m_overlapCount;
			if (rtmp < reduceMp)
			{
				*m_output = true;
			}
			...
			{
				m_this->doEvent(MAKEINT64(WCH_JXJReduceRTMp,0),MAKECREPARAM(&reduceMp,NULL));
			}
		}
	}
}
...
void crJXJGetSkillNeedRTMPMethod::inputParam(int i, void *param)
{
	...
	case 2:
		if(param)
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_output = (float *)(LOCREPARAM(param64));
			m_skillThis = (crInstanceItem*)(HICREPARAM(param64));
		}
		else
		{
			m_output = NULL;
...
//	{
//		m_hitParam->m_hitItem->doEvent(WCH_JXJReduceRTMp,MAKECREPARAM(&m_extraValue,0));
//	}
//}
//
...
void JXJ::crJXJHitParamRTMPMethod::operator()( crHandle &handle )
{
	if(m_hitParam.valid())
	{
		...
		case 0:
			...
			{
				if(m_extraValue>0)
					m_hitParam->m_fireItem->doEvent(WCH_JXJCureRTMp,MAKECREPARAM(&m_extraValue,0));
				else
				{
					float value = -m_extraValue;
					m_hitParam->m_fireItem->doEvent(WCH_JXJReduceRTMp,MAKECREPARAM(&value,0));
				}
			}
			break;
		case 1:
			...
			{
				if(m_extraValue>0)
					m_hitParam->m_hitItem->doEvent(WCH_JXJCureRTMp,MAKECREPARAM(&m_extraValue,0));
				else
				{
					float value = -m_extraValue;
					m_hitParam->m_hitItem->doEvent(WCH_JXJReduceRTMp,MAKECREPARAM(&value,0));
				}
			}
			break;
		}
...
void crJXJClearTargetExtraPerTimeMethod::operator()(crHandle &handle)
{
	...
	if(m_timer>=0.0f)
	{
		...
		{//每隔interval对外作用一次
			...
			void *param;
			crData *data = m_this->getDataClass();
			//CRCore::notify(CRCore::ALWAYS) << "角色名:" << m_this->getIName() << std::endl;
			...
			{////这个做法不合理，会引起宕机，暂时注掉
				...
				for( ExtraIDMap::iterator itr = extraIDMap->begin();
					itr != extraIDMap->end(); )
				{
					extraValue = crArgumentParser::appItoa(itr->first)+std::string("|")+crArgumentParser::appItoa(HIINT16(itr->second));
					//CRCore::notify(CRCore::ALWAYS) << "Extra = " << extraValue << std::endl;
					if(bufferAttrTab->queryOneRecord(0,extraValue,record)>=0)
					{
						if (m_exceptBufTypeSet.find(atoi(record[isNegativeIdx].c_str())) != m_exceptBufTypeSet.end())
						{
							...
							//CRCore::notify(CRCore::ALWAYS) << "Extra Cleared"<< std::endl;
						}
						else
						{
							++itr;
							//CRCore::notify(CRCore::ALWAYS) << "Extra类型不匹配" << std::endl;
						}
					}
					else
					{
						++itr;
						//CRCore::notify(CRCore::ALWAYS) << "BufferAttr表里没有" << std::endl;
					}
				}
