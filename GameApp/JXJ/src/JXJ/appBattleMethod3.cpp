...
void JXJ::crJXJPutInChatRecordDequeMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_chatInfo = (crJXJChatRecord *)LOCREPARAM(param64);
		}
		else
		{
			m_chatInfo = NULL;
	...
	//if (chat_select.valid())
	{
		...
		if(me && me->getPlayerGameData())
		{
			...
			if(playerData && itemtab.valid())
			{
				...
				CRCore::ScopedLock<crData> lock(*playerData);
				...
				if(!crScriptLoadManager::getInstance()->loadConfigScript(cfg_script,m_chat_cfg,true,""))
				{
					...
					char gbuf[256];
					sprintf(gbuf,"crJXJPutInChatRecordDequeMethod(): file open error %s\n\0",m_chat_cfg.c_str());
...
void crJXJCheckCampAddArmyEnableMethod::operator()(crHandle &handle)
{
	...
	if(myPlayer && crRunGameHandle::getInstance()->isInGame() && scene && scene->getExternSceneType() == Ext_Battle)
	{
		...
		if(myPlayerData)
		{
			...
			char isEnemy = 0;
			me->getCurrentRole()->doEvent(WCH_EnemyCheck,MAKECREPARAM(m_this,&isEnemy));
			//////////////////////////////////////////////////////////////////////////
...
void crJXJBattleInfoCenterNoticeCreateMethod::inputParam(int i, void *param)
{
	...
	case 2:
		if(param)
		{
			...
			m_iswin = *(bool *)(HICREPARAM(param64));
		}
		else
		{
			m_battleid = 0;
					...
					if(persuadeInfo.valid())
					{
						...
						if(persuadeInfo->getTime()<t)
						{
							...
							if(persuadeRewardTab.valid())
							{
								...
								m_this->doEvent(WCH_JXJRecvPlayerRewardInfo,MAKECREPARAM(rewardInfo.get(),NULL));
							}
						}
					}
...
void crJXJCheckGameServerCycleStartMethod::operator()(crHandle &handle)
{
	...
	{		
		...
		for (int i = 0;i<3;i++)
		{
			...
			isLoseCountry = false;
			m_this->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&isLoseCountry,&shiliid[i]));
			if(isLoseCountry)
			{
				countryLoseCount++;
				////将灭国的城池变为野城
		...
		if(countryLoseCount == 1 && m_cityLose == -1)
		{
			bool bStart = false;
			if(loseshiliid>0)
				m_this->doEvent(WCH_JXJCreateCountryLoseNotice,MAKECREPARAM(loseshiliid,&bStart));
		}
		else if(countryLoseCount>=2 && m_cityLose == -1)
		{
			...
			updateServerStats->buildUpdateSql(serverid, 1);
			...
			netDataManager->sendPacketToAllPlayer(packet);
			if(lastshiliid>0)
				m_this->doEvent(WCH_JXJCreateCountryLoseNotice,MAKECREPARAM(lastshiliid,&bStart));
		}
		brainData->excHandle(MAKECREPARAM(WCH_LockData,0));
	}
}
...
void crJXJTreasonCountryCanvasShowMethod::operator()(crHandle &handle)
{
	...
	if(myPlayer)
	{
		...
		if(playerData)
		{
			...
			//bool isLoseCountry = false;
			bool islose = false;
			myPlayer->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&islose,&myShiliid));
			if(islose){
			...
			for (int i = 0;i<3;i++){
				...
				myPlayer->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&islose,&shili));
							...
							if(mainRole && mainRole->getDataClass())
							{
								...
								else
								{
									...
									else
									{
										...
										{
											int needgold = atoi(crGlobalHandle::gData()->gGameGlobalValue(WCHDATA_JXJChangeShiLiNeedGold,vipLevel).c_str());
...
void crJXJRecvPlayerChangeShiliMethod::operator()(crHandle &handle)
{
	...
	if(m_netType == GameServer)
	{
		...
		if(playerData)
		{
			...
			//if (isCycle || (isLoseCountry && type == 3))
			//{
			//	_case = 2;
			...
			//{
				...
				{
					//判断所选势力是否已灭亡
					crServerBrainHandle::getInstance()->doEvent(WCH_JXJCheckCountryLose,MAKECREPARAM(&isDestroy,&shiliid));
					if(isDestroy)
					{
						_case = -6;//所选择势力已经灭亡
						break;
					...
					if (*changeTimes >= maxCount && !isLoseCountry/*type == 1*/)
					{
						_case = -1;//叛国次数已到
						break;
					}
					...
					if (/*type == 1 &&*/ (myid == chengxiangid || myid == dasimaid || myid == dajiangjunid) && !isLoseCountry)
					{
						_case = -9;//官员不能叛国
						break;
					}
					...
					playerData->getParam(WCHDATA_JXJTreasonCoolingTimer,param);
					int coolingtime = *(int*)param;
					if (/*type==1*/!isLoseCountry && coolingtime > 0)
					{
						_case = -10;//叛国冷却期
						break;
					}
					...
					time_t nowTime_t = time(0);
					struct  tm  nowTm = *localtime(&nowTime_t);
					if (/*type==1*/!isLoseCountry && (nowTm.tm_wday == 0 || nowTm.tm_wday == 6))
					{
						_case = -11;//在选举日
						break;
					}
					...
					if(mainRole)
					{
						...
						if(mainroledata)
						{
							...
							unsigned char needlv = (unsigned char)(atoi(crGlobalHandle::gData()->gGameGlobalValue(WCHDATA_JXJChangeShiLiNeedRoleLv,vipLevel).c_str()));
							if (/*type == 1*/!isLoseCountry && rolelv<needlv)
							{
								_case = -2;//等级不足
								break;
							}
							if (/*type == 1*/!isLoseCountry && mygroupid != 0)
							{
								_case = -3;//在军团中
								break;
							}
							...
							if (/*type==1*/!isLoseCountry && needgold> ((*goldingot) + (*giftgold)))
							{
								_case = -4;//元宝不足
								break;
							}
			...
			if(playerData.valid())
			{
				...
				////游戏日志
				std::string logdata = crArgumentParser::appVectoa(crVector3i(_case, shiliid, *changeTimes), '|');
				GameLogData gamelog(Log_ChangeShili, logdata);
		...
		if(_case == 1 || _case == 2)
		{
			...
			if(myPlayer)
			{
				...
				if(playerData && itemtab.valid())
				{
					...
					//crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_JiangHun,cardpiecesoffset));
					...
					//			if(jkshili == (*myshiliid)/*|| jkshili==21+ (*myshiliid)-c_startShiliID*/)
					//			{
...
void crJXJChangeShiliMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_shiliid = *(unsigned char *)(LOCREPARAM(param64));
		}
		else
		{
			m_shiliid = 0;
...
void crJXJChangeShiliMethod::operator()(crHandle &handle)
{
	...
	if(playerData && serverData && itemtab.valid())
	{
		...
		if(*shiliid!=m_shiliid && (m_shiliid == weiID || m_shiliid == shuID || m_shiliid == wuID))
		{
			...
			//		{
			//			jkshili = (unsigned char)atoi(record[cardshili].c_str());
			//			if(jkshili == (*shiliid)/* || jkshili == 21 + (*shiliid) - c_startShiliID*/)
			//			{
			...
			playerData->inputParam(WCHDATA_JXJPlayerGuanZhi, NULL);

			*shiliid = m_shiliid;
			ref_ptr<crStreamBuf> shiliStream = new crStreamBuf;
			newshiliData->excHandle(MAKECREPARAM(WCH_BuildPlayerStream,shiliStream.get()));
...
void crJXJCheckCountryLoseMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_bLoseCountry = (bool *)(LOCREPARAM(param64));
					...
					if(hasspace)
					{
						...
						*lanRewardCount-=1;
						...
						if(playerGameServerPlayerData.valid())
						{
							...
							gameServer->getNetManager()->sendPacket(playerGameServerPlayerData->getPlayerConnectServerAddress(), packet);
							m_this->doEvent(WCH_JXJRecvHuoyuedu, MAKECREPARAM(HYD_DailyLand, 1));
						}
					}
				}
			}
...
// 			if(!reward.valid())
// 			{
// 				m_this->doEvent(WCH_JXJRecvPlayerRewardInfo,MAKECREPARAM(reward.get(),NULL));
// 
...
void crJXJLandRewardDropClearMethod::operator()(crHandle &handle)
{
	...
	if(playerData)
	{
		...
		if(reward.valid())
		{
			...
			{
				crServerBrainHandle::getInstance()->doEvent(WCH_JXJCreateRewardMail,MAKECREPARAM(reward.get(),NULL));
			}
			playerData->inputParam(WCHDATA_JXJLandRewardDataInfo,NULL);//清除奖励信息
		}
		playerData->excHandle(MAKECREPARAM(WCH_LockData,0));
...
void crJXJClientRequestLandRewardMethod::operator()(crHandle &handle)
{
	...
	if(playerData)
	{
		...
		{
			//请先整理背包
			crMyPlayerData::getInstance()->getPlayerGameData()->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2008,NULL));
		}
	}
}
...
// 	ref_ptr<crDataBase> gameSession = gamedb->beginSession();
	...
	{
		gsData->excHandle(MAKECREPARAM(WCH_LockData,1));
		...
		for (int i = 0;i<3;i++)
		{
			...
			if(shiliData)
			{
				...
				for (;itr_pl!=peopleMap->end();itr_pl++)
				{
					...
					playerVipQuery->buildQuerySql(playerid, serverid);
					...
					if(!playerStoreResultVec.empty())
					{
						...
						//神器经验
						const std::string &super = playerStoreQuery->getSuper();
						if (!super.empty())
						{
							std::vector<int>v_i;
							crArgumentParser::appAtoVec(super, v_i, '|');
							int superCount = v_i.size();
							if (superCount > 0)
							{
								itr_pl->second->setSuperEquipLvupPoint(CRCore::maximum(itr_pl->second->getSuperEquipLvupPoint(), (unsigned char)v_i[0]),false);
								SuperEquipExpVec& superEquipExpVec = itr_pl->second->getSuperEquipExpVec();
								for (int i = 1; i < superCount; i++)
								{
									if (i < superEquipExpVec.size() + 1)
										superEquipExpVec[i - 1] += v_i[i];
									else
										superEquipExpVec.push_back(v_i[i]);
								}
							}
						}
						updatePlayerStore->buildUpdateSql_Cycle(playerStoreQuery->getID(),serverid,cycleCountAdd,money,totalMoney,vipexp,cardVec,itemVec, itr_pl->second->getSuper());
						crDBSavePacket::buildRequestPacket(packet,updatePlayerStore.get(),-1);
					}
					else
					{
						//插入记录
						insertPlayerStore->buildUpdateSql_Cycle(playerid,gameid,cycleCountAdd,money,totalMoney,vipexp,itr_pl->second->getSuper());
		...
		gsData->excHandle(MAKECREPARAM(WCH_LockData,0));
	}
	//2。清除相关数据库表
}
...
void crJXJRecvPlayerStoreMethod::operator()(crHandle &handle)
{
	...
	if(m_netType == GameServer)
	{
		...
		ref_ptr<crGameServerPlayerData> player = dynamic_cast<crGameServerPlayerData *>(netDataManager->getPlayerData(playerid));
		if (playerData && player.valid() && itemTable.valid())
		{
				...
				if(playerStoreData)
				{
					...
					for (int i = 0; i < size && i<CycleCount; i++)
					{
						...
						if (index < playerStoreBagVec->size() && \
							(*playerStoreBagVec)[index].valid() && (*playerStoreBagVec)[index]->getItemServerID() != serverid)
						{
					...
					//				if (haveitemcount >= storecount)
					//				{
					//					hasitem = true;
					//					selectidcount.push_back(crVector2i(i, haveitemcount - storecount));
					//					break;
					//				}
					//				else
					//				{
					//					selectidcount.push_back(crVector2i(i, 0));
					//					storecount -= haveitemcount;
					//				}
					//			}
					//		}
					//	}
					...
					//				ref_ptr<crStreamBuf> stream = new crStreamBuf;
					//				stream->createBuf(7);
					//				stream->_writeChar(1);//useResult=1
					...
					//				///游戏日志
					//				std::string logdata = "使用物品（success，itemid）：" + crArgumentParser::appVectoa(crVector2i(1, itemid));
							...
							for (; itr_item != bagVec->end(); itr_item++)
							{
									...
									itr_item->get()->getEquipMagic() == itemOrCard[i][1])
								{
									...
									if (i < playerStoreBagVec->size())
									{
										...
										tempStoreMap.insert(make_pair((unsigned char)i, tempitemData));
										(*playerStoreBagVec)[i] = tempitemData;
									}
									else
									{
										tempStoreMap.insert(make_pair((unsigned char)(playerStoreBagVec->size()), tempitemData));
...
// 												temp->setItemCount(1);
							...
							else if (_case == -5)
							{
								//道具不存在
								if (ucType == 0)
									texid = 1139;//要存储的将卡不存在！
								else
									texid = 1140;//要存储的装备不存在！
							}
						}
						//else
						//else
	...
	if (dbConductor)
	{
		...
		if (!m_taskVec.empty())
		{
				...
				++titr)
			{
				...
				//插入记录
				insertPlayerStoreRecordRecord->buildUpdateSql(playerStoreInfo->getPlayerId(), gameid, playerStoreInfo->getPlayerCycleCount(), playerStoreInfo->getPlayerMoney(), playerStoreInfo->getTotalMoney(), \
					playerStoreInfo->getVipExp(), playerStoreInfo->getStoreCardVec(), playerStoreInfo->getStoreItemVec(), playerStoreInfo->getSuperStr());
...
void crJXJQueryPlayerGameStoreMethod::operator()(crHandle &handle)
{
	...
	if(m_netType == GameServer)
	{
		...
		if(dbConductor)
		{
			...
			dbSession->releaseQuery();
			accountdb->endSession(dbSession.get());
			if(!playerStoreResultVec.empty() && playerStoreResultVec[0].valid())
			{
				playerStoreQuery = dynamic_cast<crPlayerGameStoreQueryData *>(playerStoreResultVec[0].get());
				playerData->excHandle(MAKECREPARAM(WCH_LockData,1));
				playerData->getParam(WCHDATA_JXJPlayerStore,param);
				...
				if(playerStoreData)
				{
					...
					int vipexp = playerStoreQuery->getVipExp();
					playerStoreData->inputParam(WCHDATA_JXJVipExp, &vipexp);
					playerStoreData->getParam(WCHDATA_JXJItemBagVec,param);
					JXJStoreItemBagVec *itemVec = (JXJStoreItemBagVec *)param;
					...
					*cardVec = playerStoreQuery->getCardBagVec();
					...
					//}
					//神器轮回
					if (!playerStoreQuery->getSuper().empty())
					{
						std::vector<int>v_i;
						crArgumentParser::appAtoVec(playerStoreQuery->getSuper(), v_i, '|');
						int superCount = v_i.size();
						if (superCount > 0)
						{
							...
							*lvupoint = v_i[0];
							...
							superEquipCycleExpVec->clear();
							for (int i = 1; i<superCount;i++)
							{
								superEquipCycleExpVec->push_back(v_i[i]);
							}
						}
					}
					playerStoreData->excHandle(MAKECREPARAM(WCH_LockData,0));
					...
					playerStoreData->excHandle(MAKECREPARAM(WCH_BuildPlayerStream,dataStream.get()));
	...
	else if(m_netType == GameClient_Game)
	{
		...
		//playerStoreData->excHandle(MAKECREPARAM(WCH_BuildSaveStream,streamBuffer.get()));
...
//
/////////////////////////////////////////
crJXJGetPlayerGameStoreMethod::crJXJGetPlayerGameStoreMethod():m_output(NULL){}
...
void crJXJGetPlayerGameStoreMethod::operator()(crHandle &handle)
{
	...
	if(dbConductor && gsData)
	{
		...
		if(!playerStoreResultVec.empty())
		{
			...
			playerStoreData = playerStore->getDataClass();
			ref_ptr<crTableIO>itemtab = crGlobalHandle::gData()->gGlobalTable(WCHDATA_JXJItemTab);
			if(playerStoreData && itemtab.valid() && m_stream.valid())
			{
				...
				int distillvipexp = 0;
				std::vector<int> distillSuper;
				std::string remainSuperStr;
				...
				CRCore::ScopedLock<crData> lock1(*playerStoreData);
				//playerStoreData->excHandle(MAKECREPARAM(WCH_LockData,1));
				...
				if (select == 0)
				{
					...
					if (vipexp > 0)
					{
						...
						playerStoreRecordInfo->setVipExp(vipexp);
						distillvipexp = vipexp;
					}
					logdata += crArgumentParser::appVectoa(crVector4i(select, cyclecount, distillmoney, vipexp), '|');
				}
				else if (select == 1)
				{//取礼金和活动
					int totalmoney = playerStoreQuery->getTotaladdmoney();
					...
					logdata += crArgumentParser::appVectoa(crVector4i(select, cyclecount, distillTotalMoney, giftgold), '|');
				}
				else if (select == 2||select==3)
				{
					...
					if (type == 1)
					{
						int count = m_stream->_readChar();
						for (int i = 0; i < count; i++)
						{
							IDSet.insert(m_stream->_readChar());
						}
					}
					///物品轮回
					...
					{//取将卡
						...
						*cardVec = playerStoreQuery->getCardBagVec();
						...
						{
							int needGold = atoi(crGlobalHandle::gData()->gGameGlobalValue(WCHDATA_JXJDistillForeignCardCost, vipLevel).c_str()) * foreignCardCount;
							if (needGold > 0)
							{
								MoneyChangeData moneydata(needGold, "取出轮回异国将卡");
								m_this->doEvent(WCH_JXJDeductLijinGold, MAKECREPARAM(&moneydata, &needdeductgold));
								if (moneydata.first == -1)
								{//元宝不足
									if (m_output) *m_output = -1;
									return;
								}
							}
						}
					...
					///////发放轮回物品
					m_this->doEvent(WCH_JXJRecvRewardItems, MAKECREPARAM(&rewardItems, MAKEINT32(0, 1)));
				}
				else if (select == 4)
				{//提取神器轮回参数
					//神器轮回
					if (m_this->getMainRole() && m_this->getMainRole()->getDataClass())
					{
						...
						if (superEquipZhulingData && superEquipDataVec->size() == SuperEquipCount)
						{//神器以及激活
							playerStoreData->getParam(WCHDATA_JXJSuperEquipCycleLvupPoint, param);
							unsigned char *lvupoint = (unsigned char *)param;
							playerStoreData->getParam(WCHDATA_JXJSuperEquipCycleExpVec, param);
							...
							int dbsize = superEquipCycleExpVec->size();
							distillSuper.resize(dbsize+1,0);
							...
							if (*superEquipLvupPoint < *lvupoint)
							{
								//*superEquipLvupPoint = *lvupoint;
								m_this->getMainRole()->doEvent(WCH_JXJGainSuperEquipLvupPoint, MAKECREPARAM(*lvupoint, NULL));
								logdata += "神器升级效果:" + crArgumentParser::appItoa(*lvupoint);
								distillSuper[0] = *lvupoint;
								*lvupoint = 0;
							}
							//stream->_writeUChar(*superEquipLvupPoint);
							stream->_writeUChar(*lvupoint);
							unsigned short gainpoint = 0;
							for (int i = 0; i < SuperEquipCount; i++)
							{
								if (i < dbsize)
								{
									gainpoint = (*superEquipDataVec)[i]->addSuperEquipExp((*superEquipCycleExpVec)[i], *superEquipLvupPoint);
									superEquipZhulingData->addZhulingPoint(gainpoint);
									logdata += "神器经验:" + crArgumentParser::appVectoa(crVector2i(i, (*superEquipCycleExpVec)[i]));
									distillSuper[i + 1] = (*superEquipCycleExpVec)[i];
									(*superEquipCycleExpVec)[i] = 0;
								}
								stream->_writeUChar((*superEquipDataVec)[i]->getSuperEquipLevel());
								stream->_writeInt((*superEquipDataVec)[i]->getSuperEquipExp());
							}
							...
							{
								crPlayerDataEventPacket packet;
								crPlayerDataEventPacket::buildReplyPacket(packet, playerid, WCH_JXJRecvDistillSuperEquip, stream.get());
								gameServer->getNetManager()->sendPacket(playerData->getPlayerConnectServerAddress(), packet);
							}
							remainSuperStr = crArgumentParser::appItoa(*lvupoint)+'|'+crArgumentParser::appVectoa(*superEquipCycleExpVec, '|');
							playerStoreRecordInfo->setSuper(distillSuper);
						}
					}
				}
				//playerStoreData->excHandle(MAKECREPARAM(WCH_LockData, 0));
				...
				CRCore::ref_ptr<crUpdatePlayerGameStore > updatePlayerStore = new crUpdatePlayerGameStore;
				if (select != 4)
					updatePlayerStore->buildUpdateSql_Distill(playerStoreQuery->getID(), distillmoney, distillTotalMoney, distillvipexp, distillCardVec, distillItemVec);
				else if(!remainSuperStr.empty())
					updatePlayerStore->buildUpdateSql_Distill(playerStoreQuery->getID(), remainSuperStr);
				crDBSavePacket packet;
				crDBSavePacket::buildRequestPacket(packet,updatePlayerStore.get(),-1);
				dbConductor->getNetManager()->sendPacket("all",packet);
	...
	case 20:
		{
			...
			m_strImgItem[i - 1][0] = strvec[0];
			m_strImgItem[i - 1][1] = strvec[1];
		}
		break;
	}
}
void crJXJUIAddItemDragMethod::operator()(crHandle &handle)
{
	...
	if(playerData)
	{
		...
		if(pickItemNode && pickItemNode->getParentCanvas() == lunhuiCanvas && itemTab.valid())
		{
			...
			{
				if(pickItemNode->getName() == m_strImgItem[i][0])
				{
					/*if(i>0)
					{
						...
						if(playerData)
						{
							...
							if(playerStore)
							{
								...
								if(playerStoreData)
								{
									...
									//if(i >= itemVec->size() || \
									//	(i<itemVec->size() && (*itemVec)[i].valid() && (*itemVec)[i]->getItemServerID() == 1))
									{
										//
										//crMultiSwitch *multiswitch_level =dynamic_cast<crMultiSwitch *>(lunhuiCanvas->getChildNode(m_strSwCardLv[i]));
										crImageBoxWidgetNode *imageBox = dynamic_cast<crImageBoxWidgetNode *>(lunhuiCanvas->getChildNode(m_strImgItem[i][1]));
										if(imageBox)
										{
											ref_ptr<crRadioGroupWidgetNode> cardPage = dynamic_cast<crRadioGroupWidgetNode *>(lunhuiCanvas->getWidget("XLunhui_zhuangbei_qieye"));
											...
											//itemBagCanvasData->excHandle(MAKECREPARAM(WCH_LockData,1));
											playerData->getParam(WCHDATA_JXJItemBagVec, param);
											JXJItemBagVec *itemBagVec = (JXJItemBagVec *)param;
											...
											if(itemTab->queryOneRecord(0, crArgumentParser::appItoa(item_id), record)>=0)
											{
												...
												if(canvasData)
												{
													...
													if(cardmap)
													{
														...
														//处理wupin与背包显示
														ItemOrCardMap::iterator itr_item = cardmap->find(offseti);
														...
														{
															if (offseti<itemVec->size() && (*itemVec)[offseti].valid())
															{
																ref_ptr<crBagItemData> temp = new crBagItemData;
																temp->setItemID((*itemVec)[offseti]->getItemID());
																temp->setEquipMagic((*itemVec)[offseti]->getEquipMagic());
																temp->setItemCount(1);

																//放入临时转换背包
																switchmap->insert(std::make_pair(offseti,(*itemVec)[offseti]));
											...
											//itemBagCanvasData->excHandle(MAKECREPARAM(WCH_LockData,0));
										}
									}
...
// 						}
					...
					{
						//自己
						crMyPlayerData::getInstance()->getPlayerGameData()->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(4009,NULL));
					}
				}
		...
		{
			//提示势力不一样
			crMyPlayerData::getInstance()->getPlayerGameData()->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(4010,NULL));
		}
		canvasData->excHandle(MAKECREPARAM(WCH_LockData,0));
	}
}
...
void crJXJTaskBarCanvasOnShowMethod::operator()(crHandle &handle)
{
	...
	if(playerdata)
	{
		...
		if(cardPieceoffset>0)
		{
			...
			//crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_Exp,exp));
		}
		//playerdata->excHandle(MAKECREPARAM(WCH_LockData,1));
	}
}
		...
		if(playerdata)
		{
			...
			if(cardPieceoffset>0)
			{
				*cardPiece += cardPieceoffset;
				...
				crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKECREPARAM(T_JiangHun,cardPieceoffset));
			}
...
void crJXJRecvFubenKillInfoMethod::inputParam(int i, void *param)
{
	...
	case 2:
		if(param)
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_stream = (crStreamBuf *)(LOCREPARAM(param64));
			crVector2i nettypepair = *(crVector2i *)(HICREPARAM(param64));
...
void crJXJCreateCountryLoseNoticeMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_shiliid = LOCREPARAM(param64);
			m_isCycle = *(bool *)(HICREPARAM(param64));
		}
		else
		{
			m_shiliid = 0;
		...
		if (autoclose > 0.0f)
		{
			...
			if (*timer <= 0.0f)
			{
				...
				*curstep += 1;
				...
				m_this->doEvent(WCH_JXJJXJShowScenarioTalk, MAKECREPARAM(&str, NULL));
				*timer = autoclose;
...
void crJXJInsertBattleReportDisposeMethod::operator()(crHandle &handle)
{
	...
	if (!disposeDeq->empty())
	{
		...
		if (insertid > 0)
		{
			...
			m_this->doEvent(WCH_JXJSystermNoticeCreate, MAKECREPARAM(achatInfo.get(), NULL));
			m_this->doEvent(WCH_JXJSystermNoticeCreate, MAKECREPARAM(dchatInfo.get(), NULL));
		}
	}
...
void crJXJClickHeadCanvasSelectRoleMethod::operator()(crHandle &handle)
{
	...
	if (canvasData && camera.valid())
	{
		...
		if (item != NULL)
		{
			...
			item->doEvent(WCH_Touch, MAKECREPARAM(m_ea, NULL));
		}
	}
}
...
void crJXJSkillEffectRangeMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_centerPos = *(crVector3f *)(LOCREPARAM(param64));
			crVector2f vec = *(crVector2f *)(HICREPARAM(param64));
			m_range = vec[0];
...
void crJXJSkillEffectRangeMethod::operator()(crHandle &handle)
{
		...
		++itr)
	{
		...
		if (checkPointInRange(role->getPosition(), m_centerPos, m_range))
		{
			crMyPlayerData::getInstance()->getCurrentRole()->doEvent(WCH_EnemyCheck, MAKECREPARAM(role, &isEnemy));
		...
		++itr)
	{
		...
		{
			crMyPlayerData::getInstance()->getCurrentRole()->doEvent(WCH_EnemyCheck, MAKECREPARAM(item, &isEnemy));
			if (isEnemy == -1 && (m_targetType & Target_Enemy))
			{
				crSetObjectOutlineColorModeVisitor selectNodeVisitor(crObject::OL_Red);
...
void crJXJSkillEffectRectangleMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			CREPARAM& param64 = *(CREPARAM*)param;
			m_matrixVolume = (crMatrixVolumeNode *)(LOCREPARAM(param64));
			m_targetType = *(unsigned char *)(HICREPARAM(param64));
		}
		else
		{
			m_matrixVolume = NULL;
