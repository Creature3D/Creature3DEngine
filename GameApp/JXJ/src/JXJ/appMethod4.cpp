...
void crJXJAdvanceTask2Method::operator()(crHandle &handle)
{
	...
	if(playerData && playerData->getDataClass())
	{
		playerData->doEvent(WCH_JXJAdvanceTask, MAKECREPARAM(m_taskid,m_stepType));
	}
}
...
void crJXJBuildingTaskEffectMethod::operator()(crHandle &handle)
{
	...
	if(scene)
	{
		...
		if(item)
		{
			item->doEvent(WCH_JXJBuildingTaskEffect,MAKECREPARAM(m_visiable?1:0,NULL));
		}
	}
}
/////////////////////////////////////////
...
void crJXJCheckDirectlySubmitTaskMethod::operator()(crHandle &handle)
{
	...
	if(playerData)
	{
		playerData->doEvent(WCH_JXJReferTask,MAKECREPARAM(m_taskid,NULL));
	}
}
...
void crJXJCheckPutOnJiangKaTaskMethod::operator()(crHandle &handle)
{
	...
	if(formationgdata)
	{
		formationgdata->excHandle(MAKECREPARAM(WCH_LockData,1));
		formationgdata->getParam(WCHDATA_JXJTempFormationVec,param);
		FormationInfoVec *formationInfoVec = (FormationInfoVec *)param;
		if (formationInfoVec->size() > 0)
		{
		...
		formationgdata->excHandle(MAKECREPARAM(WCH_LockData,0));
	}
	handle.outputParam(0,&check);
}
	...
	if (playerGameData && tips.valid())
	{
		...
		{		
			...
			if(mainroledata)
			{
				...
				mainRole->doEvent(WCH_JXJGetBingFa,MAKECREPARAM(&totalbingfa,NULL));
				...
				mainRole->doEvent(WCH_JXJGetZhenFa,MAKECREPARAM(&totalzhenfa,NULL));
...
void crJXJAutoMainLineTaskMethod::operator()(crHandle &handle)
{
	...
	if(playerData && playerData->getDataClass())
	{
		playerData->doEvent(WCH_JXJReferTask,MAKECREPARAM(m_this->getTaskID(),NULL));
	}
}
...
void crJXJUIFuKuProResourceCountMethod::operator()(crHandle &handle)
{
	...
	if(mainRole && data)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			playerData->doEvent(WCH_JXJGetTechResProductPercent,MAKECREPARAM(&percent1[i],i+1));
...
void crJXJCardShopUpdateMethod::addParam(int i, const std::string& str)
{
	...
	case 62:
		...
		break;
	case 63:
		m_tjCardTypeRadio = str;
		break;
	case 64:
		m_tjCardLevelRadio = str;
		break;
	case 65:
	case 66:
	case 67:
...
void crJXJCardShopUpdateMethod::operator()(crHandle &handle)
{
	...
	if(m_this && playerGameData && itemtab.valid())
	{
		...
		if (canvasdata && typeradio && colorradio && typeindex >= 0 && colorindex >= 0 && iconindex >= 0 && priceindex >= 0)
		{
				...
				if (!cardrecords.empty())
				{
						...
						++itr)
					{
						if (atoi((*itr)[priceindex].c_str()) > 0)
						{
							...
							if (cardshili == 2 || shiliid == cardshili)
							{
								cardCount++;
			...
			else if (jiangkatype == 3)
			{//星卡图鉴
				if (foreignCardSwitch.valid())
					foreignCardSwitch->setActiveSwitchSet(1);
				crRadioGroupWidgetNode *tjCardTypeRadio = dynamic_cast<crRadioGroupWidgetNode *>(m_this->getWidget(m_tjCardTypeRadio));
				crRadioGroupWidgetNode *tjCardLevelRadio = dynamic_cast<crRadioGroupWidgetNode *>(m_this->getWidget(m_tjCardLevelRadio));
				//crButtonWidgetNode *tjPageUp = dynamic_cast<crButtonWidgetNode *>(m_this->getWidget(m_tjPageUp));
				//crButtonWidgetNode *tjPageDown = dynamic_cast<crButtonWidgetNode *>(m_this->getWidget(m_tjPageDown));
				crStaticTextWidgetNode *tjPageNum = dynamic_cast<crStaticTextWidgetNode *>(m_this->getWidget(m_tjPageNum));
				crImageBoxWidgetNode *tjCardIcon[12] = { NULL };
				crMultiSwitch *tjCardSW[12];
				for (int i = 0; i < 12; i++)
				{
					tjCardIcon[i] = dynamic_cast<crImageBoxWidgetNode *>(m_this->getWidget(m_tjCard[i]));
					tjCardSW[i] = dynamic_cast<crMultiSwitch *>(m_this->getChildNode(m_tjCardSW[i]));
					if (tjCardIcon[i])tjCardIcon[i]->setVisiable(false);
					if (tjCardSW[i])
					{
						tjCardSW[i]->setVisiable(false);
						tjCardSW[i]->setActiveSwitchSet(0);
					}
				}
				crTableIO::DataVec cardrecords;
				int jiangkatype = tjCardTypeRadio->getSelect();
				if (jiangkatype < 3)
				{
					int jiangkacolor = tjCardLevelRadio->getSelect();
					//crRadioGroupWidgetNode::RadioGroup&radioGroup = tjCardLevelRadio->getRadioGroup();
					int colorsize = jiangkacolor+5;
					canvasdata->getParam(WCHDATA_JXJCardShopCurPage, param);
					int curpage = *(int *)param;
					canvasdata->getParam(WCHDATA_JXJCardShopItemDeq, param);
					ShopCardIDDeq *cardiddeq = (ShopCardIDDeq *)param;
					...
					int totalpage = 0;
					itemtab->queryRecords(typeindex, crArgumentParser::appItoa(jiangkatype), colorindex, crArgumentParser::appItoa(colorsize), cardrecords);
					//CRCore::notify(CRCore::ALWAYS) << "type = " << jiangkatype << "colorindex=" << colorsize << " cardrecords=" << cardrecords.size() << std::endl;
					if (!cardrecords.empty())
					{
						for (crTableIO::DataVec::iterator itr = cardrecords.begin();
							itr != cardrecords.end();
							++itr)
						{
							...
							//{
								cardshili = atoi((*itr)[shiliindex].c_str());
								if (cardshili != 20 /*< c_startShiliID || shiliid == cardshili*/)
								{
									cardCount++;
									if (row >= 0 && row < onepagecount)
									{
										...
										if (tjCardIcon[row])
										{
											tjCardIcon[row]->setVisiable(true);
											tjCardIcon[row]->setImageName((*itr)[iconindex]);
											//CRCore::notify(CRCore::ALWAYS) << "row = " << row << "setImageName=" << (*itr)[iconindex] << std::endl;
										}
										if (tjCardSW[row])
										{
											tjCardSW[row]->setVisiable(true);
											tjCardSW[row]->setActiveSwitchSet(atoi((*itr)[colorindex].c_str())-5);
										}
									}
									row++;
								}
					...
					if (tjPageNum)
					{
						tjPageNum->setString(crArgumentParser::appItoa(curpage) + "/" + crArgumentParser::appItoa(totalpage));
					}
				}
			}
			else if(jiangkatype == 4 && lv >= openlv)
			{
				if (foreignCardSwitch.valid())
					foreignCardSwitch->setActiveSwitchSet(2);
				...
				if (needbagcount == 0)
				{
					...
					if(itemtab->queryOneRecord(0,crArgumentParser::appItoa(cardid),record) >= 0)
					{
						...
						if (priceindex > 0)
						{
							...
							{
								playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2039,NULL));
							}
						}
					}
				}
...
void crJXJPurChaseCardMethod::operator()(crHandle &handle)
{
	...
	if(playerGameData)
	{
		...
		{
			playerGameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2082,NULL));//请先研发将卡商店科技！
		}
	}
}
/////////////////////////////////////////
...
void crJXJRecvPurChaseCardMethod::operator()(crHandle &handle)
{
	...
	if(m_this && m_stream.valid())
	{
					...
					if (needbagcount == 0)
					{
						...
						if(itemtab->queryOneRecord(0,crArgumentParser::appItoa(cardid),record) >= 0)
						{
							...
							if (priceindex > 0)
							{
								price = -atoi(record[priceindex].c_str());
								data->excHandle(MAKECREPARAM(WCH_LockData,1));
								m_this->doEvent(WCH_JXJChangePlayerData,MAKECREPARAM(&price,T_JiangHun));
								if(0 == price)
								{
									success = 1;
									//*cardpieces -= price;
		...
		else if(m_netType == GameClient_Game)
		{
			...
			{
				m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2034,NULL));//将卡背包已满
			}
			else if (success == 4)
			{
				m_this->doEvent(WCH_JXJUIShowTipsCanvas,MAKECREPARAM(2082,NULL));//请先研发将卡商店科技！
			}
		}
	}
}
			...
			if (data)
			{
				...
				if (progresscount > 1)
				{
					...
					if(FuBenTab->queryOneRecord(0,crArgumentParser::appItoa(selfubenid),record) >= 0)
					{
						...
						playerData->doEvent(WCH_JXJCheckLijinGold,MAKECREPARAM(&checkneedgold,NULL));
						if(checkneedgold == 0)
						{
...
void crJXJUIGongFangRefineEquipUpdateMethod::operator()(crHandle &handle)
{
	if(m_this)
	{
		...
		if (data &&equipRaido && equipListctrl && gongfangradio && lordequipradio && mainroledata)
		{
			...
			int equipsize = openedEquipVec.size();
			if (loadequipselect<equipsize)
			{
				equipInfo = openedEquipVec[loadequipselect];
			}
			crRadioGroupWidgetNode::RadioGroup &radioGroup = lordequipradio->getRadioGroup();
			for (int i = 0; i < radioGroup.size(); i++)
			{
				radioGroup[i].second->setEnable(i < equipsize);
			}
...
void crJXJUITheSmallMapPointInitMethod::operator()(crHandle &handle)
{
	...
	if(myPlayer)
	{
		...
		ref_ptr<crWidgetNode> cityHallOrBossIcon = dynamic_cast<crWidgetNode *>(Canvas->getWidget(m_cityHallOrBossIcon));
		if(!cityHallOrBossIcon) return;
		crStateSet *state_set = dynamic_cast<crObject *>(( dynamic_cast<CRCore::crGroup *>(cityHallOrBossIcon.get()))->getChild(0))->getDrawable(0)->getStateSet();
		if(!state_set) return;
		...
		rotMapWidget->insertChild(1, cityHallOrBossIcon.get());
			...
			if(taskTitleTab->queryOneRecord(taskidindex,crArgumentParser::appItoa(taskid),taskrecord) >= 0)
			{
				...
				if (curfubenid == fubenid)
				{
					...
					if (itrcur != fubenProgress->end() && itrcur->second->getComplete() > 0  && (*rushTaskCount) > 0)
					{
						...
						//	}
						//	else */
						//	if(m_this->isTaskCanAdvance(TS_One))
						//	{
						//		playerData->doEvent(WCH_JXJAdvanceTask,MAKEINT64(taskid,TS_One));
						//		//m_trueExecCount++;
						//	}
						...
						if (m_this->isTaskCanAdvance(TS_One))
						{
							playerData->doEvent(WCH_JXJAdvanceTask, MAKECREPARAM(taskid, TS_One));
						}
						(*rushTaskCount)--;
					}
				}
	...
	if(playerData)
	{
		...
		if(taskTab->queryOneRecord(0,crArgumentParser::appItoa(taskid),record) >= 0)
		{
			...
			case 0:
				...
				break;
			case 1:
				playerData->doEvent(WCH_JXJReferTask,MAKECREPARAM(taskid,TS_One));
				break;
			default:
				break;
			}
