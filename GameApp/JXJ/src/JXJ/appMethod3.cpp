...
void JXJ::crJXJUIOnFormationShowMethod::operator()(crHandle &handle)
{
	...
	int total_lead = 0;
	playerData->doEvent(WCH_JXJFormationGetLeaderPower,MAKECREPARAM(&total_lead,NULL));
	// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
	// global usage
	// temp
...
void JXJ::crJXJUIOnFormationArmCancelMethod::operator()(crHandle &handle)
{
	...
	card_data->excHandle(MAKECREPARAM(WCH_LockData, 1));
	...
	if (cardcount > 0)
	{
		...
		card_data->excHandle(MAKECREPARAM(WCH_LockData, 0));
...
void crJXJUIOnFormationStoreRecvMethod::operator()(crHandle &handle)
{
	...
	if(m_net_type == CRNetApp::GameServer)
	{
		...
		m_this->doEvent(WCH_JXJFormationGetLeaderPower,MAKECREPARAM(&totalleaderpower,NULL));
		if (formationmax == 0)
		{
			formationmax = 1;
		}
		...
		if(is_valid==0)
		{
			...
			crServerBrainHandle::getInstance()->doEvent(WCH_GameLog,MAKECREPARAM(player_id,&gamelog));
		}
	}
	else if(m_net_type == CRNetApp::GameClient_Game)
	{
		...
		//void *param;
		//CRCore::crData	*player_data		= m_this->getDataClass();
		//player_data->excHandle(MAKECREPARAM(WCH_LockData,1)); // locked +++++++
		...
		if (pageselect == -1)
		{
			...
			formation_canvas_data->excHandle(MAKECREPARAM(WCH_LockData,1));
			formation_canvas_data->addParam(WCHDATA_JXJFormationMove, "0");
			formation_canvas_data->excHandle(MAKECREPARAM(WCH_LockData,0));
...
//	room_data->excHandle(MAKECREPARAM(WCH_LockData,1));
...
//	room_data->excHandle(MAKECREPARAM(WCH_LockData,0));
//}
...
//{
//	crData *brain_data = crBrain::getInstance()->getDataClass();
//	//brain_data->excHandle(MAKECREPARAM(WCH_LockData,1));
//	void *param;
//	brain_data->getParam(WCHDATA_JXJBrainCurOpened, param);
...
//		brain_data->inputParam(WCHDATA_JXJBrainCurRTTRF, &needRefresh);
//	}
//	//brain_data->excHandle(MAKECREPARAM(WCH_LockData,0));
//}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
