...
{//SceneServer Method每秒相当于当前普通攻击/生命/护甲百分比的伤害
public:
...
{//SceneServer Method 立即受到相当于当前普通攻击/生命/护甲百分比的伤害
public:
...
{//SceneServer Method 回复血量 m_type;//0:根据最大血量回复，1根据已经损失血量回复
public:
...
protected:
	...
	float m_extraValue;
	float m_percent;
	char m_type;//0:根据最大血量回复，1根据已经损失血量回复
...
protected:
	...
	float m_value;//
	float m_percent;//最大血量/已经损失血量百分比
	char m_curtype;//0:根据最大血量回复，1根据已经损失血量回复
};
...
{//SceneServer & GameClient Method血量护盾
public:
...
{//SceneServer & GameClient Method,m_duration期间血量护盾被打破返回true
public:
...
{//SceneServer & GameClient Method,返回true表示近战
public:
...
protected:
	...
	char m_flg;//0:攻击者位置向外移动,1:攻击者位置向内移动,2混乱移动,3朝目标点位移,4:目标点位置向外移动,5:目标点位置向内移动
...
{//SceneServer & GameClient Method 延迟触发 在延迟时间内返回True，超过延迟时间返回false
public:
...
{//SceneServer Method 攻击者或者受击者正处于某些类型的BUF下
public:
...
{//SceneServer & GameClient Method,m_this是否正处于某些类型的BUF下
public:
...
{//SceneServer & GameClient Method buff叠加次数控制器，返回值为叠加次数
public:
...
{//SceneServer Method & GameClient Method进入关卡玩家等级检测
public:
...
{//SceneServer Method & GameClient Method进入关卡玩家战斗力检测
public:
...
{//SceneServer Method 攻击者血量<受击者返回true
public:
...
{//SceneServer Method 血量<总血量的百分比返回true
public:
...
protected:
	...
	CRCore::ref_ptr<CRNetApp::HitParam> m_hitParam;
	char m_type;//0:fireitem,1hititem
	float m_percent;//默认0.5
};
class crJXJExtraHpPercentLesserMethod : public CRCore::crMethod
{//SceneServer & GameClient Method 当玩家当前血量<总血量的百分比返回true
public:
...
{//SceneServer Method 攻击者属性<受击者返回true 291:当前血量,60护甲,59攻击力 269暴击 270格挡 271闪避 274视野 70移动速度 250潜行 252反潜
public:
...
{//SceneServer Method 受击者受到攻击者当前攻击力*m_percent+m_baseDamage的伤害
public:
...
{//SceneServer Method 攻击者移速>受击者返回true
public:
...
{//SceneServer & GameClient Method 当攻击距离>=m_dist时返回true,否则返回false
public:
...
protected:
	CRNetApp::crInstanceItem* m_this;
	CRCore::crVector3 m_firePos;
	float m_dist;//当攻击距离>=m_dist时返回true,否则返回false
};
...
protected:
	CRNetApp::crInstanceItem* m_this;
	int m_maxCount;//当触发次数<m_maxCount，返回true,否则返回false
	int m_counter;
};
class crJXJExtraArmyCheckMethod : public CRCore::crMethod
{//SceneServer Method,对m_this身边的部队数量检测>=m_maxCount返回true
public:
...
protected:
	...
	float m_range;//范围,米
	int m_maxCount;
	int m_enemyType;//1:队友，2敌人，3全部
	int m_npcType;//0:玩家，1:npc，2玩家或者npc
	int m_troopType;//0:步兵，1弓兵 2骑兵 3器械 4建筑 5全部, 6和我同兵种,7和我不同兵种
};
...
{//SceneServer Method受到伤害类型检测，当前伤害类型相同返回true
public:
...
{//SceneServer Method 受到攻击者的一次普通攻击
public:
