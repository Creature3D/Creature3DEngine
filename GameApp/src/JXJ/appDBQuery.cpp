...
void crPlayerGameStoreQueryData::addParam(int i, char *buf, int length)
{
	...
	case 13:
		if (length > 0)//0,id,1,count,2,serverid,3,iscycle
		{
			...
			m_cardBagVec.push_back(bag);
		}
		else
		{
			m_cardBagVec.push_back(NULL);
		}
		break;
	case 14:
	case 15:
...
{
	char sql[1024];
	sprintf(sql, "select id,cyclecount,money,totaladdmoney,card0,card1,card2,card3,card4,card5,card6,card7,card8,card9,item0,item1,item2,item3,item4,item5,item6,item7,item8,item9 from playergamestore where playerid=%d and gameid=%d\0", playerid, gameid);
	setQuerySql(sql);
	setNumColumn(24);
}
...
short crPlayerGameStoreQueryData::getCyclecount()
{
	return m_cyclecount;
}
...
void crUpdatePlayerGameStore::buildUpdateSql(int id,int serverid,JXJStoreItemBagVec & cardVec,JXJStoreItemBagVec & itemVec)
{
	...
	for (int i = 0; i < CycleStoreCount; ++i)
	{
		strValue.clear();
		if(i<itemcount)
		{
...
void crUpdatePlayerGameStore::buildUpdateSql_Cycle(int id,int serverid, int cycleCountAdd, int money, int totaladdmoney,JXJStoreItemBagVec &cardVec, JXJStoreItemBagVec &itemVec)
{
	m_sqlstream = new crStreamBuf;
	m_sqlstream->createBuf(2048);
	char sql[512];
	sprintf(sql, "update playergamestore set cyclecount=cyclecount+%d,money=money+%d,totaladdmoney=totaladdmoney+%d,\0",cycleCountAdd, money, totaladdmoney);
	m_sqlstream->write(sql, strlen(sql));
...
void crUpdatePlayerGameStore::buildUpdateSql_Distill(int id,int distillmoney,int distilltotalmoney,std::vector<int>&cardVec,std::vector<int>&itemVec)
{
	m_sqlstream = new crStreamBuf;
	m_sqlstream->createBuf(2048);
	char sql[512];
	sprintf(sql, "update playergamestore set money=money-%d,totaladdmoney=totaladdmoney-%d,\0",distillmoney,distilltotalmoney);
	m_sqlstream->write(sql, strlen(sql));
...
void crInsertPlayerGameStore::buildUpdateSql(int playerid, int gameid,JXJStoreItemBagVec & cardVec, JXJStoreItemBagVec & itemVec)
{
	...
	m_sqlstream->createBuf(2048);
	char sql[512];
	sprintf(sql, "insert into playergamestore(playerid,gameid,card0,card1,card2,card3,card4,card5,card6,card7,card8,card9,item0,item1,item2,item3,item4,item5,item6,item7,item8,item9,createdate) values(%d,%d,\0", playerid, gameid);
	m_sqlstream->write(sql, strlen(sql));
	...
	for (int index = 0; index < CycleStoreCount; ++index)
	{
		strValue.clear();
		if (index < size)
		{
...
void crInsertPlayerGameStore::buildUpdateSql_Cycle(int playerid, int gameid, int cycleCountAdd, int money, int totaladdmoney)
{
	char sql[256];
	sprintf(sql, "insert into playergamestore(playerid,gameid,cyclecount,money,totaladdmoney,createdate) values(%d,%d,%d,%d,%d,now())\0",playerid,gameid,cycleCountAdd,money,totaladdmoney);
	setUpdateSql(sql);
}
/////////////////////////////////////////
//
...
//
/////////////////////////////////////////
void crInsertGameStoreDistillRecord::buildUpdateSql(int playerid, int gameid, int money, int totaladdmoney, JXJStoreItemBagVec & cardVec, JXJStoreItemBagVec & itemVec)
{
	m_sqlstream = new crStreamBuf;
	m_sqlstream->createBuf(2048);
	char sql[512];
	sprintf(sql, "insert into gamestoredistillrecord(playerid,gameid,cyclecount,money,totaladdmoney,card0,card1,card2,card3,card4,card5,card6,card7,card8,card9,item0,item1,item2,item3,item4,item5,item6,item7,item8,item9,createdate) values(%d,%d,0,%d,%d,\0", playerid, gameid, money, totaladdmoney);
	m_sqlstream->write(sql, strlen(sql));
...
level,troopslv,consortiaid,issmallacc,killnum,newerstep,\
deadtroopsnum,giftgold,totalrecharge,troopsnum,exploit,updatedate) values(%d,%d,'%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,NOW())\0",
		playerid,shili,name.c_str(),lv,troopsLv,consortiaID,ismallacc,killNum,newerStep,deadTroopsNum,giftGold,totalRecharge,troopsnum,exploit);
	setUpdateSql(sql);
}
...
level = %d,troopslv = %d,consortiaid = %d,issmallacc = %d,killnum = %d,newerstep = %d,\
deadtroopsnum = %d,giftgold = %d,totalrecharge = %d,troopsnum = %d,exploit = %d,updatedate = now() where playerid = %d\0",
		shili,name.c_str(),lv,troopsLv,consortiaID,ismallacc,killNum,newerStep,deadTroopsNum,giftGold,totalRecharge,troopsnum,exploit,playerid);
	setUpdateSql(sql);
...
{
	char sql[512];
	sprintf(sql,"select playerid,shili,name,level,troopslv,consortiaid,issmallacc,killnum,newerstep,deadtroopsnum ,giftgold ,totalrecharge, troopsnum, exploit from peoplemap order by id limit %d,%d\0",start,count);
	setQuerySql(sql);
	setNumColumn(14);
}
