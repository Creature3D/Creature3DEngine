...
void sendReward(std::vector<CRCore::crVector4i>& awardVec,crPlayerGameData* pPlayer,const std::string &command)
{
	...
	for (int i = 0; i < awardVec.size(); ++i)
	{
		...
		{
		case T_Food:
			pPlayer->doEvent(WCH_JXJRecvFood,MAKEINT64(award[3],NULL));
			break;
		case T_Wood:
			pPlayer->doEvent(WCH_JXJRecvWood,MAKEINT64(award[3],NULL));
			break;
		case T_Iron:
			pPlayer->doEvent(WCH_JXJRecvIron,MAKEINT64(award[3],NULL));
			break;
		case T_Horse:
			pPlayer->doEvent(WCH_JXJRecvHorse,MAKEINT64(award[3],NULL));
			break;
		case T_Copper:
			pPlayer->doEvent(WCH_JXJRecvCoppercash,MAKEINT64(award[3],NULL));
			break;
		case T_Exp:
			if(pMainRole)
				pMainRole->doEvent(WCH_JXJRecvExperience,MAKEINT64(award[3],NULL));
			break;
		case T_Achievement:
			if(pMainRole)
				pMainRole->doEvent(WCH_JXJRecvAchievement,MAKEINT64(award[3],NULL));
			break;
		case T_Giftgold:
			{
				...
				pPlayer->doEvent(WCH_JXJRecvGiftgold,MAKEINT64(&moneydata,NULL));
			}
			break;
		case T_Exploit:
			if(pMainRole)
				pMainRole->doEvent(WCH_JXJRecvExploit,MAKEINT64(award[3],NULL));
			break;
		case T_Contribute:
			pPlayer->doEvent(WCH_JXJRecvCountryContribute,MAKEINT64(award[3],NULL));
			break;
		case T_JiangHun:
			pPlayer->doEvent(WCH_JXJRecvCardPieces,MAKEINT64(award[3],NULL));
			break;
		case T_Zhanquan:
			pPlayer->doEvent(WCH_JXJRecvZhengZanQuan,MAKEINT64(award[3],NULL));
			break;
		case T_VIPExp:
			pPlayer->doEvent(WCH_JXJAddVipExp,MAKEINT64(award[3],NULL));
			break;
		...
		case T_HorseExtraProduce:
			{
				...
				pPlayer->doEvent(WCH_JXJAddExtraProduce,MAKEINT64(&vec3i,NULL));
			}
			break;
		case T_Item:
			{
		...
		case T_Troops:
			{
				...
				pPlayer->doEvent(WCH_JXJAddTroops,MAKEINT64(&vec2i,NULL));
			}
			break;
		default:
			break;
	...
	if (!items.empty())
	{
		pPlayer->doEvent(WCH_JXJRecvRewardItems,MAKEINT64(&items,0));
	}
		...
		if(m_netType == GameServer)
		{
			...
			if(data && pRoleData)
			{
				...
				if (success == 1)
				{
					...
					pRoleData->excHandle(MAKEINT64(WCH_LockData,1));
					...
					pRoleData->excHandle(MAKEINT64(WCH_LockData,0));
	...
	if(m_this && m_stream.valid())
	{
		...
		if(m_netType == GameServer)
		{
			...
			{				
				...
				*mycount += ceil(total);
				...
				//增加活跃度
				m_this->doEvent(WCH_JXJRecvHuoyuedu, MAKEINT64(HYD_UpgradeCard, 1));
		...
		else if(m_netType == GameClient_Game)
		{
			...
			*myCount = serverCount;
			...
			crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKEINT64(T_Food+index,gainCount));
...
void crJXJNewGuideTrainningCopyMethod::operator()(crHandle &handle)
{
	...
	if(player && pPlayerData)
	{
		...
		pPlayerData->excHandle(MAKEINT64(WCH_LockData,1));
		...
		{
			player->doEvent(WCH_JXJAdvanceTask,MAKEINT64(m_this->getTaskID(),0));
		}

		pPlayerData->excHandle(MAKEINT64(WCH_LockData,0));
...
void crJXJCheckPresentSoldierMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_lastlv = LOINT64(param64);
			m_nowlv = HIINT64(param64);
		...
		if (praiselvvec.size() > 0)
		{
			...
			for (int i=0; i<praiselvvec.size(); i++)
			{
				if(presentSoldierTab.valid() && presentSoldierTab->queryOneRecord(0,crArgumentParser::appItoa(praiselvvec[i]),record) >= 0 )
				{
					...
					if (!items.empty())
					{
						m_this->doEvent(WCH_JXJRecvRewardItems,MAKEINT64(&items,0));
					}
