...
void crJXJUpdateJKIconMethod::operator()(crHandle &handle)
{
	...
	if(role1)
	{
		...
		unsigned int guisestate = GS_Normal;
		role1->doEvent(MAKEINT64(WCH_GetGuiseState,0),MAKEINT64(&guisestate,NULL));
		roledata->getParam(WCHDATA_RTHP,param);
		float rthp = *(float*)param;
			...
			if (itemtab->queryOneRecord(0,crArgumentParser::appItoa(equips[0]),itemrecord1)>=0)
			{
				...
				if(itemChild && itemChild->isLoaded())
				{
					...
					if(skillData)
					{
						...
						unsigned char datatype = *(unsigned char*)param;
						if(datatype == DT_Attack)
							role1->doEvent(MAKEINT64(WCH_GetSkillCD,0),MAKEINT64(&cdreduce,NULL));
						else
							role1->doEvent(MAKEINT64(WCH_GetSkillCD,1),MAKEINT64(&cdreduce,NULL));
						if(cdreduce<=-1.0f)
							cdreduce = -0.99f; 
						cdtime = cdtime/(1.0f+cdreduce);
						//cdtime += protectTime;
	...
	if(role2)
	{
		...
		unsigned int guisestate = GS_Normal;
		role2->doEvent(MAKEINT64(WCH_GetGuiseState,0),MAKEINT64(&guisestate,NULL));
		roledata->getParam(WCHDATA_RTHP,param);
		float rthp = *(float*)param;
			...
			if (itemtab->queryOneRecord(0,crArgumentParser::appItoa(equips[0]),itemrecord1)>=0)
			{
				...
				if(itemChild && itemChild->isLoaded())
				{
					...
					if(skillData)
					{
						...
						unsigned char datatype = *(unsigned char*)param;
						if(datatype == DT_Attack)
							role2->doEvent(MAKEINT64(WCH_GetSkillCD,0),MAKEINT64(&cdreduce,NULL));
						else
							role2->doEvent(MAKEINT64(WCH_GetSkillCD,1),MAKEINT64(&cdreduce,NULL));
						if(cdreduce<=-1.0f)
							cdreduce = -0.99f; 
						cdtime = cdtime/(1.0f+cdreduce);
						//cdtime += protectTime;
	...
	if(canvasData)
	{
		...
		if(selectItem)
		{
			...
			if(roleData)
			{
				...
				perHp = *(int *)param;
				selectItem->doEvent(MAKEINT64(WCH_GetHP,NULL),MAKEINT64(&maxhp,NULL));	
				roleData->getParam(WCHDATA_RTHP,param);
				rthp = *(float*)param;
				if(armyCount>1)
				{
				...
				selectItem->doEvent(MAKEINT64(WCH_GetDamage,NULL),MAKEINT64(&outParam,NULL));
				...
				selectItem->doEvent(MAKEINT64(WCH_GetDefend,NULL),MAKEINT64(&outParam,NULL));
				...
				if(aTipCanvas.valid())
				{
					...
					if(attackMap && !attackMap->empty())
					{
						...
						if(attackitem && attackitem->isLoaded())
						{
							...
							unsigned char datatype = *(unsigned char*)param;
							if(datatype == DT_Attack)
								selectItem->doEvent(MAKEINT64(WCH_GetSkillCD,0),MAKEINT64(&cdreduce,NULL));
							else
								selectItem->doEvent(MAKEINT64(WCH_GetSkillCD,1),MAKEINT64(&cdreduce,NULL));
							if(cdreduce<=-1.0f)
								cdreduce = -0.99f; 
							cdtime = cdtime/(1.0f+cdreduce);
					...
					if (itemid != 0)
					{
						...
						if(itemChild && itemChild->isLoaded())
						{
							...
							selectItem->doEvent(MAKEINT64(WCH_GetAttackDist, NULL), MAKEINT64(&outParam, NULL));
					...
					selectItem->doEvent(MAKEINT64(WCH_GetCrit,NULL),MAKEINT64(&outParam,NULL));
				...
				if(dTipCanvas.valid())
				{
					...
					selectItem->doEvent(MAKEINT64(WCH_GetDodge,NULL),MAKEINT64(&outParam,NULL));
					...
					selectItem->doEvent(MAKEINT64(WCH_GetParry,NULL),MAKEINT64(&outParam,NULL));
					...
					selectItem->doEvent(MAKEINT64(WCH_GetDefend,1),MAKEINT64(&outParam,NULL));
...
// 	selectItem->doEvent(MAKEINT64(WCH_GetSpeed,NULL),MAKEINT64(&outParam,NULL));
...
void crJXJMyArmyInfoShowMethod::operator ()(crHandle &handle)
{
	...
	if(camera.valid())
	{
		...
		if (isRole1Picked)
		{
			...
			maxhp = 0.0f;
			//roledata->excHandle(MAKEINT64(WCH_LockData,1));
			role1->doEvent(MAKEINT64(WCH_GetHP,NULL),MAKEINT64(&maxhp,NULL));
			//roledata->excHandle(MAKEINT64(WCH_LockData,0));
			roledata->getParam(WCHDATA_JXJArmyCount,param);
			armyCount = *(/*unsigned short*/int *)param;
			roledata->getParam(WCHDATA_JXJRTArmyCount,param);
		...
		if (isRole2Picked)
		{
			...
			maxhp = 0.0f;
			//roledata->excHandle(MAKEINT64(WCH_LockData,1));
			role2->doEvent(MAKEINT64(WCH_GetHP,NULL),MAKEINT64(&maxhp,NULL));
			//roledata->excHandle(MAKEINT64(WCH_LockData,0));
			roledata->getParam(WCHDATA_JXJArmyCount,param);
			armyCount = *(/*unsigned short*/int *)param;
			roledata->getParam(WCHDATA_JXJRTArmyCount,param);
...
void crJXJCardInfoShowMethod::operator ()(crHandle &handle)
{
	...
	if(m_isShow && isvisiable)
	{
		...
		ref_ptr<crStaticTextWidgetNode>  cardName = dynamic_cast<crStaticTextWidgetNode *>(canvas->getWidget(m_name));
		ref_ptr<crStaticTextWidgetNode>  skillName = dynamic_cast<crStaticTextWidgetNode *>(canvas->getWidget(m_SkillName));
		ref_ptr<crHypertextWidgetNode>  skillInfo = dynamic_cast<crHypertextWidgetNode *>(canvas->getWidget(m_information));
		...
		float ylength = canvas->getBoundBox().yMax() - canvas->getBoundBox().yMin();
		float xlength = canvas->getBoundBox().xMax() - canvas->getBoundBox().xMin();
		const crBoundingBox &canvasbox = canvas->getBoundBox();
		const crBoundingBox &bbox = m_this->getBoundBox();
		canvas->setPosition(crVector3f(bbox.center().x()+xlength/2+m_offsetx,bbox.yMax()+ylength+m_offsety,0.0f));
		...
		if(equips[m_equipIndex]>0)
		{
			if(itemtab->queryOneRecord(0,crArgumentParser::appItoa(equips[m_equipIndex]),itemrecord)>=0)
			{
				...
				if(skillInfo.valid())
				{
					...
					me->getPlayerGameData()->doEvent(WCH_JXJCreateItemTips,MAKEINT64(&item,&outscript));
					skillInfo->setHypertext(outscript);
				}
	...
	crFilterRenderManager::getInstance()->showCanvas(canvas.get(),m_isShow);		
}
					...
					else
					{
						if(vipLevel > 1)
						{
							...
							while (itr!=armyinfo.end())
							{
								...
								myPlayer->doEvent(WCH_JXJRecruitCheck,MAKEINT64(itr->first,&recruitCheck));
...
void crJXJRecvBattleAddArmyMethod::operator()(crHandle &handle)
{
	if(m_this && m_stream.valid())
	{
		...
		if(m_netType == GameServer)
		{
			...
			//如果不够执行军师扩展
			int code = 0;
			m_this->doEvent(MAKEINT64(WCH_JXJVIPExtend,WCH_JXJRecvAddArmy),MAKEINT64(&code,NULL));
				...
				//if (isHaveBirthpoint)
				{
					...
					else
					{
						if (vipLevel > 1)
						{
							...
							while (itr != addArmyInfo.end())
							{
								...
								RecruitCheckResult recruitCheck;
								m_this->doEvent(WCH_JXJRecruitCheck, MAKEINT64((*itr)[1], &recruitCheck));
					...
					if (_case >= 0)
					{
						...
						else if (_case == 3)
						{
							...
							//m_this->doEvent(WCH_JXJAddVipExp,MAKEINT64(goldneed * goldToVipExpFactor,NULL));
							//发包
							stream->createBuf(2+addArmyInfo.size()*8);
							stream->_writeChar(_case);
							stream->_writeChar(addArmyInfo.size());
...
void crJXJMouseOnRoleDecalUpdateMethod::operator()(crHandle &handle)
{
	...
	for (;itr_item!=mouseOnRoleVec.end();itr_item++)
	{
		...
		if(targetItem)
		{
			...
			me->doEvent(WCH_EnemyCheck,MAKEINT64(targetItem,&isEnemy));
			if(targetItem)
			{
...
// // 							m_this->doEvent(WCH_JXJSkillEffectRange, MAKEINT64(&centerPos, &fscope));
// 						}
					}
				}
			}
	...
	if(preScopeDecalNode && scope>0 && rolePreUseSkill && preScopeDecalNode->getVisiable())
	{
		...
		if(callback)
		{
			...
			else
			{
				...
				m_this->doEvent(WCH_JXJSkillEffectRange, MAKEINT64(&pos, &vec));
			}
		}
	}
}
...
void crJXJRecvPlayerRewardInfoMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_playerRewardData = (crPlayerRewardData *)(LOINT64(param64));
		}
		else
		{
			m_playerRewardData = NULL;
...
void crJXJRecvPlayerRewardInfoMethod::operator ()(crHandle &handle)
{
	if(m_this && m_this->getMainRole() && m_this->getMainRole()->getDataClass() && m_playerRewardData.valid())
	{
		...
		if(m_playerRewardData->getCooperCashReward()!=0)
		{
			...
			//data->excHandle(MAKEINT64(WCH_LockData,0));
			m_this->doEvent(WCH_JXJRecvCoppercash,MAKEINT64(m_playerRewardData->getCooperCashReward(),NULL));
		}
		if(m_playerRewardData->getExperience()!=0)
		{
			mainRole->doEvent(WCH_JXJRecvExperience,MAKEINT64(m_playerRewardData->getExperience(),NULL));
		}
		if(m_playerRewardData->getExploit()!=0)
		{
			mainRole->doEvent(WCH_JXJRecvExploit,MAKEINT64(m_playerRewardData->getExploit(),NULL));
		}
		if (m_playerRewardData->getScore()!=0)
		{
			m_this->doEvent(WCH_JXJRecvGuoZhanJiFen, MAKEINT64(m_playerRewardData->getScore(), NULL));
		}
		if(m_playerRewardData->getAchievement()!=0)
		{
			mainRole->doEvent(WCH_JXJRecvAchievement,MAKEINT64(m_playerRewardData->getAchievement(),NULL));
		}
		if(m_playerRewardData->getFood()!=0)
		{
			m_this->doEvent(WCH_JXJRecvFood,MAKEINT64(m_playerRewardData->getFood(),NULL));
		}
		if(m_playerRewardData->getWood()!=0)
		{
			m_this->doEvent(WCH_JXJRecvWood,MAKEINT64(m_playerRewardData->getWood(),NULL));
		}
		if(m_playerRewardData->getIron()!=0)
		{
			m_this->doEvent(WCH_JXJRecvIron,MAKEINT64(m_playerRewardData->getIron(),NULL));
		}
		if(m_playerRewardData->getHorse()!=0)
		{
			m_this->doEvent(WCH_JXJRecvHorse,MAKEINT64(m_playerRewardData->getHorse(),NULL));
		}
		...
		{
			m_this->doEvent(WCH_JXJRecvCountryContribute,MAKEINT64(m_playerRewardData->getCountryContribution(),NULL));
		}
		if(!m_playerRewardData->getEquipsVec().empty())
		{
			RewardEquipsVec& rewardEquipsVec = m_playerRewardData->getEquipsVec();
			m_this->doEvent(WCH_JXJRecvRewardEquips,MAKEINT64(&rewardEquipsVec,NULL));
		}
		...
		{
			m_this->doEvent(WCH_JXJRecvHuoyuedu, MAKEINT64(HYD_BattleGround, 1));
		}
...
void crJXJCreateRewardMailMethod::operator()(crHandle &handle)
{
	if(m_this!=NULL && m_playerRewardData!=NULL)
	{
		...
		std::string key;
		m_this->doEvent(WCH_JXJGainGiftsPackKey,MAKEINT64(&key,NULL));
		...
		//////////////////////////////////////////////////////////////////////////写邮件
		m_this->doEvent(WCH_JXJWriteRewardMail,MAKEINT64(m_playerRewardData.get(),NULL));
	}
}
...
void crJXJWriteRewardMailMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_giftsPack = (crPlayerRewardData *)(LOINT64(param64));
		}
		else
		{
			m_giftsPack = NULL;
		...
		if(maindata)
		{
			...
			if(playerData.valid() && playerData->getPlayerGameData() && playerData->getPlayerGameData()->getDataClass())
			{
				...
				if(lvchange)
				{
					...
					playerData->getPlayerGameData()->doEvent(WCH_JXJCheckPresentSoldier,MAKEINT64((int)lastlv,(int)(*mylv)));
					//if(getvipexp>0)
		...
		if(m_netType == GameClient_Game)
		{
			...
			if(m_this->getMainRole() && m_this->getMainRole()->getDataClass())
			{
				...
				*experience = exp;
				data->excHandle(MAKEINT64(WCH_LockData,0));
				if(extraexp!=0)
					crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKEINT64(T_Exp,extraexp));
			}
		}
	}
}
...
void crJXJClientRecvExploitMethod::operator()(crHandle &handle)
{
	if(m_this && m_stream.valid())
	{
		...
		if(m_netType == GameClient_Game)
		{
			...
			if(extra!=0)
				crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKEINT64(T_Exploit,extra));
		}
	}
}
...
void crJXJClientRecvAchievementMethod::operator()(crHandle &handle)
{
	...
	{
		void *param;
		if(m_netType == GameClient_Game && m_this->getMainRole() && m_this->getMainRole()->getDataClass())
		{
			...
			if(data)
			{
				...
				*achievement= temp_achievement;
				data->excHandle(MAKEINT64(WCH_LockData,0));
				if(extra!=0)
					crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKEINT64(T_Achievement,extra));
			}
		}
	}
}
...
void crJXJRecvRewardItemsMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_rewardItems = (RewardItemVec *)(LOINT64(param64));
			_crInt32 hi = HIINT64(param64);
			m_type = LOINT32(hi);
			m_cycle = HIINT32(hi);
		}
		else
		...
		if(data)
		{
			...
			if(!newitembagvec.empty())
			{
				if(packetRemainSize >= newitembagvec.size())
				{
					...
					{
						m_this->doEvent(WCH_JXJPutItemInBag,MAKEINT64(itr_newitem->get(),MAKEINT32(0,m_cycle)));
						itr_newitem++;
					}
				...
				else
				{
					...
					crServerBrainHandle::getInstance()->doEvent(WCH_JXJCreateRewardMail,MAKEINT64(temp_RewardInfo.get(),NULL));
			...
			if(!newcardbagvec.empty())
			{
				if(cardpacketRemainSize >= newcardbagvec.size())
				{
					...
					{
						m_this->doEvent(WCH_JXJPutItemInBag,MAKEINT64(itr_newcard->get(),MAKEINT32(0,m_cycle)));
						itr_newcard++;
					}
	...
	if(m_this && m_stream.valid())
	{
		if(m_netType == GameClient_Game)
		{
			...
			if(data)
			{
				...
				//while (itr_newitem!=newitembagvec.end())
				//{
				//	m_this->doEvent(WCH_JXJPutItemInBag,MAKEINT64(itr_newitem->get(),NULL));
				//	itr_newitem++;
				//}
				//JXJItemBagVec::iterator itr_newcard = newcardbagvec.begin();
				//while (itr_newcard!=newcardbagvec.end())
				//{
				//	m_this->doEvent(WCH_JXJPutItemInBag,MAKEINT64(itr_newcard->get(),NULL));
				//	itr_newcard++;
				//}
				crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeItems,MAKEINT64(&newitembagvec,&newcardbagvec));
			}
		}
	}
}
...
void crJXJRecvRewardEquipsMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_rewardEquips = (RewardEquipsVec *)(LOINT64(param64));
		}
		else
		{
			m_rewardEquips = NULL;
...
{///如果归属发生变化，重置城防值
	...
	{//城池被攻下
		...
		//刷新国家所属州
		m_this->doEvent(WCH_JXJCountryCantonMapRefresh,MAKEINT64(itr->first,itr->second->getChengzhuShili()));
			...
			if (foundCountryCityTab.valid())
			{
					...
					,shiliIdx,crArgumentParser::appItoa(winshili),record)>=0)
				{
					...
					if (order < 8 && !(winCity & (1 << order)))
					{
						...
						{
							// 建国完成
							m_this->doEvent(WCH_JXJGenerateCountryOfficial,MAKEINT64(true,winshili));
						}
					}
				}
			}
	...
	//判断轮回是否开始
	m_this->doEvent(WCH_JXJCheckGameServerCycleStart,MAKEINT16(cityLose,NULL));
}
//////////////////////////////////////////////////////////////////////////
//
...
void crJXJCardPushSelectRoleMethod::operator()(crHandle &handle)
{
	...
	if(role && camera.valid())
	{
		...
		camera->doEvent(WCH_CameraFocus,MAKEINT64(&pos,NULL));
	}
}
...
// 						}
		...
		{
			crMyPlayerData::getInstance()->getPlayerGameData()->doEvent(WCH_JXJRecvGameChat,MAKEINT64(m_stream.get(),NULL));
		}
	}
}
/////////////////////////////////////////
...
void crJXJClientRecvDamageMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_damage = LOINT64(param64);
			m_type = (unsigned char)(LOINT32(HIINT64(param64)));
			m_ifire = (char)(HIINT32(HIINT64(param64)));
		}
		else
		{
			m_damage = 0;
...
{
	int type = 0;
	crBrain::getInstance()->doEvent(WCH_JXJCheckItemType,MAKEINT64(m_itemid,&type));
	if(type == IT_Jiangka)
	{
...
// 		int bossCount = 0;
		...
		if (fubenid == 3010)
		{
			m_this->doEvent(WCH_JXJDisposeNewHandlGuoZhan,MAKEINT64(playerid,(int)fubenid));
			return;
		}
		...
		if(fubentab->queryOneRecord(0,crArgumentParser::appItoa(fubenid),record)>=0)
		{
			if (atoi(record[chaptureIdx].c_str()) == 1) // 军团副本结算
			{
				...
				if (win)
				{
					m_this->doEvent(WCH_JXJJunTuanFuBenSettle,MAKEINT64(&fuBenPlayerInfo,&circle));
					return;
				}
			}
		}
...
// 		}
		...
		if(!recordVec.empty())
		{
			...
			if(win)
				m_this->doEvent(WCH_JXJFubenDropItems,MAKEINT64(&fubenid,&(reward_info->getFubenDropVec())));
			int _type = 0;
			for (;itr_player!=playerMap.end();itr_player++)
			{
				...
				if(playerData.valid() && playerData->getPlayerGameData() && playerData->getPlayerGameData()->getDataClass())
				{
					...
					//playerGameData->doEvent(WCH_JXJFubenDropItems,MAKEINT64(&fubenid,&fubenDropVec));
					...
					if(data)
					{
						...
						if(win)
						{
							...
							{
								_type = 0;	//征战天下活动
								playerGameData->doEvent(WCH_JXJUpdate7DayHuoDong,MAKEINT64(_type,NULL));
				...
				{///玩家不在线，随机一个发邮件
					...
					m_this->doEvent(WCH_JXJCreateRewardMail,MAKEINT64(reward_info_copy.get(),NULL));
				}
				///游戏日志
				//std::string logdata = crArgumentParser::appItoa(win);
				//GameLogData gamelog(Log_FubenEnd,logdata);
				//crServerBrainHandle::getInstance()->doEvent(WCH_GameLog,MAKEINT64(*itr_player,&gamelog));
			}
		}
	}
}
...
void crJXJFubenDropItemsMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_fubenid = *(unsigned short *)LOINT64(param64);
			m_dropVec = (FubenDropRewardVec *)HIINT64(param64);
		}
		else
		{
			m_fubenid = 0;
	...
	if(m_this)
	{
		...
		if(playGameData)
		{
			...
			if(btSwitch.valid()) 
			{
				...
				{
					btSwitch->setActiveSwitchSet(index-1);
				}
			}
				...
				if(rewardData.valid())
				{
					...
					for (;itr!=dropItemVec.end();itr++,index++)
					{
						...
						if(doubleindex >= 2 && needGold>0)
						{
							...
							////消耗元宝增加成长值
							//int goldToVipExpFactor = atoi(crGlobalHandle::gData()->gGameGlobalValue(WCHDATA_JXJGoldToVipExpFactor, vipLevel).c_str());
							//m_this->doEvent(WCH_JXJAddVipExp,MAKEINT64(needGold * goldToVipExpFactor,NULL));
						}
						type = itr->first;
	...
	if(m_this)
	{
		...
		if(playerData)
		{
			...
			{//玩家掉线，随机一个奖励到邮箱
				...
				crServerBrainHandle::getInstance()->doEvent(WCH_JXJCreateRewardMail,MAKEINT64(reward_info_copy.get(),NULL));
				playerData->inputParam(WCHDATA_JXJFubenRewardInfo,NULL);
				////////////////
...
void crJXJFubenRushRewardMethod::operator ()(crHandle &handle)
{
	if(m_this)
	{
		...
		crServerBrainHandle::getInstance()->doEvent(WCH_JXJFubenDropItems,MAKEINT64(&m_fubenID,&(reward_info->getFubenDropVec())));
...
void crJXJRecvFubenReliveMethod::operator ()(crHandle &handle)
{
	if(m_this && m_stream.valid())
	{
			...
			if(playerData.valid())
			{
				...
				if(scene && room && !roleMap.empty())
				{
					...
					if(roomplayer)
					{
						...
						room->doEvent(MAKEINT64(WCH_JXJPlayerRelive,WCH_Server),MAKEINT64(m_this,NULL));
					}
				}
			}
		}
		else if(m_netType == GameClient_Game)
		{
			...
			crRoom *room = crMyPlayerData::getInstance()->getSelectedRoom();
			if(room)
				room->doEvent(MAKEINT64(WCH_JXJPlayerRelive,WCH_Client),MAKEINT64(m_this,NULL));
		}
	}
}
	...
	if(m_this)
	{
		...
		if(m_netType == SceneServer)
		{
			...
			if(playerData.valid())
			{
				...
				if(scene && room)
				{
					room->doEvent(WCH_JXJFubenReliveCheck,MAKEINT64(playerData.get(),NULL));
				}
			}
		}
		else if(m_netType == GameClient_Game)
					...
					if (role)
					{
						...
						if (roleData)
						{
								...
								++itr_extra)
							{
								...
								if (bufferAttrTab->queryOneRecord(0, extraValue, record) >= 0 && timeIndex >= 0)
								{
									buffertime = atof(record[timeIndex].c_str());
									if (buffertime == 0.0f) buffertime = FLT_MAX;//一直显示

									BufferInfoMap::iterator itr_Buffer = bufferInfoMap[i].find(itr_extra->first);
									if (itr_Buffer != bufferInfoMap[i].end())
									{
										tempBufferInfoMap.insert(std::make_pair(itr_extra->first, std::make_pair(secondValue, itr_Buffer->second.second)));
									}
									else
									{
									...
									if (overlapCount > 0 && bufferText[i][j].valid())
									{
...
void crJXJClientUserRecvSomeItemsMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_itemVec = (JXJItemBagVec *)LOINT64(param64);
			m_cardVec = (JXJItemBagVec *)HIINT64(param64);
		}
		else
		{
			m_itemVec = NULL;
...
void crJXJOnlineRewardCanvasOnShowMethod::operator()(crHandle &handle)
{
	if(m_this)
	{
		...
		{
			me->getPlayerGameData()->getDataClass()->excHandle(MAKEINT64(WCH_LockData,1));
			...
			if(count<=0)
				btGet->setEnable(false);
			me->getPlayerGameData()->getDataClass()->excHandle(MAKEINT64(WCH_LockData,0));
		}
	}
}
		...
		if(m_netType == GameServer)
		{
			...
			if(gsBrainData)
			{
				...
				else
				{
					...
					crServerBrainHandle::getInstance()->doEvent(WCH_JXJSystermSendMailToPlayer,NULL);
				}			
...
void crJXJSystermSendMailToPlayerMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_systermMailInfo = (crJXJSystermMailInfo *)(LOINT64(param64));
		}
		else
		{
			m_systermMailInfo = NULL;
				...
				if (!itemDeq->empty())
				{
					...
					if(bagItem.valid())
					{
						...
						if(itemType==T_Item)
						{
							...
							crBrain::getInstance()->doEvent(WCH_JXJCheckItemType,MAKEINT64(itemID,&itemType));
							...
							if(itemType == IT_Jiangka)
							{
								...
								if(cpImageNode.valid())
								{
									...
									if (itemtab.valid())
									{
										...
										imageCanvasData->excHandle(MAKEINT64(WCH_LockData,1));
										imageBox = dynamic_cast<crImageBoxWidgetNode *>(cpImageNode->getWidget(m_imageJK));
										...
										imageCanvasData->excHandle(MAKEINT64(WCH_LockData,0));
									}
								}
							}
							else
...
void crJXJIsGetSelectRolesSpeedMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_isCheckSelectRoles = HIINT64(param64) == 0;
		}
		break;
	}
}
...
void crJXJGetSelectRolesSpeedMethod::operator()(crHandle &handle)
{
	if(crRunGameHandle::getInstance()->isInGame() && m_output)
	{
		...
		m_this->doEvent(MAKEINT64(WCH_GetSpeed,NULL),MAKEINT64(&thisSpeed,1));
			...
			if(cameraData)
			{
				...
				if(isRoleInVec)
				{
					for (SelectMyRoleVec::iterator itr_item = selectMyRoles->begin();itr_item!=selectMyRoles->end();itr_item++)
					{
						...
						(*itr_item)->doEvent(MAKEINT64(WCH_GetSpeed,NULL),MAKEINT64(&itemSpeed,1));
...
void crJXJSystermNoticeCreateMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_chatInfo = (crJXJChatMessageInfo *)(LOINT64(param64));
		}
		else
		{
			m_chatInfo = NULL;
	...
	if(chatInfo.valid())
	{
		...
		if (mapitr != consortiamap->end())
		{
			...
			if (condata)
			{
				...
				condata->excHandle(MAKEINT64(WCH_LockData,1));
				...
				condata->excHandle(MAKEINT64(WCH_LockData,0));
			}
		}
		data->excHandle(MAKEINT64(WCH_LockData,0));
	}
				...
				if(netDataManager && vipTab.valid())
				{
					...
					if(playerGameServerPlayerData.valid())
					{
						...
						if(playerGameData.valid() && playerGameData->getDataClass())
						{
							...
							playerData->excHandle(MAKEINT16(WCH_LockData,1));
							playerData->getParam(WCHDATA_JXJVipLv,param);
							vipLevel = (unsigned char*)param;
							...
							if(vipTab->queryOneRecord(0,crArgumentParser::appItoa(*vipLevel+1),record)>=0)
							{
								if(expIndex>=0)
								{
									...
									if(*vipExp >= needvipExp)
									{
										...
										m_this->doEvent(WCH_VIPChange,MAKEINT64(&vipchange,&success));
										//受VIP等级影响的参数、、、
							...
							playerData->excHandle(MAKEINT16(WCH_LockData,0));
						}
					}
				}
				///游戏日志
		...
		if(gameServer)
		{
			...
			if(netDataManager)
			{
				if(m_this && m_this->getDataClass())
				{
					...
					{
						m_this->doEvent(WCH_JXJRecvHuoyuedu, MAKEINT64(HYD_Vip, addviplv));
					}
				}
			}
		}
				...
				if(netDataManager)
				{
					...
					if(playerGameServerPlayerData.valid() && roomPlayerGameServerPlayerData.valid() && roomPlayerGameServerPlayerData->getRoomID()!=0)
					{
						...
						if(playerGameData.valid() && playerGameData->getDataClass() && playerGameData->getMainRole() && playerGameData->getMainRole()->getDataClass())
						{
							...
							playerData->excHandle(MAKEINT16(WCH_LockData,1));
							playerMainRoleData->excHandle(MAKEINT16(WCH_LockData,1));
...
// 							unsigned short playerBF = *(unsigned short*)param;
							...
							playerGameData->getMainRole()->doEvent(WCH_JXJGetTongShuai,MAKEINT64(&tongshuai,NULL));
							...
							//{
								...
								//		if(itr->get()->getRoleID()==itr_role->first)
								//		{
								//			itr_role->second->doEvent(WCH_JXJGetRoleFightingPower,MAKEINT64(&output,itr->get()));
								//			fightPower+=output;
							...
							playerMainRoleData->excHandle(MAKEINT16(WCH_LockData,0));
							playerData->excHandle(MAKEINT16(WCH_LockData,0));							
		...
		if(canvasData)
		{
			...
			if (curSelect->getItemtype() == crInstanceItem::Role)
			{
				...
				if (itr_info != infoMap->end())
				{
					...
					curSelect->doEvent(WCH_JXJGetRoleFightingPower,MAKEINT64(&output,NULL));
...
void crJXJBattleLordEquipTipInfoMethod::operator()(crHandle &handle)
{
	...
	ref_ptr<crCanvasNode>  infocanvas = crFilterRenderManager::getInstance()->findCanvas(m_infocanvas);
	crPlayerGameData *playerData = crMyPlayerData::getInstance()->getPlayerGameData();
	if(tipsCanvas.valid() && playerData && infocanvas.valid() && canvas.valid())
	{
		...
		if(data && canvasData && chengchiTab.valid())
		{
			...
			else
			{
				...
				if(chengchiTab->queryOneRecord(0,crArgumentParser::appItoa(chengchiId),record)>=0)
				{
					...
					{
						//提示军令不足
						gameData->doEvent(WCH_JXJUIShowTipsCanvas,MAKEINT64(2009,NULL));
					}
				}
			}
		}
							...
							if(itr_chengchi!=(*rebelCityMap)[shiliID].end())
							{
								if(*freeCount>0)
								{
									...
									if(chengchiTab->queryOneRecord(0,crArgumentParser::appItoa((*itr_info)->getChengChiID()),record)>=0)
									{
										...
										playerGameData->doEvent(WCH_JXJRecvPlayerRewardInfo,MAKEINT64(reward_info.get(),NULL));
									}
								}
		...
		if(playerData)
		{
			...
			if(canInspireCount>0 && inspireLv<canInspireCount )
			{
				...
				else if (goldingot >= inspireCost)
				{
					...
					me->getPlayerGameData()->doEvent(WCH_JXJUIShowTipsCanvas,MAKEINT64(7033,NULL));//鼓舞令和元宝都不足，鼓舞失败
					//me->getPlayerGameData()->doEvent(WCH_JXJUIShowTipsCanvas,MAKEINT64(3005,NULL));//鼓舞失败
			}
			else
			{
				me->getPlayerGameData()->doEvent(WCH_JXJUIShowTipsCanvas,MAKEINT64(3006,NULL));//鼓舞失败
			}
		}
	}
	if(bCanInspire)
						...
						{//itemId不为0时才表示 使用道具进行鼓舞，否则是元宝鼓舞
								...
								++itemitr)
							{
								if (itemitr->get())
								{
									...
									{
										itemCount = (int)itemitr->get()->getItemCount();
										if (inspireCost <= itemCount)
										{
											itemitr->get()->setItemCount(itemCount-inspireCost);
										}
										else
							...
							//crBrain::getInstance()->doEvent(WCH_JXJClientUserRecvSomeValue,MAKEINT64((int)itemId,-1));
...
void crJXJGetRoleFightingPowerMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_fightingPower = (float *)LOINT64(param64);
...
void crJXJGetRoleFightingPowerMethod::operator ()(crHandle &handle)
{
	...
	if(m_this && itemTab.valid())
	{
		...
		if(itemData && factorTab.valid())
		{
			...
			float outParam = 0.0f;
			//itemData->excHandle(MAKEINT64(WCH_LockData,1));//防止阻塞
			m_this->doEvent(MAKEINT64(WCH_GetCrit,NULL),MAKEINT64(&outParam,NULL));
			fightingPower+= outParam*factorMap[FP_Crit];
			m_this->doEvent(MAKEINT64(WCH_GetDamage,NULL),MAKEINT64(&outParam,NULL));
			fightingPower+= outParam*factorMap[FP_Attack];
			m_this->doEvent(MAKEINT64(WCH_GetDefend,NULL),MAKEINT64(&outParam,NULL));
			fightingPower+= outParam*factorMap[FP_Defend];
			m_this->doEvent(MAKEINT64(WCH_GetHP,NULL),MAKEINT64(&outParam,NULL));
			fightingPower += outParam *factorMap[FP_HP];
			m_this->doEvent(MAKEINT64(WCH_GetSpeed,NULL),MAKEINT64(&outParam,NULL));
			fightingPower += outParam *factorMap[FP_MoveSpeed];
			m_this->doEvent(MAKEINT64(WCH_GetDodge,NULL),MAKEINT64(&outParam,NULL));
			fightingPower += outParam * factorMap[FP_Dodge];
			m_this->doEvent(MAKEINT64(WCH_GetParry,NULL),MAKEINT64(&outParam,NULL));
			fightingPower += outParam * factorMap[FP_Parry];
			m_this->doEvent(MAKEINT64(WCH_GetSkillCD,1),MAKEINT64(&outParam,NULL));
			...
			if(role_data_table->queryOneRecord(0, crArgumentParser::appItoa(abstractid), recordData) >= 0)
			{
				...
				if(role_skill_table->queryOneRecord(0, attackRange2, recordSkill) >= 0)
				{
					...
					//{
						...
						outParam = 0.0f;
						m_this->doEvent(MAKEINT64(WCH_GetAttackDist,NULL),MAKEINT64(&outParam,NULL));
					...
					m_this->doEvent(MAKEINT64(WCH_GetSkillCD,0),MAKEINT64(&cdreduce,NULL));
					if(cdreduce<=-1.0f)
						cdreduce = -0.99f; 
					cdtime = cdtime/(1.0f+cdreduce);
			...
			//m_this->doEvent(MAKEINT64(WCH_GetSkillCD,0),MAKEINT64(&outParam,NULL));
			//fightingPower += outParam * factorMap[FP_AttackSpeed];

			//m_this->doEvent(MAKEINT64(WCH_GetAttackDist, NULL), MAKEINT64(&outParam, NULL));
			//fightingPower += outParam * factorMap[FP_AttackDistance];

			m_this->doEvent(MAKEINT64(WCH_GetDefend,1),MAKEINT64(&outParam,NULL));
...
void crJXJGetPlayerFightingPowerMethod::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_fightingPower = (int *)(LOINT64(param64));
		}
		else
		{
			m_fightingPower = NULL;
		...
		if(playerData)
		{
			...
			for(crMyPlayerData::MyRoleNpcMap::iterator itr_role = roleMap.begin();itr_role!=roleMap.end();itr_role++,index++)
			{
				for(FormationInfoVec::iterator itr = myteam->begin();itr!=myteam->end();itr++)
				{
					if(itr->get()->getRoleID()==itr_role->first)
					{
						itr_role->second.first->doEvent(WCH_JXJGetRoleFightingPower,MAKEINT64(&output,itr->get()));
						fightPower += output;
						//fightPower+=output * itr->get()->getCount();
						//equips = itr->get()->getEquips();
						//if(equips[0]>0)
			...
			*m_fightingPower = ceil(fightPower);
			playerData->excHandle(MAKEINT64(WCH_LockData,0));
		}
	}
}
