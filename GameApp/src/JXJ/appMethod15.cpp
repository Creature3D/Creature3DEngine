...
void crJXJGetStartedDaysMethod::operator()(crHandle &handle)
{
	if(m_output)
	{
		...
		*m_output = (nowTime_t - startTime_t) / 86400; //开服天数
	}
}
			...
			if (data)
			{
				...
				if (10000 != fubenid)
				{
					if(chengchiTab->queryOneRecord(0,crArgumentParser::appItoa(fubenid),record) >= 0 )
					{
						...
						sceneid = atoi(record[chengchiTab->getTitleIndex("sceneid")].c_str());
					}
				}
				else
				{
					...
					if (extraId != 0)
					{
							...
							++itr )
						{
							...
							if(param)
							{
								...
								(*itr)->doEvent(WCH_ExtraHandle,MAKEINT64(MAKEINT32(extraId,-1),NULL));
	...
	if(m_stream.valid() && m_this->getDataClass())
	{
		...
		if(fireType == crInstanceItem::Role)
		{
			...
			if(role.valid() && role->getDataClass())
			{
				...
				ref_ptr<crRoom> roomofplayer =  netCallback->findRoom(m_this->getRoomID());
							...
							if (extraId != 0)
							{
								...
								if(param)
								{
									...
									if(old_extraId !=0 && extraIDMap->find(old_extraId)!=extraIDMap->end())
									{
										...
										scene->sendPacketToItemNeighbor(role.get(),packet);
									}
									extraData->excHandle(MAKEINT64(WCH_LockData,0));

									role->doEvent(WCH_ExtraHandle,MAKEINT64(MAKEINT32(extraId,-1),NULL));
...
void crJXJReliveItemsFromTabExMethod::operator()(crHandle &handle)
{
	...
	data->excHandle(MAKEINT64(WCH_LockData,0));
	m_this->doEvent(WCH_JXJReliveItemsFromTab,MAKEINT64(_case, boshu));
}
/////////////////////////////////////////////////////
//
...
crJXJPlayerAutoBattleMethod::crJXJPlayerAutoBattleMethod(const crJXJPlayerAutoBattleMethod& handle):
	...
	m_cancleCameraLock(handle.m_cancleCameraLock)
{
}
		...
		if(m_netType == GameServer)
		{
			...
			if (playerData.valid() && gsBrainData && gameServer)
			{
				...
				if (pdata.valid())
				{
					...
					unsigned short startDays = (nowTime_t - startTime_t) / 86400;//开服天数
					if (startedTimer <= 0)
					{
						startedTimer = 0;
					}
