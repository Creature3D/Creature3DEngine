...
void crJXJMousePickLogic::operator()(crHandle &handle)
{
	...
	if(lastTargetItem)
		lastTargetItem->doEvent(WCH_UnTouch,MAKEINT64(m_ea,m_this));
	...
	if(m_targetType & Target_Instance || m_targetType & Target_StaticNpc/* || m_targetType & Target_StaticItem*//* || m_targetType & Target_Self*/)
	{
		...
		if(targetItem)
		{
			targetItem->doEvent(WCH_Touch,MAKEINT64(m_ea,NULL));
		}
	}
...
void crJXJBattleLMousePickLogic::operator()(crHandle &handle)
{
	...
	camdir[2] = 0.0f;
	camdir.normalize();
	if((m_targetType & Target_Coord) && ((attackToPos == Cam_MoveToPos)||(attackToPos == Cam_Patrol) || (attackToPos == Cam_AttackToPos)))
	{
...
//////////////////////////////////////////////////////////////////////////信号
	if(attackToPos == Cam_Signal )
	{
		...
		m_this->doEvent(WCH_JXJPlaySignalFx,MAKEINT64(&m_targetPosition,NULL));
...
// 			data->inputParam(WCHDATA_ItemState,&itemstate);
	...
	do 
	{
		...
		if(role || !selectMyRoles->empty())
		{
			...
			char isEnemy = 0;
			if(targetItem)
				role->doEvent(WCH_EnemyCheck,MAKEINT64(targetItem,&isEnemy));
			...
			else if((attackToPos == Cam_AttackToPos) || (attackToPos == Cam_MoveToPos))
			{
				crRole *myRole;
				isattack = true;
				int targetid = 0;
			...
			else if(attackToPos == Cam_Signal)
			{
			}
		}
		if(targetItem)
											...
											if(!hits.empty())
											{
												...
												{//buf描述
													if (!bufRec[effectIndex].empty())
													{
														...
														camera->getAttachedNode()->doEvent(WCH_JXJSkillEffectRange, MAKEINT64(&pickPoint, &vec));
													}
												}
											...
											else
											{
												...
												else
												{
													...
													if (skilltab->queryOneRecord(0, crArgumentParser::appItoa(item->getAbstractItemID()), skillrecord) >= 0)
													{
														...
														camera->getAttachedNode()->doEvent(WCH_JXJSkillEffectRectangle, MAKEINT64(volumeNode.get(), &targettype));
													}
...
void crJXJNpcAILogic::operator()(crHandle &handle) 
{
	...
	if(targetItem.valid())
	{
		...
		else
		{
			...
			targetItem->doEvent(MAKEINT64(WCH_GetGuiseState,0),MAKEINT64(&targetGuisestate,NULL));
...
void crJXJPlayerAILogic::operator()(crHandle &handle) 
{
	...
	else if(itemstate == IS_Defend || itemstate == IS_Move || itemstate == IS_MoveToPos)
	{
		m_itemstate_rec = IS_Stop;
	}
	if(m_itemstate_rec == IS_AttackToPos)
	{
		if(itemstate == IS_AttackToPos)
		{
	...
	else if(itemstate==IS_AttackToPos)
	{
		...
		m_itemstate_rec = IS_AttackToPos;
	}
	...
	if(m_time>m_interval)
	{
		...
		{//这几种状态不可攻击
			...
			thisData->inputParam(WCHDATA_TargetType,&targetType);
			m_aiTargetTimer = 0.0f;
			if(m_patrolHandle.valid() && !(guisestate & GS_Garrison) && !(guisestate & GS_Fetter) && !(guisestate & GS_Swoon) && !(guisestate & GS_Stagnate) && !(guisestate & GS_Taunt) && !(guisestate & GS_ExtraShifting) && !(guisestate & GS_Confusion) )
			{
				if(m_itemstate_rec == IS_AttackToPos && itemstate == IS_Stop)
				{
		...
		if(m_aiTargetItem.valid())
		{
			...
			else
			{
				...
				{
					unsigned int targetGuisestate = GS_Normal;
					m_aiTargetItem->doEvent(MAKEINT64(WCH_GetGuiseState,0),MAKEINT64(&targetGuisestate,NULL));
					if( (targetGuisestate & GS_Hide && !(targetGuisestate & GS_UnHide)) || targetGuisestate & GS_Stagnate || targetGuisestate & GS_NoAttack ||
						targetGuisestate & GS_UnVisiable || targetGuisestate & GS_StaticUnVisiable ||
		...
		do
		{
			...
			//			//		if(weapon)
			//			//			weapon->doEvent(WCH_AdjustBulletSpeed,MAKEINT64(m_this,NULL));
			//			//	}
			//			//}
			//			bool isAboutToUseItem = false;
			//			m_useItemAndSendHandle->inputParam(1,m_this);
...
void crJXJPreLoadDownloadTaskLogic::inputParam(int i, void *param)
{
	...
	case 2:
		...
		{
			_crInt64 param64 = *(_crInt64*)param;
			m_file = *(std::string *)(LOINT64(param64));
		}
		break;
	}
}
...
void crJXJJianTaRangeDecalCreateLogic::operator()(crHandle &handle)
{
	...
	if(me && m_this->getRelNode() && camera.valid())
	{
		...
		if(isEnemy == -1)
		{
			...
			ref_ptr<crTableIO>skilldataAttrTab = crGlobalHandle::gData()->gGlobalTable(WCHDATA_SkillDataAttrTab);
			if(m_decalNode && dataAttrTab.valid() && skilldataAttrTab.valid())
			{
				...
				if(skillRangeMatrix)
				{
					...
					else
					{
						...
						float fscopey =bbox.yLength();
						float fscope = (float)range * crGlobalHandle::gData()->gUnitScale();
						skillRangeMatrix->setMatrix(crMatrix::scale(fscope/10.0f,fscope/10.0f,1.0f)*crMatrix::translate(crVector3f(pos[0],pos[1],pos[2]+m_zoffset)));
					}
				}
			}
		}
