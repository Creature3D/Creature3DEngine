...
{
	HANDLE hMutex = CreateMutex(NULL,TRUE,L"JXJLancher");
	if(GetLastError()==ERROR_ALREADY_EXISTS)
	{
		MessageBoxA(::GetDesktopWindow(),"程序已经在运行中，不能重复启动！","将星诀",MB_OK);
		CloseHandle(hMutex);
		return FALSE;
	}
	...
	if (2 == __argc)
	{
		//if(IDOK == MessageBox(NULL,L"是否确定卸载将星诀？",L"",MB_OKCANCEL))
		//{
			CUninstallDlg dlg;
			m_pMainWnd = &dlg;
			dlg.DoModal();
