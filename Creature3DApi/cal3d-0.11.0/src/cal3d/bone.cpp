...
CalBone::CalBone(CalCoreBone* coreBone)
  : m_pSkeleton(0),
  m_externTransSetted(false),
  m_externRotationSetted(false)
{
  assert(coreBone);
...
void CalBone::calculateState()
{
  ...
  if(parentId == -1)
  {
	...
	m_translationAbsolute = m_externTransSetted?m_translationExtern + m_translation:m_translation;
	m_rotationAbsolute = m_externRotationSetted?m_rotationExtern * m_rotation:m_rotation;
  }
  else
  {
    // get the parent bone
...
//****************************************************************************//
void CalBone::setRotationExtern(const CalQuaternion& rotation)//add by 吴财华
{
	m_rotationExtern = rotation;
	m_externRotationSetted = true;
}
void CalBone::setTranslationExtern(const CalVector& translation)//add by 吴财华
{
	m_translationExtern = translation;
	m_externTransSetted = true;
}