...
void main(void)
{
	...
	uv = gl_MultiTexCoord0.xy;//(vec2( gl_Position.x, - gl_Position.y ) + vec2( 1.0 ) ) * 0.5;
}
...
//const vec3 pSphere[10] = vec3[](vec3(-0.010735935, 0.01647018, 0.0062425877),vec3(-0.06533369, 0.3647007, -0.13746321),vec3(-0.6539235, -0.016726388, -0.53000957),vec3(0.40958285, 0.0052428036, -0.5591124),vec3(-0.1465366, 0.09899267, 0.15571679),vec3(-0.44122112, -0.5458797, 0.04912532),vec3(0.03755566, -0.10961345, -0.33040273),vec3(0.019100213, 0.29652783, 0.066237666),vec3(0.8765323, 0.011236004, 0.28265962),vec3(0.29264435, -0.40794238, 0.15964167));
   ...
   vec4 currentPixelSample = texture2D(CRE_NormalDepth,uv);
   ...
   vec3 norm = normalize(currentPixelSample.xyz * 2.0 - 1.0);//currentPixelSample.xyz;
   ...
   for(int i=0; i<SAMPLES;++i)
   {
      ...
      vec4 occluderFragment = texture2D(CRE_NormalDepth,se.xy);
      
      // get the normal of the occluder fragment
      occNorm = normalize(occluderFragment.xyz * 2.0 - 1.0);
      
      // if depthDifference is negative = occluder is behind current fragment
      depthDifference = currentPixelDepth-occluderFragment.a;
      
      // calculate the difference between the normals as a weight
      //normDiff = (2.0-dot(occNorm,norm))/2.0; // used to be 1.0 - 
      normDiff = (1.0-dot(occNorm,norm)); // used to be 1.0 - 
      // the falloff equation, starts at falloff and is kind of 1/x^2 falling 
      if(dot(occNorm,norm) > 0.0)
          bl += step(falloff,depthDifference)*normDiff*(1.0-smoothstep(falloff,strength,depthDifference));
   }
   
   // output the result
   float ao = 1.0-totStrength*bl*invSamples;
   gl_FragColor = texture2D(CRE_LDR,uv) * ao;
