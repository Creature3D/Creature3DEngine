...
CRCore::ref_ptr<crDefaultFont> crDefaultFont::m_instance = NULL;
crDefaultFont::crDefaultFont()
{
    m_minFilterHint = CRCore::crTexture::LINEAR/*_MIPMAP_LINEAR*/;
    m_magFilterHint = CRCore::crTexture::LINEAR/*NEAREST*/;
    constructGlyphs();
}
...
crDefaultFont* crDefaultFont::instance()
{
	if (m_instance==NULL) 
	{
		m_instance = new crDefaultFont;
		crBrain::getInstance()->pushInstance(m_instance.get());
	}
	return m_instance.get();
}

void crDefaultFont::clear()
{
	releaseObjects();
	m_instance = NULL;
}

CRText::crGlyph* crDefaultFont::getGlyph(const FontResolution& fontRes, unsigned int charcode)
{
    if (m_sizeGlyphMap.empty()) return 0;

    FontSizeGlyphMap::iterator itr = m_sizeGlyphMap.find(fontRes);
    if (itr==m_sizeGlyphMap.end())
    {
        // no font found of correct size, will need to find the nearest.
        itr = m_sizeGlyphMap.begin();
...
CRCore::crVector2 crDefaultFont::getKerning(unsigned int,unsigned int, KerningType)
{
    // no kerning on default font.
    return CRCore::crVector2(0.0f,0.0f);
}

bool crDefaultFont::hasVertical() const
{
    return true;
}

void crDefaultFont::constructGlyphs()
{
   static GLubyte rasters[][12] = { // ascii symbols 32-127, small font
    ...
    {
        CRCore::ref_ptr<crGlyph> glyph = new crGlyph(this, i);
        ...
        glyph->setHorizontalBearing(CRCore::crVector2(0.0f,0.0f)); // bottom left.
        glyph->setHorizontalAdvance(1.0f);
        glyph->setVerticalBearing(CRCore::crVector2(0.5f,1.0f)); // top middle.
