...
struct look_ahead_iterator
{
    ...
    bool valid() const { return m_index<m_string.length(); }
    
    look_ahead_iterator& operator ++ ()
    {
        if (m_index<m_string.length()) ++m_index; 
        return *this;
    }
    ...
    {
        if (m_index<m_string.length()) m_index = CRCore::minimum((unsigned int)(m_index+offset),(unsigned int)m_string.length());
        return *this;
    }
    
    unsigned char operator * () const
    {
        if (m_index<m_string.length()) return m_string[m_index];
        else return m_nullCharacter;
    }

    unsigned char operator [] (unsigned int offset) const
    {

        if (m_index+offset<m_string.length()) return m_string[m_index+offset];
        else return m_nullCharacter;
    }
...
crString::Encoding findEncoding(look_ahead_iterator& charString,crString::Encoding overrideEncoding)
{
    switch (charString[0])
    {
        ...
        case 0xFE: // big-endian 16-bit
        {
            ...
            {
                charString+=2;
                return crString::ENCODING_UTF16_BE;
            }
            break;
        }
        case 0xFF: // little-endian
        {
            ...
            if (charString[1]==0xFE)
            {
                ...
                if ((charString[2]==0) && (charString[3]==0) && (overrideEncoding != crString::ENCODING_UTF16)) //32-bit
                {
                    charString+=4;
                    return crString::ENCODING_UTF32_LE;
                }
                else //16-bit
                {
                    charString+=2;
                    return crString::ENCODING_UTF16_LE;
                }
            }
            break;
        }
        case 0x00: // 32-bit big-endian
        {
            ...
            {
                charString+=4;
                return crString::ENCODING_UTF32_BE;
            }
            break;
        }
    }
    return crString::ENCODING_ASCII;
}


unsigned int getNextCharacter(look_ahead_iterator& charString,crString::Encoding encoding)
{
    // For more info on unicode encodings see: 
    // http://www-106.ibm.com/developerworks/unicode/library/u-encode.html
    switch(encoding)
    {
        case crString::ENCODING_ASCII:
        {
            return *charString++;
        }
        case crString::ENCODING_UTF8:
        {
        ...
        case crString::ENCODING_UTF16_BE:
        {
            int char0 = *charString++;
            int char1 = *charString++;
        ...
        case crString::ENCODING_UTF16_LE:
        {
            int char1 = *charString++;
            int char0 = *charString++;
            if ((char0<=0xD7) || (char0>=0xE0)) // simple character
        ...
        case crString::ENCODING_UTF32_BE:
        {
        ...
        case crString::ENCODING_UTF32_LE:
        {
        ...
        default:
        {
            ...
            CRCore::notify(CRCore::FATAL)<<"Error: Invalid string encoding"<<std::endl;
            break;
        }
    }
    return 0;
...
////////////////////////////////////////////////////////////////////////////
//
// crString implemention.
//

crString::crString(const crString& str):
    vector_type(str)
{
}

crString& crString::operator = (const crString& str)
{
...
void crString::set(const std::string& text)
{
    clear();
...
void crString::set(const wchar_t* text)
{
    clear();
    while(*text)
    {
...
void crString::set(const std::string& text,Encoding encoding)
{
    clear();
...
std::string crString::createUTF8EncodedString() const
{
