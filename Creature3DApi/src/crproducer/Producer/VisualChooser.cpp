...
#include <CRProducer/Producer/VisualChooser>
...
VisualChooser::VisualChooser( void )
{
	...
	_numMultiSamples = 0;//add by wch
	_vsync = false;//add by wch
}

VisualChooser::~VisualChooser(void)
{
   clear();
   //std::cerr << "~VisualChooser()" << std::endl;
}
...
void VisualChooser::setNumMultiSamples( unsigned int num )//add by wch
{
	_numMultiSamples = num;
}

void VisualChooser::setVsync( bool vsync )//add by wch
{
	_vsync = vsync;
}
...
VisualInfo *VisualChooser::choose( Display *dpy, int screen, bool strict_adherence)
{
	if(_vinfo != NULL)
	{
	  ...
	  GLint val;
	  if(aglDescribePixelFormat(*_vinfo, AGL_NO_RECOVERY, &val)) return _vinfo;
	}
	else 
	{
    ...
    if (failed || !ext || !ext->isSupported(WGLExtensions::ARB_pixel_format))
    {
        ...
        do
        {
            ...
            for (std::vector<VisualAttribute>::const_iterator i=_visual_attributes.begin(); i!=_visual_attributes.end(); ++i)
            {
                ...
                if (i->attribute() == DoubleBuffer)      pfd.dwFlags |= PFD_DOUBLEBUFFER;// | PFD_SWAP_COPY;//PFD_SWAP_EXCHANGE;
