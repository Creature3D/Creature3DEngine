...
//
	...
	{ 
		...
		//aopts[0]->fccHandler=mmioFOURCC('x','v','i','d');
		//aopts[0]->dwFlags=AVICOMPRESSF_KEYFRAMES|AVICOMPRESSF_VALID;//|AVICOMPRESSF_DATARATE;
		//aopts[0]->dwKeyFrameEvery=1;
		...
		if (hr != AVIERR_OK) 
		{
			...
			case AVIERR_NOCOMPRESSOR:
				CRCore::notify(CRCore::FATAL)<<"AVIMakeCompressedStream error: A suitable compressor cannot be found "<<std::endl;
				break;
			case AVIERR_MEMORY:
				CRCore::notify(CRCore::FATAL)<<"AVIMakeCompressedStream error: There is not enough memory to complete the operation. "<<std::endl;
				break;
			case AVIERR_UNSUPPORTED:
				CRCore::notify(CRCore::FATAL)<<"AVIMakeCompressedStream error: Compression is not supported for this type of data. This error might be returned if you try to compress data that is not audio or video. "<<std::endl;
			    break;
			default:
				CRCore::notify(CRCore::FATAL)<<"AVIMakeCompressedStream error: unknown error "<<hr<<std::endl;
				break;
			}
			au->iserr=true; 
			return hr;
...
{ 
	...
	{ 
		...
		strhdr.fccType = streamtypeVIDEO;// stream type
		strhdr.fccHandler = 0; 
		//strhdr.fccHandler = mmioFOURCC('M','P','G','4'); 
		strhdr.dwScale = 1;
		strhdr.dwRate = au->period;
		strhdr.dwSuggestedBufferSize  = dibs.dsBmih.biSizeImage;
		SetRect(&strhdr.rcFrame, 0, 0, dibs.dsBmih.biWidth, dibs.dsBmih.biHeight);
		...
		if (hr!=AVIERR_OK) 
		{
			CRCore::notify(CRCore::FATAL)<<"AVIFileCreateStream = "<<hr<<std::endl;

			au->iserr=true; 
			return hr;
		}

		//hr = AVIStreamSetFormat(au->ps, 0, &dibs.dsBmih, dibs.dsBmih.biSize+dibs.dsBmih.biClrUsed*sizeof(RGBQUAD));
		//if (hr!=AVIERR_OK) 
		//{
		//	CRCore::notify(CRCore::FATAL)<<"AVIStreamSetFormat = "<<hr<<std::endl;
		//	au->iserr=true; 
		//	return hr;
		//}
	}
	...
	{ 
		...
		if (hr != AVIERR_OK) 
		{
			CRCore::notify(CRCore::FATAL)<<"AVIMakeCompressedStream = "<<hr<<std::endl;
			au->iserr=true; 
			return hr;
		}
		hr = AVIStreamSetFormat(au->psCompressed, 0, &dibs.dsBmih, dibs.dsBmih.biSize+dibs.dsBmih.biClrUsed*sizeof(RGBQUAD));
	...
	if (hr!=AVIERR_OK) 
	{
		CRCore::notify(CRCore::FATAL)<<"AVIStreamWrite = "<<hr<<std::endl;
		au->iserr=true; 
		return hr;
	}
	au->nframe++; 
