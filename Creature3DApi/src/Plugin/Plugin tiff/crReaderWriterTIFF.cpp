...
class ReaderWriterTIFF : public CRIOManager::crReaderWriter
{
    public:
        virtual const char* className() const { return "TIFF crImage Reader"; }
        virtual bool acceptsExtension(const std::string& extension) const
        { 
            if( CRIOManager::equalCaseInsensitive(extension,"tiff")) return true;
            if( CRIOManager::equalCaseInsensitive(extension,"tif") ) return true;
            return false;
        }
        ...
        ReadResult readTIFStream(std::istream& fin) const
        {
            ...
            if (imageData==NULL) 
            {
                ...
                CRCore::notify(CRCore::WARN) << err_msg << std::endl;
        ...
        WriteResult::WriteStatus writeTIFStream(std::ostream& fout, const CRCore::crImage& img) const
        {
...
// reader/writer.
static CRIOManager::RegisterReaderWriterProxy<ReaderWriterTIFF> g_readerWriter_TIFF_Proxy;
