...
//#include <CRCore/ObjectRecordData.h>
...
FltExportVisitor::writeHeader( const std::string& headerName )
{
    ...
    _dos.writeInt16( 0 ); // next crLod id
...
//
//  Convenience routine for writing crGroup nodes that aren't animated
//
void
FltExportVisitor::writeGroup( const CRCore::crGroup& group )
{
...
//
//  Since OpenFlight doesn't have 'crSequence' records---just crGroup records that
//  may, optionally, be animated---this routine sets the animation-related
//  parameters for a crGroup record and simply forwards to writeGroup()
//
void
FltExportVisitor::writeSequence( const CRCore::crSequence& sequence )
{
    ...
    if (mode == CRCore::crSequence::SWING)
    {
        flags |= SWING_ANIM;
    }
    ...
    lastFrameDuration = 0;//sequence.getLastFrameTime();
...
void
FltExportVisitor::writeObject( const CRCore::crGroup& group/*, CRCore::ObjectRecordData* ord*/ )
{
    uint16 length( 28 );
    IdHelper id(*this, group.getName() );
	...
	_records->writeInt32( 0 );
	_records->writeInt16( 0 );
	_records->writeUInt16( 255 );
	_records->writeUInt16( 0 );
	_records->writeUInt16( 0 );
	_records->writeUInt16( 0 );
...
void
FltExportVisitor::writeDegreeOfFreedom( const CRCore::crDOFTransform* dof )
{
    const CRCore::crMatrix& invPut = dof->getInversePutMatrix();

    // Origin of DOF coord sys
    CRCore::crVector3d origin( invPut.getTrans() );

    CRCore::crVector3 xAxis( invPut(0,0), invPut(0,1), invPut(0,2) );
    CRCore::crVector3 yAxis( invPut(1,0), invPut(1,1), invPut(1,2) );
    // Reference point along DOF coord sys's X axis
    CRCore::crVector3d pointOnXAxis = origin + xAxis;
    // Reference point in DOF coord sys's X-Y plane
    CRCore::crVector3d pointInXYPlane = origin + yAxis;

    // Translations
    CRCore::crVector3d minTranslate( dof->getMinTranslate() );
    CRCore::crVector3d maxTranslate( dof->getMaxTranslate() );
    CRCore::crVector3d currTranslate( dof->getCurrentTranslate() );
    CRCore::crVector3d incrTranslate( dof->getIncrementTranslate() );

    // Rotations
    CRCore::crVector3d minHPR( dof->getMinHPR() );
    CRCore::crVector3d maxHPR( dof->getMaxHPR() );
    CRCore::crVector3d currHPR( dof->getCurrentHPR() );
    CRCore::crVector3d incrHPR( dof->getIncrementHPR() );

    // Scaling
    CRCore::crVector3d minScale( dof->getMinScale() );
    CRCore::crVector3d maxScale( dof->getMaxScale() );
    CRCore::crVector3d currScale( dof->getCurrentScale() );
    CRCore::crVector3d incrScale( dof->getIncrementScale() );
    ...
    _records->writeFloat64( CRCore::RadiansToDegrees(minHPR[1]) );
    _records->writeFloat64( CRCore::RadiansToDegrees(maxHPR[1]) );
    _records->writeFloat64( CRCore::RadiansToDegrees(currHPR[1]) );
    _records->writeFloat64( CRCore::RadiansToDegrees(incrHPR[1]) );

    _records->writeFloat64( CRCore::RadiansToDegrees(minHPR[2]) );
    _records->writeFloat64( CRCore::RadiansToDegrees(maxHPR[2]) );
    _records->writeFloat64( CRCore::RadiansToDegrees(currHPR[2]) );
    _records->writeFloat64( CRCore::RadiansToDegrees(incrHPR[2]) );

    _records->writeFloat64( CRCore::RadiansToDegrees(minHPR[0]) );
    _records->writeFloat64( CRCore::RadiansToDegrees(maxHPR[0]) );
    _records->writeFloat64( CRCore::RadiansToDegrees(currHPR[0]) );
    _records->writeFloat64( CRCore::RadiansToDegrees(incrHPR[0]) );
...
void
FltExportVisitor::writeExternalReference( const CRCore::crProxyNode& proxy )
{
    //uint16 length( 216 );
...
void
FltExportVisitor::writeLevelOfDetail( const CRCore::crLod& lod,
                                    CRCore::crVector3d const& center,
                                    double switchInDist,
                                    double switchOutDist)
{
    ...
    _records->writeFloat64( switchOutDist );   // crSwitch-out distance
    _records->writeInt16( 0 );                 // Special Effect ID1
    _records->writeInt16( 0 );                 // Special Effect ID2
...
void
FltExportVisitor::writeLightSource( const CRCore::crLightSource& node )
{

    static const unsigned int ENABLED = 0x80000000u >> 0;
    static const unsigned int GLOBAL  = 0x80000000u >> 1;

    CRCore::crLight const* light = dynamic_cast<const CRCore::crLight *>(node.getLight());
    int index = _lightSourcePalette->add(light);

    CRCore::crVector4 const& lightPos = light->getPosition();
    CRCore::crVector3 const& lightDir = light->getDirection();

    uint32 flags = 0;
    CRCore::crStateSet const* ss = getCurrentStateSet();
    if (ss->getMode(GL_LIGHT0 + light->getLightNum() ) & CRCore::crStateAttribute::ON)
    {
        flags |= ENABLED;
    }

    // If this light is enabled for the node at the top of our crStateSet stack,
    ...
    _records->writeInt32( 0 );              // Reserved
    _records->writeVec3d( CRCore::crVector3d(
...
void
FltExportVisitor::writeSwitch( const CRCore::crMultiSwitch* ms )
{
    ...
    for (int i = 0; i < numMasks; ++i)
    {
        ...
        uint32 maskWord = 0;
        const CRCore::crMultiSwitch::ValueList& maskBits = ms->getValueList(i);
...
void
FltExportVisitor::writeSwitch( const CRCore::crSwitch* sw )
{
    // An CRCore::crSwitch is just a special case of an CRCore::crMultiSwitch
    ...
    uint32 maskWord = 0;
    const CRCore::crSwitch::ValueList& maskBits = sw->getValueList();
...
void
FltExportVisitor::writeLightPoint( const CRCore::LightPointNode* lpn )
{
    //enum Directionality
    //{
    //    OMNIDIRECTIONAL = 0,
    ...
    //// parameters, but in OpenFlight, a crLight Point Record contains a list of homogeneous
    ...
    //const CRCore::LightPoint& lp0 = lpn->getLightPoint( 0 );
    ...
    //const CRCore::DirectionalSector* ds = dynamic_cast< CRCore::DirectionalSector* >( lp0._sector.get() );
    //if (ds)
    //{
    //    directionality = UNIDIRECTIONAL;
    //    horizLobe = CRCore::RadiansToDegrees( ds->getHorizLobeAngle() );
    //    vertLobe = CRCore::RadiansToDegrees( ds->getVertLobeAngle() );
    //    lobeRoll = CRCore::RadiansToDegrees( ds->getLobeRollAngle() );
    //}
    ...
    //    _records->writeVec3f( CRCore::crVector3( 0.f, 0.f, 0.f ) ); // Axis of rotation
    //}

    //{
    //    CRCore::ref_ptr< CRCore::Vec3Array > v = new CRCore::Vec3Array( lpn->getNumLightPoints() );
    //    CRCore::ref_ptr< CRCore::Vec4Array > c = new CRCore::Vec4Array( lpn->getNumLightPoints() );
    //    CRCore::ref_ptr< CRCore::Vec3Array > n = new CRCore::Vec3Array( lpn->getNumLightPoints() );
    //    CRCore::crVector3 normal( 0.f, 0.f, 1.f );

    //    unsigned int idx;
    //    for( idx=0; idx<lpn->getNumLightPoints(); idx++)
    //    {
    //        const CRCore::LightPoint& lp = lpn->getLightPoint( idx );
    //        (*v)[ idx ] = lp._position;
    //        (*c)[ idx ] = lp._color;

    //        const CRCore::DirectionalSector* ds = dynamic_cast< CRCore::DirectionalSector* >( lp._sector.get() );
    //        if (ds)
    //            normal = ds->getDirection();
    //        (*n)[ idx ] = normal;
    //    }
    //    _vertexPalette->add( (const CRCore::Array*)NULL, v.get(), c.get(), n.get(), NULL, true, true, false );
    //}
