...
CRCore::crVector4 getColorFromPool(int index, const ColorPool* colorPool)
{
    CRCore::crVector4 color(1,1,1,1);
    if (colorPool)
...
class VertexC : public Record
{
    public:
        ...
        virtual void readRecord(RecordInputStream& in, Document& document)
        {
            ...
            CRCore::crVector4 packedColor = in.readColor32();
...
class VertexCNT : public Record
{
    ...
    protected:
        ...
        virtual void readRecord(RecordInputStream& in, Document& document)
        {
            ...
            if (!coord.valid())
            {
                CRCore::notify(CRCore::NOTICE)<<"Warning: data error detected in VertexCNT::readRecord coord="<<coord.x()<<" "<<coord.y()<<" "<<coord.z()<<std::endl;
            }

            if (!normal.valid())
            {
                CRCore::notify(CRCore::NOTICE)<<"Warning: data error detected in VertexCNT::readRecord normal="<<normal.x()<<" "<<normal.y()<<" "<<normal.z()<<std::endl;
            }

            if (!uv.valid())
            {
                CRCore::notify(CRCore::NOTICE)<<"Warning: data error detected in VertexCNT::readRecord uv="<<uv.x()<<" "<<uv.y()<<std::endl;
            }
...
class AbsoluteVertex : public Record
{
    ...
    protected:
        ...
        virtual void readRecord(RecordInputStream& in, Document& document)
        {
            ...
            // coord
            vertex.setCoord(CRCore::crVector3(x,y,z) * document.unitScale());
...
class NormalVertex : public Record
{
    ...
    protected:
        ...
        virtual void readRecord(RecordInputStream& in, Document& document)
        {
            ...
            /*uint8 shadingFlag =*/ in.readUInt8();
            int colorIndex = (int)in.readInt16();
            CRCore::crVector3 normal = in.readVec3d();
