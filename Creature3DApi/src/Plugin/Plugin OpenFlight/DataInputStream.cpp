...
DataInputStream::DataInputStream(std::streambuf* sb):
    std::istream(sb)
{
    _byteswap = CRCore::getCpuByteOrder() == CRCore::LittleEndian;
}
...
CRCore::crVector4 DataInputStream::readColor32()
{
    ...
    CRCore::crVector4 color((float)red/255,(float)green/255,(float)blue/255,(float)alpha/255);

    return color;
}


CRCore::crVector2 DataInputStream::readVec2f()
{
    float32 x = readFloat32();
    float32 y = readFloat32();

    CRCore::crVector2 vec(x,y);

    return vec;
}


CRCore::crVector3 DataInputStream::readVec3f()
{
    float32 x = readFloat32();
    float32 y = readFloat32();
    float32 z = readFloat32();

    CRCore::crVector3 vec(x,y,z);

    return vec;
}


CRCore::crVector4 DataInputStream::readVec4f()
{
    float32 x = readFloat32();
    float32 y = readFloat32();
    float32 z = readFloat32();
    ...
    CRCore::crVector4 vec(x,y,z,w);

    return vec;
}


CRCore::crVector3d DataInputStream::readVec3d()
{
    float64 x = readFloat64();
    float64 y = readFloat64();
    float64 z = readFloat64();

    CRCore::crVector3d vec(x,y,z);
