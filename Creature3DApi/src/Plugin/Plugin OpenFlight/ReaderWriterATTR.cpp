...
#include <Plugin OpenFlight/DataInputStream.h>
...
class ReaderWriterATTR : public CRIOManager::crReaderWriter
{
    public:
        ...
        virtual ReadResult readObject(const std::string& fileName, const crReaderWriter::Options*) const;
        virtual crReaderWriter::WriteResult writeObject(const CRCore::crBase& object, const std::string& fileName, const Options* options) const;
};


crReaderWriter::ReadResult ReaderWriterATTR::readObject(const std::string& file, const crReaderWriter::Options* options) const
{
    using std::ios;
...
crReaderWriter::WriteResult
ReaderWriterATTR::writeObject(const CRCore::crBase& object, const std::string& fileName, const Options* options) const
{
    using std::ios;
    ...
    if (attr == NULL)
    {
        CRCore::notify(CRCore::FATAL) << "AttrWriter: Invalid crBase." << std::endl;
...
// reader/writer.
CRIOManager::RegisterReaderWriterProxy<ReaderWriterATTR> g_readerWriter_ATTR_Proxy;
