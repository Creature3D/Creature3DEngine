...
{
    _subsurfaceDepth = new CRCore::crDepth(CRCore::crDepth::LESS, 0.0, 1.0,false);
}
...
void Document::pushExtension()
{
    ...
    {
        CRCore::notify(CRCore::WARN) << "No current primary in Document::pushExtension()." << std::endl;
        return;
    }
...
void Document::popExtension()
{
    ...
    if (!_currentPrimaryRecord.valid())
    {
        CRCore::notify(CRCore::WARN) << "Can't decide primary in Document::popExtension()." << std::endl;
        return;
    }
...
CRCore::crNode* Document::getInstanceDefinition(int no)
{
...
void Document::setSubSurfacePolygonOffset(int level, CRCore::crPolygonOffset* po)
{
    _subsurfacePolygonOffsets[level] = po;
}

CRCore::crPolygonOffset* Document::getSubSurfacePolygonOffset(int level)
{
    CRCore::notify(CRCore::DEBUG_INFO)<<"Document::getSubSurfacePolygonOffset("<<level<<")"<<std::endl;
    CRCore::ref_ptr<CRCore::crPolygonOffset>& po = _subsurfacePolygonOffsets[level];
    if (!po)
    {
        po = new CRCore::crPolygonOffset(-1.0f*float(level), -1.0f);
    }
    return po.get();
}
