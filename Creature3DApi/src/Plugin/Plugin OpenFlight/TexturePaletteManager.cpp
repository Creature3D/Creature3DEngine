...
int
TexturePaletteManager::add( int unit, const CRCore::crTexture2D* texture )
{
    if( (!texture) ||
        (!texture->getImage()) )
        return -1;
...
TexturePaletteManager::write( DataOutputStream& dos ) const
{
    ...
    {
        const CRCore::crTexture2D* texture = it->first;
        int index = it->second;

        std::string fileName;
        if ( _fltOpt.getStripTextureFilePath() )
            fileName = CRIOManager::getSimpleFileName( texture->getImage()->getFileName() );
        else
