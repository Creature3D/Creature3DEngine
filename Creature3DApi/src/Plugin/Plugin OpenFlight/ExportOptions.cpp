 ...
 *  crTexture Palette. By default, the exporter doesn't strip the path,
 *  and the name written to the crTexture Palette is taken directly from
 *  the CRCore::Image object referenced by the CRCore::crTexture2D crStateAttribute.
 */
...
ExportOptions::ExportOptions( const CRIOManager::crReaderWriter::Options* opt )
  : _version( VERSION_16_1 ),
    _units( METERS ),
    _validate( false ),
    _lightingDefault( true ),
...
ExportOptions::parseOptionsString()
{
    ...
    while (pos != str.npos)
    {
        ...
        {
            CRCore::notify(CRCore::INFO) << "fltexp: Found: " << token << std::endl;
        ...
        if ( pos == str.npos )
        {
            CRCore::notify(CRCore::WARN) << "fltexp: Bogus OptionString: " << token << std::endl;
            continue;
        }
        ...
        {
            CRCore::notify(CRCore::INFO) << "fltexp: Token: " << token << ", Value: " << value << std::endl;
            ...
            if( value == std::string( "15.7" ) )
                ...
                CRCore::notify(CRCore::WARN) << "fltexp: Unsupported version: " << value << ". Defaulting to 16.1." << std::endl;
        ...
        else if (token == _unitsOption)
        {
            ...
            if( value == std::string( "KILOMETERS" ) )
                ...
                CRCore::notify(CRCore::WARN) << "fltexp: Unsupported units: " << value << ". Defaulting to METERS." << std::endl;
        ...
        else if (token == _lightingOption)
        {
            ...
            if (value == std::string( "OFF" ) )
                ...
                CRCore::notify(CRCore::WARN) << "fltexp: Unsupported lighting value: " << value << ". Defaulting to ON." << std::endl;
