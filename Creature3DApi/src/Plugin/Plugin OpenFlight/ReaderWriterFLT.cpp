...
USE_FLTRECORD(crMatrix, MATRIX_OP)
...
USE_FLTRECORD(crBase, OBJECT_OP)
USE_FLTRECORD(crLightSource, LIGHT_SOURCE_OP)
...
class ReadExternalsVisitor : public CRCore::crNodeVisitor
{
    CRCore::ref_ptr<crReaderWriter::Options> _options;
    bool _cloneExternalReferences;

public:

    ReadExternalsVisitor(crReaderWriter::Options* options) :
    ...
    virtual void apply(crProxyNode& node)
    {
        ...
        for (unsigned int pos=0; pos<node.getNumFileNames(); pos++)
        {
            ...
            // read external
            CRCore::ref_ptr<CRCore::crNode> external = CRIOManager::readNodeFile(filename,_options.get());
            if (external.valid())
            {
                if (_cloneExternalReferences)
                    external = dynamic_cast<CRCore::crNode*>(external->clone(CRCore::crCopyOp(CRCore::crCopyOp::DEEP_COPY_NODES)));
...
<table>
  <tr>
    <th></th>
    <th align="center">crNode</th>
    <th align="center">crBase</th>
...
class FLTReaderWriter : public crReaderWriter
{
    public:
        FLTReaderWriter()
          : _implicitPath( "." )
        {
            ...
            //supportsOption( "stripTextureFilePath", "Export option: If present in the Options string, the exporter strips the path from texture file names, and writes only the texture file name to the FLT crTexture Palette. By default, the exporter doesn't strip the path, and the name written to the crTexture Palette is taken directly from the CRCore::Image object referenced by the CRCore::crTexture2D crStateAttribute." );
        }
        ...
        {
            return equalCaseInsensitive(extension,"fltw") || extension.empty();
        }
        ...
        virtual ReadResult readNode(const std::string& file, const Options* options) const
        {
            ...
            // in local cache?
            {
                CRCore::crNode* node = flt::crRegistry::instance()->getExternalFromLocalCache(fileName);
                if (node)
            ...
            local_opt->getDatabasePathList().push_front(CRIOManager::getFilePath(fileName));
            ...
            {
                // add to local cache.
                flt::crRegistry::instance()->addExternalToLocalCache(fileName,rr.getNode());
        ...
        virtual ReadResult readNode(std::istream& fin, const Options* options) const
        {
            ...
            if (options)
            {
                ...
                CRCore::notify(CRCore::DEBUG_INFO) << readerMsg << "clampToEdge=" << document.getReplaceClampWithClampToEdge() << std::endl;
                ...
                CRCore::notify(CRCore::DEBUG_INFO)  << readerMsg << "replaceDoubleSidedPolys=" << document.getReplaceDoubleSidedPolys() << std::endl;
                ...
                CRCore::notify(CRCore::DEBUG_INFO) << readerMsg << "billboardCenter=" << document.getUseBillboardCenter() << std::endl;
                ...
                CRCore::notify(CRCore::DEBUG_INFO) << readerMsg << "readObjectRecordData=" << !document.getReadObjectRecordData() << std::endl;

                document.setPreserveNonOsgAttrsAsUserData((options->getOptionString().find("preserveNonOsgAttrsAsUserData")!=std::string::npos));
                CRCore::notify(CRCore::DEBUG_INFO) << readerMsg << "preserveNonOsgAttrsAsUserData=" << document.getPreserveNonOsgAttrsAsUserData() << std::endl;
            ...
            if (!document.getPreserveFace())
            {
                ...
                optimizer.optimize(document.getHeaderNode(),
                    CRUtil::crOptimizer::DEFAULT_OPTIMIZATIONS);
            }
        ...
        virtual WriteResult writeNode( const CRCore::crNode& node, const std::string& fileName, const Options* options ) const
        {
            if ( fileName.empty() )
            {
                return WriteResult::FILE_NOT_HANDLED;
            ...
            if ( fOut.fail())
            {
                CRCore::notify(CRCore::FATAL) << "fltexp: Failed to open output stream." << std::endl;
        ...
        virtual WriteResult writeNode( const CRCore::crNode& node, std::ostream& fOut, const Options* options ) const
        {
            ...
            if (!fltOpt->getTempDir().empty())
            {
                ...
                if ( !CRIOManager::makeDirectory( fltOpt->getTempDir() ) )
                {
                    CRCore::notify(CRCore::FATAL) << "fltexp: Error creating temp dir: " << fltOpt->getTempDir() << std::endl;
                    return WriteResult::ERROR_IN_WRITING_FILE;
                }
            }
            ...
            //   must use crNode::accept() which requires 'node' to be non-const.
            //   Pretty much requires casting away const.
            CRCore::crNode* nodeNonConst = const_cast<CRCore::crNode*>( &node );
            if (!nodeNonConst)
                return WriteResult::ERROR_IN_WRITING_FILE;
