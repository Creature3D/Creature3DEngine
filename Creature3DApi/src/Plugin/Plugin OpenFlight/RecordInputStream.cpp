...
bool RecordInputStream::readRecordBody(opcode_type opcode, size_type size, Document& document)
{
    ...
    {
        CRCore::notify(CRCore::INFO) << "Little endian pop-level record" << std::endl;
        opcode=POP_LEVEL_OP;
        size=4;
    }
    ...
    Record* prototype = crRegistry::instance()->getPrototype((int)opcode);
    ...
    if (prototype)
    {
        ...
        CRCore::ref_ptr<Record> record = prototype->cloneType();
    ...
    {
        CRCore::notify(CRCore::WARN) << "Unknown record, opcode=" << opcode << " size=" << size << std::endl;

        // Add to registry so we only have to see this error message once.
        crRegistry::instance()->addPrototype(opcode,new DummyRecord);
    }
