...
/** If the DataOutputStream parameter is NULL, write to the _records
    ...
    */
void
FltExportVisitor::writeComment( const CRCore::crNode& node, DataOutputStream* dos )
{
    if (dos==NULL)
        dos = _records;
...
void
FltExportVisitor::writeMatrix( const CRCore::Referenced* ref )
{
    const CRCore::RefMatrix* rm = dynamic_cast<const CRCore::RefMatrix*>( ref );
    if (!rm)
        return;
...
{
    CRCore::notify(CRCore::DEBUG_INFO) << "fltexp: Continuation record length: " << length+4 << std::endl;
