...
{ // the main area of changes from the pic format loader.
 ...
 //   unsigned char palette[256][3];
    ...
    if (hd.FileType == MB) {
        ...
        delete [] hdr;
       // CRCore::notify(CRCore::INFO) << "loading bmp file "<<swap<<" "<<infsize<< " "<<sizeof(inf) << " "<<sizeof(bmpheader) << std::endl;
        ...
        //CRCore::notify(CRCore::INFO) << "readbmp " <<inf.width<< " "<<inf.height << std::endl;
        
        // previous size calculation, see new calcs below.
        int32 size_prev = hd.siz[1]+hd.siz[0]*65536;
        //CRCore::notify(CRCore::INFO) << "previous size calc = "<<size_prev<<"  hd.siz[1]="<<hd.siz[1]<<"  hd.siz[0]="<<hd.siz[0]<<std::endl;
        
        // order of size calculation swapped, by Christo Zietsman to fix size bug.
        int32 size = hd.siz[1]*65536+hd.siz[0];
        //CRCore::notify(CRCore::INFO) << "new size calc = "<<size<<"  hd.siz[1]="<<hd.siz[1]<<"  hd.siz[0]="<<hd.siz[0]<<std::endl;

        // handle size==0 in uncompressed 24-bit BMPs -Eric Hammil
        if (size==0) size = filelen;
       // CRCore::notify(CRCore::INFO) << "size after zero correction = "<<size<<"  hd.siz[1]="<<hd.siz[1]<<"  hd.siz[0]="<<hd.siz[0]<<std::endl;
...
class ReaderWriterBMP : public CRIOManager::crReaderWriter
{
    public:
        virtual const char* className() const { return "BMP crImage Reader"; }
        virtual bool acceptsExtension(const std::string& extension) const { return CRIOManager::equalCaseInsensitive(extension,"bmp"); }
        ...
        bool WriteBMPStream(const CRCore::crImage &img, std::ostream& fout, const std::string &fileName) const
        {
            ...
            struct BMPInfo inf;
            //CRCore::notify(CRCore::INFO) << "sizes "<<size << " "<<sizeof(inf)<< std::endl;
           ...
           // CRCore::notify(CRCore::INFO) << "save screen "<<fileName <<inf.width<< " "<<inf.height << std::endl;
          //  CRCore::notify(CRCore::INFO) << "sizes "<<size << " "<<infsize <<" "<<sizeof(inf)<< std::endl;
            ...
            switch(img.computeNumComponents(img.getPixelFormat()))
            {
                ...
                break;
                default:
                    CRCore::notify(CRCore::WARN) << "Cannot write images with other number of components than 3 or 4" << std::endl;
                break;
            }
        ...
        virtual WriteResult writeImage(const CRCore::crImage &img,const std::string& fileName, const CRIOManager::crReaderWriter::Options*) const
        {
...
// reader/writer.
static CRIOManager::RegisterReaderWriterProxy<ReaderWriterBMP> g_readerWriter_BMP_Proxy;
