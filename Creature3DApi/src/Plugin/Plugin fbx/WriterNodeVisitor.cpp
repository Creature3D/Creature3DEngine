...
#include <cassert>
#include <CRCore/CullFace>
#include <CRCore/crMatrixTransform>
#include <CRCore/crNodeVisitor>
#include <CRCore/crPrimitive>
#include <CRIOManager/FileUtils>
#include <CRIOManager/WriteFile>
...
class PrimitiveIndexWriter : public CRCore::PrimitiveIndexFunctor
{
public:
    PrimitiveIndexWriter(const CRCore::crGeometry* geo,
        ...
        _material(material),
        _curNormalIndex(0),
        _normalBinding(CRCore::crGeometry::BIND_OFF),
        _mesh(0)
    {
        _normalBinding = geo->getNormalBinding();
        if (!geo->getNormalArray() || geo->getNormalArray()->getNumElements()==0)
        {
            _normalBinding = CRCore::crGeometry::BIND_OFF;        // Turn off binding if there is no normal data
        }
        reset();
    }
    ...
    virtual void setVertexArray(unsigned int, const CRCore::Vec3d*) {}
    ...
    void writeTriangle(unsigned int i1, unsigned int i2, unsigned int i3)
    {
        ...
        triangle.t2 = i2;
        triangle.t3 = i3;
        if (_normalBinding == CRCore::crGeometry::BIND_PER_VERTEX){
...
protected:
    template <typename T> void drawElementsImplementation(GLenum mode, GLsizei count, const T* indices)
    {
        ...
        if (_normalBinding == CRCore::crGeometry::BIND_PER_PRIMITIVE_SET) ++_curNormalIndex;
    }
...
private:
    ...
    const CRCore::crGeometry* _geo;
    unsigned int         _lastFaceIndex;
    int                  _material;
    unsigned int         _curNormalIndex;
    CRCore::crGeometry::AttributeBinding _normalBinding;
    FbxMesh*            _mesh;
};
...
WriterNodeVisitor::crMaterial::crMaterial(WriterNodeVisitor& writerNodeVisitor,
    ...
    _osgImage(NULL)
{
    CRCore::crVector4 diffuse(1,1,1,1),
    ...
    {
        assert(stateset);
        diffuse = mat->getDiffuse(CRCore::crMaterial::FRONT);
        ambient = mat->getAmbient(CRCore::crMaterial::FRONT);
        specular = mat->getSpecular(CRCore::crMaterial::FRONT);
        shininess = mat->getShininess(CRCore::crMaterial::FRONT);
        emission = mat->getEmission(CRCore::crMaterial::FRONT);
        transparency = 1 - diffuse.w();

        const CRCore::StateAttribute* attribute = stateset->getAttribute(CRCore::StateAttribute::CULLFACE);
        if (attribute)
        {
            assert(dynamic_cast<const CRCore::CullFace*>(attribute));
            CRCore::CullFace::Mode mode = static_cast<const CRCore::CullFace*>(attribute)->getMode();
            if (mode == CRCore::CullFace::FRONT)
            {
                OSG_WARN << "FBX Writer: Reversed face (culled FRONT) not supported yet." << std::endl;
            }
            else if (mode != CRCore::CullFace::BACK)
            {
                assert(mode == CRCore::CullFace::FRONT_AND_BACK);
...
int WriterNodeVisitor::processStateSet(const CRCore::crStateSet* ss)
{
    //OSG_ALWAYS << "Trying Adding " << ss->getAttribute(CRCore::StateAttribute::MATERIAL)->getName() << std::endl;
    ...
    const CRCore::crMaterial* mat = dynamic_cast<const CRCore::crMaterial*>(ss->getAttribute(CRCore::StateAttribute::MATERIAL));
    const CRCore::crTexture* tex = dynamic_cast<const CRCore::crTexture*>(ss->getTextureAttribute(0, CRCore::StateAttribute::TEXTURE));

    if (mat || tex)
    {
        int matNum = _lastMaterialIndex;
        _materialMap.insert(MaterialMap::value_type(MaterialMap::key_type(ss),
            crMaterial(*this, _externalWriter, ss, mat, tex, _pSdkManager, _options, matNum)));
        ++_lastMaterialIndex;
        return matNum;
    }
    return -1;
...
void
WriterNodeVisitor::setControlPointAndNormalsAndUV(const CRCore::crObject& geo,
                                                  ...
                                                  FbxMesh*         mesh)
{
    ...
    {
        const CRCore::crGeometry* pGeometry = geo.getDrawable(it->first.drawableIndex)->asGeometry();
        unsigned int vertexIndex = it->first.vertexIndex;
        unsigned int normalIndex = it->first.normalIndex;

        const CRCore::Array * basevecs = pGeometry->getVertexArray();
        assert(basevecs);
        ...
        FbxVector4 vertex;
        if (basevecs->getType() == CRCore::Array::Vec3ArrayType)
        {
            const CRCore::crVector3  & vec = (*static_cast<const CRCore::Vec3Array  *>(basevecs))[vertexIndex];
            vertex.Set(vec.x(), vec.y(), vec.z());
        }
        else if (basevecs->getType() == CRCore::Array::Vec3dArrayType)
        {
            const CRCore::Vec3d & vec = (*static_cast<const CRCore::Vec3dArray *>(basevecs))[vertexIndex];
            vertex.Set(vec.x(), vec.y(), vec.z());
        }
        else
        {
            OSG_NOTIFY(CRCore::FATAL) << "Vertex array is not crVector3 or Vec3d. Not implemented" << std::endl;
            throw "Vertex array is not crVector3 or Vec3d. Not implemented";
            //_succeeded = false;
            //return;
        }
        ...
        const CRCore::Array * basenormals = pGeometry->getNormalArray();

        if (basenormals && basenormals->getNumElements()>0)
        {
            FbxVector4 normal;
            if (basenormals->getType() == CRCore::Array::Vec3ArrayType)
            {
                const CRCore::crVector3  & vec = (*static_cast<const CRCore::Vec3Array  *>(basenormals))[normalIndex];
                normal.Set(vec.x(), vec.y(), vec.z(), 0);
            }
            else if (basenormals->getType() == CRCore::Array::Vec3dArrayType)
            {
                const CRCore::Vec3d & vec = (*static_cast<const CRCore::Vec3dArray *>(basenormals))[normalIndex];
                normal.Set(vec.x(), vec.y(), vec.z(), 0);
            }
            else
            {
                OSG_NOTIFY(CRCore::FATAL) << "Normal array is not crVector3 or Vec3d. Not implemented" << std::endl;
                throw "Normal array is not crVector3 or Vec3d. Not implemented";
                //_succeeded = false;
                //return;
            }
        ...
        if (texcoords)
        {
            const CRCore::Array * basetexcoords = pGeometry->getTexCoordArray(0);
            if (basetexcoords && basetexcoords->getNumElements()>0)
            {
                FbxVector2 texcoord;
                if (basetexcoords->getType() == CRCore::Array::Vec2ArrayType)
                {
                    const CRCore::Vec2 & vec = (*static_cast<const CRCore::Vec2Array *>(basetexcoords))[vertexIndex];
                    texcoord.Set(vec.x(), vec.y());
                }
                else if (basetexcoords->getType() == CRCore::Array::Vec2dArrayType)
                {
                    const CRCore::Vec2d & vec = (*static_cast<const CRCore::Vec2dArray *>(basetexcoords))[vertexIndex];
                    texcoord.Set(vec.x(), vec.y());
                }
                else
                {
                    OSG_NOTIFY(CRCore::FATAL) << "crTexture coords array is not Vec2 or Vec2d. Not implemented" << std::endl;
                    throw "crTexture coords array is not Vec2 or Vec2d. Not implemented";
                    //_succeeded = false;
                    //return;
                }
...
void WriterNodeVisitor::buildFaces(const CRCore::crObject& geo,
                                   ListTriangle&     listTriangles,
                                   bool              texcoords)
{
    MapIndices index_vert;
...
void WriterNodeVisitor::createListTriangle(const CRCore::crGeometry* geo,
                                           ListTriangle&        listTriangles,
                                           bool&                texcoords,
                                           unsigned int&        drawable_n)
{
    ...
    {
        const CRCore::Array * vecs = geo->getVertexArray();
        if (vecs)
        {
            nbVertices = vecs->getNumElements();

            // crTexture coords
            const CRCore::Array * texvec = geo->getTexCoordArray(0);
            if (texvec)
            {
    ...
    {
        const CRCore::crPrimitive* ps = geo->getPrimitiveSet(iPrimSet);
        const_cast<CRCore::crPrimitive*>(ps)->accept(pif);
    }
}

void WriterNodeVisitor::apply(CRCore::crObject& node)
{
    ...
    {
        const CRCore::crGeometry* g = node.getDrawable(i)->asGeometry();
        if (g != NULL)
        {
...
void WriterNodeVisitor::apply(CRCore::crGroup& node)
{
    FbxNode* parent = _curFbxNode;
...
void WriterNodeVisitor::apply(CRCore::crMatrixTransform& node)
{
    FbxNode* parent = _curFbxNode;
    _curFbxNode = FbxNode::Create(_pSdkManager, node.getName().empty() ? "DefaultName" : node.getName().c_str());
    parent->AddChild(_curFbxNode);

    const CRCore::Matrix& matrix = node.getMatrix();
    CRCore::Vec3d pos, scl;
    CRCore::Quat rot, so;
