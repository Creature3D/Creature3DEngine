...
CRCore::Quat makeQuat(const FbxDouble3& degrees, EFbxRotationOrder fbxRotOrder)
{
    double radiansX = CRCore::DegreesToRadians(degrees[0]);
    double radiansY = CRCore::DegreesToRadians(degrees[1]);
    double radiansZ = CRCore::DegreesToRadians(degrees[2]);

    switch (fbxRotOrder)
    {
    case eEulerXYZ:
        return CRCore::Quat(
            radiansX, CRCore::Vec3d(1,0,0),
            radiansY, CRCore::Vec3d(0,1,0),
            radiansZ, CRCore::Vec3d(0,0,1));
    case eEulerXZY:
        return CRCore::Quat(
            radiansX, CRCore::Vec3d(1,0,0),
            radiansZ, CRCore::Vec3d(0,0,1),
    ...
    case eSphericXYZ:
        {
            ...
            CRCore::Quat quat;
            quat.makeRotate(CRCore::Vec3d(1.0, 0.0, 0.0), CRCore::Vec3d(degrees[0], degrees[1], degrees[2]));
            return quat;
        }
    default:
...
void makeLocalMatrix(const FbxNode* pNode, CRCore::Matrix& m)
{
                         ...
                         FbxScene& fbxScene)
{
    if (isAnimated(prop, fbxScene))
    {
        ...
        else
        {
            ...
            CRCore::crVector3 axes[3] = {CRCore::crVector3(1,0,0), CRCore::crVector3(0,1,0), CRCore::crVector3(0,0,1)};

            FbxDouble3 fbxPropValue = prop.Get();
            fbxPropValue[0] = CRCore::DegreesToRadians(fbxPropValue[0]);
            fbxPropValue[1] = CRCore::DegreesToRadians(fbxPropValue[1]);
            fbxPropValue[2] = CRCore::DegreesToRadians(fbxPropValue[2]);
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                {
                    staticTransform.preMultRotate(CRCore::Quat(fbxPropValue[j], axes[j]));
                }
            }
        }
    }
...
void readUpdateMatrixTransform(osgAnimation::UpdateMatrixTransform* pUpdate, FbxNode* pNode, FbxScene& fbxScene)
{
    ...
    FbxDouble3 fbxRotPiv = pNode->RotationPivot.Get();
    staticTransform.preMultTranslate(CRCore::Vec3d(
...
CRCore::crGroup* createGroupNode(FbxManager& pSdkManager, FbxNode* pNode,
    const std::string& animName, const CRCore::Matrix& localMatrix, bool bNeedSkeleton,
    std::map<FbxNode*, CRCore::crNode*>& nodeMap, FbxScene& fbxScene)
{
    if (bNeedSkeleton)
    {
        osgAnimation::Bone* osgBone = new osgAnimation::Bone;
        osgBone->setDataVariance(CRCore::Object::DYNAMIC);
        ...
        nodeMap.insert(std::pair<FbxNode*, CRCore::crNode*>(pNode, osgBone));
    ...
    else
    {
        ...
        {
            CRCore::crGroup* pGroup = new CRCore::crGroup;
            pGroup->setName(pNode->GetName());
            return pGroup;
        }

        CRCore::crMatrixTransform* pTransform = new CRCore::crMatrixTransform(localMatrix);
...
CRIOManager::crReaderWriter::ReadResult OsgFbxReader::readFbxNode(
    FbxNode* pNode,
    bool& bIsBone, int& nLightCount)
{
    ...
    CRCore::NodeList skeletal, children;
    ...
    for (int i = 0; i < nChildCount; ++i)
    {
        ...
        CRIOManager::crReaderWriter::ReadResult childResult = readFbxNode(
            pChildNode, bChildIsBone, nLightCount);
        if (childResult.error())
        {
            return childResult;
        }
        else if (CRCore::crNode* osgChild = childResult.getNode())
        {
            if (bChildIsBone)
            {
    ...
    case FbxNodeAttribute::eMesh:
        {
            ...
            CRIOManager::crReaderWriter::ReadResult meshRes = readFbxMesh(pNode, stateSetList);
            if (meshRes.error())
            {
                return meshRes;
            }
            else if (CRCore::crNode* node = meshRes.getNode())
            {
                bEmpty = false;
                ...
                {
                    return CRIOManager::crReaderWriter::ReadResult(node);
                }
    ...
    case FbxNodeAttribute::eLight:
        {
            CRIOManager::crReaderWriter::ReadResult res =
            ...
            else if (CRCore::crGroup* resGroup = dynamic_cast<CRCore::crGroup*>(res.getObject()))
            {
                bEmpty = false;
                if (animName.empty() &&
                    bLocalMatrixIdentity)
    ...
    for (CRCore::NodeList::iterator it = skeletal.begin(); it != skeletal.end(); ++it)
    {
        pAddChildrenTo->addChild(it->get());
    }
    for (CRCore::NodeList::iterator it = children.begin(); it != children.end(); ++it)
    {
        pAddChildrenTo->addChild(it->get());
    }


    return CRIOManager::crReaderWriter::ReadResult(osgGroup.get());
}
