...
CRCore::Quat makeQuat(const FbxDouble3&, EFbxRotationOrder);

CRCore::Quat makeQuat(const CRCore::crVector3& radians, EFbxRotationOrder fbxRotOrder)
{
    FbxDouble3 degrees(
        CRCore::RadiansToDegrees(radians.x()),
        CRCore::RadiansToDegrees(radians.y()),
        CRCore::RadiansToDegrees(radians.z()));
...
void readKeys(FbxAnimCurve* curveX, FbxAnimCurve* curveY, FbxAnimCurve* curveZ,
              ...
              std::vector<osgAnimation::TemplateKeyframe<CRCore::crVector3> >& keyFrameCntr, float scalar = 1.0f)
{
              ...
              std::vector<osgAnimation::Vec3CubicBezierKeyframe>& keyFrameCntr, float scalar = 1.0f)
{
    ...
    {
        double fTime = *it;
        CRCore::crVector3 val, cpIn, cpOut;
        for (int i = 0; i < 3; ++i)
        {
            if (curveTimeMap[i].empty()) continue;
    ...
    const char* targetName, EFbxRotationOrder rotOrder)
{
    ...
    for (KeyFrameCntr::iterator it = eulerFrameCntr.begin(), end = eulerFrameCntr.end();
        it != end; ++it)
    {
        const CRCore::crVector3& euler = it->getValue();
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            if (interpolationType == FbxAnimCurveDef::eInterpolationCubic)
            {
                ...
                for (int j = 0; j < curve->KeyGetCount(); ++j)
                {
                    ...
                    osgAnimation::FloatCubicBezier key(
                        CRCore::DegreesToRadians(angle),
                        CRCore::DegreesToRadians(angle - leftTangent.mDerivative / 3.0),
                        CRCore::DegreesToRadians(angle + rightTangent.mDerivative / 3.0));
            ...
            else
            {
                ...
                for (int j = 0; j < curve->KeyGetCount(); ++j)
                {
                    ...
                    keys->push_back(osgAnimation::FloatKeyframe(
                        ...
                        static_cast<float>(CRCore::DegreesToRadians(key.GetValue()))));
                }
