...
CRIOManager::crReaderWriter::ReadResult OsgFbxReader::readFbxLight(FbxNode* pNode, int& nLightCount)
{
    ...
    CRCore::Light* osgLight = new CRCore::Light;
    CRCore::LightSource* osgLightSource = new CRCore::LightSource;
    ...
    osgLight->setPosition(CRCore::crVector4(0,0,0,fbxLightType != FbxLight::eDirectional));
    ...
    CRCore::Vec3f osgDiffuseSpecular(1.0f, 1.0f, 1.0f);
    CRCore::Vec3f osgAmbient(0.0f, 0.0f, 0.0f);
    ...
    osgLight->setDiffuse(CRCore::Vec4f(osgDiffuseSpecular, 1.0f));
    osgLight->setSpecular(CRCore::Vec4f(osgDiffuseSpecular, 1.0f));
    osgLight->setAmbient(CRCore::Vec4f(osgAmbient, 1.0f));

    return CRIOManager::crReaderWriter::ReadResult(osgLightSource);
}
