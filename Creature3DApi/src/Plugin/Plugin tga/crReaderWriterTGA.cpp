...
class ReaderWriterTGA : public CRIOManager::crReaderWriter
{
public:
	virtual const char* className() const { return "TGA Image Format"; }
	virtual bool acceptsExtension(const std::string& extension) const { return CRIOManager::equalCaseInsensitive(extension,"tga"); }

	//ReaderWriterTGA()
	//{
	//	supportsExtension("tga","Tga crImage format");
	//}
	...
	virtual ReadResult readObject(std::istream& fin,const CRIOManager::crReaderWriter::Options* options =NULL) const
	{
		return readImage(fin, options);
	}

	virtual ReadResult readObject(const std::string& file, const CRIOManager::crReaderWriter::Options* options =NULL) const
	{
	...
	bool saveTGAStream(const CRCore::crImage& image, std::ostream& fout) const
	{
		...
		fout.put(numPerPixel * 8);  // crImage pixel size
		fout.put(0);  // crImage descriptor
	...
	virtual WriteResult writeImage(const CRCore::crImage& image, const std::string& fileName, const Options* options) const
	{
...
// reader/writer.
static CRIOManager::RegisterReaderWriterProxy<ReaderWriterTGA> g_readerWriter_TGA_Proxy;
