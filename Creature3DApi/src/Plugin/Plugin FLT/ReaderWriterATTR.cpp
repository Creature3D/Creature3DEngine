...
{    
    crTexEnv* outTexEnv = new crTexEnv;
	crTexture2D* outTexture = new crTexture2D;
    ...
    if (wrapMode_u == WRAP_CLAMP) outTexture->setWrap(crTexture2D::WRAP_S,crTexture2D::CLAMP);
    else outTexture->setWrap(crTexture2D::WRAP_S,crTexture2D::REPEAT);

    if (wrapMode_v == WRAP_CLAMP) outTexture->setWrap(crTexture2D::WRAP_T,crTexture2D::CLAMP);
    else outTexture->setWrap(crTexture2D::WRAP_T,crTexture2D::REPEAT);


    switch (texEnvMode)
    {
    case TEXENV_MODULATE:
        outTexEnv->setMode(crTexEnv::MODULATE);
        break;
    case TEXENV_BLEND:
        outTexEnv->setMode(crTexEnv::BLEND);
        break;
    case TEXENV_DECAL:
        outTexEnv->setMode(crTexEnv::DECAL);
        break;
    case TEXENV_COLOR:
        outTexEnv->setMode(crTexEnv::REPLACE);
        break;
    }

/*  An email on ATTR mappings to OpenGL : "[CRCore-news] OpenFlight Texture Filter Modes"
    from Joseph Steel:
...
*/
    ...
    case MIN_FILTER_POINT:
        outTexture->setFilter(CRCore::crTexture2D::MIN_FILTER, crTexture2D::NEAREST);
        break;
    case MIN_FILTER_BILINEAR:
		outTexture->setFilter(CRCore::crTexture2D::MIN_FILTER, crTexture2D::LINEAR);
		//outTexture->setFilter(CRCore::crTexture2D::MIN_FILTER, crTexture2D::LINEAR_MIPMAP_LINEAR);
        break;
    case MIN_FILTER_MIPMAP_POINT:
        outTexture->setFilter(CRCore::crTexture2D::MIN_FILTER, crTexture2D::NEAREST_MIPMAP_NEAREST);
        break;
    case MIN_FILTER_MIPMAP_LINEAR:
        outTexture->setFilter(CRCore::crTexture2D::MIN_FILTER, crTexture2D::NEAREST_MIPMAP_LINEAR);
        break;
    case MIN_FILTER_MIPMAP_BILINEAR:
        outTexture->setFilter(CRCore::crTexture2D::MIN_FILTER, crTexture2D::LINEAR_MIPMAP_NEAREST);
        break;
    case MIN_FILTER_MIPMAP_TRILINEAR:
        outTexture->setFilter(CRCore::crTexture2D::MIN_FILTER, crTexture2D::LINEAR_MIPMAP_LINEAR);
        break;
    case MIN_FILTER_BICUBIC://set for Not Compresse normal map
		//outTexture->setFilter(CRCore::crTexture2D::MIN_FILTER, crTexture2D::LINEAR_MIPMAP_LINEAR);
		//outTexture->setInternalFormatMode(crTexture::USE_IMAGE_DATA_FORMAT);
		outTexture->setFilter(CRCore::crTexture2D::MIN_FILTER, crTexture2D::LINEAR_MIPMAP_LINEAR);
		break;
    ...
    case MAG_FILTER_POINT:
		outTexture->setFilter(CRCore::crTexture2D::MAG_FILTER, crTexture2D::NEAREST);
        break;
    ...
    case MAG_FILTER_MODULATE_DETAIL:
        outTexture->setFilter(CRCore::crTexture2D::MAG_FILTER, crTexture2D::LINEAR);
        break;
    }
    ...
    // crStateSet to store the texture an modes, it is probably best
    ...
    // This is now a bit diferrent. We create a new AttrData object, and crStateSet object
    // is instead with attribute information needed to setup detail texutre
    //
    // Julian Ortiz, June 18th 2003.    

	attrdata->stateset = new crStateSet;
	attrdata->stateset->setTextureAttributeAndModes( 0, outTexEnv );
	attrdata->stateset->setTextureAttributeAndModes( 0, outTexture, crStateAttribute::ON );
	...
	attrdata->comment.insert(0,comments,strlen(this->comments));
	//if(strstr(comments,"max_anisotropy"))
	//{
	//	float anis;
	//    CRCore::crArgumentParser::readKeyValue(comments,"max_anisotropy",anis);
 //       outTexture->setMaxAnisotropy(anis);
	//}
	if(strstr(comments,"#TEXENV_ADD"))
	{
		outTexEnv->setMode(crTexEnv::ADD);
	}
	if(strstr(comments,"#ALPHAFUNC"))
	{
		std::string func;
		float value;
		CRCore::crArgumentParser::readKeyValue(comments,"comparisonFunction",func);
		CRCore::crArgumentParser::readKeyValue(comments,"referenceValue",value);
		CRCore::crAlphaFunc *alphaFunc = new CRCore::crAlphaFunc;
		if(func.compare("NEVER")==0)
		{
            alphaFunc->setFunction(CRCore::crAlphaFunc::NEVER,value);
		}
		else if(func.compare("LESS")==0)
		{
			alphaFunc->setFunction(CRCore::crAlphaFunc::LESS,value);
		}
		else if(func.compare("EQUAL")==0)
		{
			alphaFunc->setFunction(CRCore::crAlphaFunc::EQUAL,value);
		}
		else if(func.compare("LEQUAL")==0)
		{
			alphaFunc->setFunction(CRCore::crAlphaFunc::LEQUAL,value);
		}
		else if(func.compare("GREATER")==0)
		{
			alphaFunc->setFunction(CRCore::crAlphaFunc::GREATER,value);
		}
		else if(func.compare("NOTEQUAL")==0)
		{
			alphaFunc->setFunction(CRCore::crAlphaFunc::NOTEQUAL,value);
		}
		else if(func.compare("GEQUAL")==0)
		{
			alphaFunc->setFunction(CRCore::crAlphaFunc::GEQUAL,value);
		}
		else if(func.compare("ALWAYS")==0)
		{
			alphaFunc->setFunction(CRCore::crAlphaFunc::ALWAYS,value);
		}
		attrdata->stateset->setAttributeAndModes(alphaFunc,crStateAttribute::ON);
		attrdata->stateset->setRenderingHint(CRCore::crStateSet::ALPHATEST_BIN);
		//CRCore::notify(CRCore::WARN)<<"#ALPHAFUNC "<< std::endl;
	}
	int int1;
	if(CRCore::crArgumentParser::readKeyValue(comments,"EnableBlendShadow",int1))
	{
		outTexture->setBlendShadowEnable(int1);
	}
	if(CRCore::crArgumentParser::readKeyValue(comments,"EnableAlphaShadow",int1))
	{
		outTexture->setAlphaShadowEnable(int1);
	}

	std::string str;
	if(CRCore::crArgumentParser::readKeyValue(comments,"tex_wrap_mode",str))
	{
		if(str.compare("CLAMP_TO_EDGE") == 0)
		{
			//CRCore::notify(CRCore::WARN)<<"CLAMP_TO_BORDER "<< std::endl;
			outTexture->setWrap(crTexture2D::WRAP_S,crTexture2D::CLAMP_TO_EDGE);
			outTexture->setWrap(crTexture2D::WRAP_T,crTexture2D::CLAMP_TO_EDGE);
			outTexture->setWrap(crTexture2D::WRAP_R,crTexture2D::CLAMP_TO_EDGE);
		}
		else if(str.compare("CLAMP_TO_BORDER") == 0)
		{
			outTexture->setWrap(crTexture2D::WRAP_S,crTexture2D::CLAMP_TO_BORDER);
			outTexture->setWrap(crTexture2D::WRAP_T,crTexture2D::CLAMP_TO_BORDER);
			outTexture->setWrap(crTexture2D::WRAP_R,crTexture2D::CLAMP_TO_BORDER);
		}
		else if(str.compare("MIRROR") == 0)
		{
			outTexture->setWrap(crTexture2D::WRAP_S,crTexture2D::MIRROR);
			outTexture->setWrap(crTexture2D::WRAP_T,crTexture2D::MIRROR);
			outTexture->setWrap(crTexture2D::WRAP_R,crTexture2D::MIRROR);
		}
	}
    return attrdata;
}
...
class ReaderWriterATTR : public CRIOManager::crReaderWriter
{
    public:
        ...
        {
            return CRIOManager::equalCaseInsensitive(extension,"attr");
        }

        virtual ReadResult readObject(const std::string& file, const CRIOManager::crReaderWriter::Options*)const
        {
            ...
            const CRIOManager::crReaderWriter::Options*  rwOptions=
                CRIOManager::crRegistry::instance()->getOptions();
            ...
            //crStateSet* stateset = attr.createOsgStateSet();
            // Julian Ortiz, June 18th 2003.        
...
// reader/writer.
static CRIOManager::RegisterReaderWriterProxy<ReaderWriterATTR> g_readerWriter_ATTR_Proxy;
