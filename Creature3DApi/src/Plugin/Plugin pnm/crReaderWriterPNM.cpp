    ...
    unsigned char* read_color_ascii(FILE* fp, int width, int height)
{
    ...
    CRCore::notify(CRCore::NOTICE)<<"Width = "<<width<<std::endl;
    CRCore::notify(CRCore::NOTICE)<<"Height = "<<height<<std::endl;
    ...
    while(dst < end)
    {
        ...
        do
        {
            ...
            if (feof(fp) || ferror(fp))
            {
                CRCore::notify(CRCore::NOTICE)<<"Error 1 "<<numRead<<std::endl;
        ...
        do
        {
            ...
            if (feof(fp) || ferror(fp))
            {
                CRCore::notify(CRCore::NOTICE)<<"Error 2"<<std::endl;
    ...
    CRCore::notify(CRCore::NOTICE)<<"Completed"<<std::endl;
    ...
    unsigned char* read_grayscale_binary(FILE* fp, int width, int height)
{
    ...
    if (sizeof(T) > 1 && getCpuByteOrder() == CRCore::LittleEndian)
    {
...
class ReaderWriterPNM : public CRIOManager::crReaderWriter
{
    public:
        virtual const char* className() const { return "PNM crImage Reader/Writer"; }
        virtual bool acceptsExtension(const std::string& extension) const
        {
            return CRIOManager::equalCaseInsensitive(extension, "pnm") ||
                CRIOManager::equalCaseInsensitive(extension, "ppm") ||
                CRIOManager::equalCaseInsensitive(extension, "pgm") ||
                CRIOManager::equalCaseInsensitive(extension, "pbm");
        }
        ...
        virtual WriteResult writeImage(const CRCore::crImage& image,std::ostream& fout,const CRIOManager::crReaderWriter::Options* options) const
        {
        ...
        virtual WriteResult writeImage(const CRCore::crImage& image,const std::string& fileName, const CRIOManager::crReaderWriter::Options* options) const
        {
...
// reader/writer.
static CRIOManager::RegisterReaderWriterProxy<ReaderWriterPNM> g_readerWriter_PNM_Proxy;
