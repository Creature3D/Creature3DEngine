...
#include <CRCore/io_utils.h> // just for debugging
...
dxfCircle::drawScene(scene* sc)
{
    ...
    if (_useAccuracy) {
        ...
        newtheta=CRCore::RadiansToDegrees(newtheta)*2.0; 
    ...
    for(unsigned int r=0;r<=numsteps;r++) {
        b = a + crVector3d(_radius * (double) sin(angle1), _radius * (double) cos(angle1), 0);
        angle1 += anglestep;
        vlist.push_back(b);
    }
...
dxfArc::drawScene(scene* sc)
{
    ...
    //cout<<"arc theta="<<CRCore::RadiansToDegrees(theta)<<" end="<<end<<" start="<<start<<" numsteps="<<numsteps<<" e-s/theta="<<((end-start)/theta)<<" end-start="<<(end-start)<<endl;
...
dxfPolyline::drawScene(scene* sc)
{
    ...
    sc->ocs(m);
    std::vector<crVector3d> vlist;
    std::vector<crVector3d> qlist;
    crVector3d a, b, c, d;
    bool invert_order = false;
    if (_flag & 16) {
        std::vector<crVector3d> nlist;
        crVector3d nr;
        bool nset = false;
        //dxfVertex* v = NULL;
        unsigned int ncount;
...
dxfInsert::drawScene(scene* sc)
{
    ...
    crMatrixd back = sc->backMatrix();
    crMatrixd m;
    m.makeIdentity();
    sc->pushMatrix(m, true);
    crVector3d trans = _block->getPosition();
    sc->blockOffset(-trans);
    if (_rotation) {
        sc->pushMatrix(crMatrixd::rotate(CRCore::DegreesToRadians(_rotation), 0,0,1));
    }
    sc->pushMatrix(crMatrixd::scale(_scale.x(), _scale.y(), _scale.z()));
    sc->pushMatrix(crMatrixd::translate(_point.x(), _point.y(), _point.z()));
    getOCSMatrix(_ocs, m);
    sc->pushMatrix(m);
    sc->pushMatrix(back);
    ...
    sc->popMatrix(); // back
    sc->blockOffset(crVector3d(0,0,0));
...
{
    CRText::crText::AlignmentType align;

    crMatrixd m;
    getOCSMatrix(_ocs, m);
    sc->ocs(m);

    ref_ptr<CRText::crText> _text = new CRText::crText;
    _text->setText(_string);

    _text->setCharacterSize( _height, 1.0/_xscale );
    _text->setFont("arial.ttf");
      
    crQuat qr( DegreesToRadians(_rotation), Z_AXIS );
    
    if ( _flags & 2 ) qr = crQuat( PI, Y_AXIS ) * qr;
    if ( _flags & 4 ) qr = crQuat( PI, X_AXIS ) * qr;
    
    _text->setAxisAlignment(CRText::crText::USER_DEFINED_ROTATION);
    ...
    case 3:
        switch (_hjustify) {
        case 2:
            align = CRText::crText::RIGHT_TOP;
            break;
        case 1:
            align = CRText::crText::CENTER_TOP;
            break;
        default:
            align = CRText::crText::LEFT_TOP;
        }
        break;
    case 2:
        switch (_hjustify) {
        case 2:
            align = CRText::crText::RIGHT_CENTER;
            break;
        case 1:
            align = CRText::crText::CENTER_CENTER;
            break;
        default:
            align = CRText::crText::LEFT_CENTER;
        }
        break;
    case 1:
        switch (_hjustify) {
        case 2:
            align = CRText::crText::RIGHT_BOTTOM;
            break;
        case 1:
            align = CRText::crText::CENTER_BOTTOM;
            break;
        default:
            align = CRText::crText::LEFT_BOTTOM;
        }
        break;
    default:
        switch (_hjustify) {
        case 2:
            align = CRText::crText::RIGHT_BOTTOM_BASE_LINE;
            break;
        case 1:
            align = CRText::crText::CENTER_BOTTOM_BASE_LINE;
            break;
        default:
            align = CRText::crText::LEFT_BOTTOM_BASE_LINE;
        }
        break;
    }
...
dxfEntity::assign(dxfFile* dxf, codeValue& cv)
{
    ...
    if (cv._groupCode == 66 && !(_entity && string("TABLE") == _entity->name())) {
        // The funny thing here. crGroup code 66 has been called 'obsoleted'
        ...
        // and post the correction to CRCore mailing list
        _seqend = true;
