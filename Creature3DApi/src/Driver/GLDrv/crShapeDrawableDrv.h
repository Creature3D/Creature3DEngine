...
class DrawShapeVisitor : public crConstShapeVisitor
{
public:

	DrawShapeVisitor(crState& state,const crTessellationHints* hints):
	  m_state(state),
		  m_hints(hints)
	  {
#if 0
		  if (hints)
		  {
			  notify(NOTICE)<<"Warning: crTessellationHints ignored in present CRCore::crShapeDrawable implementation."<<std::endl;
		  }
#endif   
	  }
...
protected:
	...
	// helpers for apply( crCylinder | crSphere | crCapsule )
...
void DrawShapeVisitor::apply(const crSphere& sphere)
{
	glPushMatrix();
	...
	float ratio = (m_hints ? m_hints->getDetailRatio() : 1.0f);
...
void DrawShapeVisitor::apply(const crBox& box)
{
	// evaluate hints
	bool createBody = (m_hints ? m_hints->getCreateBody() : true);
	bool createTop = (m_hints ? m_hints->getCreateTop() : true);
	bool createBottom = (m_hints ? m_hints->getCreateBottom() : true);
	...
	if (!box.zeroRotation())
	{
		crMatrix rotation(box.getRotationMatrix());
...
void DrawShapeVisitor::apply(const crCone& cone)
{
	glPushMatrix();
	...
	{
		crMatrix rotation(cone.getRotationMatrix());
		glMultMatrix(rotation.ptr());
	}
...
void DrawShapeVisitor::apply(const crCylinder& cylinder)
{
	glPushMatrix();
	...
	{
		crMatrix rotation(cylinder.getRotationMatrix());
		glMultMatrix(rotation.ptr());
	}
...
void DrawShapeVisitor::apply(const crCapsule& capsule)
{
	glPushMatrix();
	...
	{
		crMatrix rotation(capsule.getRotationMatrix());
		glMultMatrix(rotation.ptr());
	}
...
void DrawShapeVisitor::apply(const crInfinitePlane&)
{
	notify(NOTICE)<<"Warning: DrawShapeVisitor::apply(const crInfinitePlane& plane) not yet implementated. "<<std::endl;
}

void DrawShapeVisitor::apply(const crTriangleMesh& mesh)
{
	...
	if (vertices && indices)
	{
		...
		{
			const CRCore::crVector3& v1=(*vertices)[indices->index(i)];
			const CRCore::crVector3& v2=(*vertices)[indices->index(i+1)];
			const CRCore::crVector3& v3=(*vertices)[indices->index(i+2)];
			crVector3 normal = (v2-v1)^(v3-v2);
			normal.normalize();
...
void DrawShapeVisitor::apply(const crConvexHull& hull)
{
	apply((const crTriangleMesh&)hull);
}

void DrawShapeVisitor::apply(const crHeightField& field)
{
	if (field.getNumColumns()==0 || field.getNumRows()==0) return;
	bool hascolor = field.hasColor();
	glPushMatrix();
	...
	{
		crMatrix rotation(field.getRotationMatrix());
		glMultMatrix(rotation.ptr());
	}
	...
	if (field.getSkirtHeight()!=0.0f)
	{
		...
		for(col=0;col<field.getNumColumns();++col,u+=du)
		{
			...
			if(hascolor)
			{
				glColor4fv(field.getColor(col,0).ptr());
			}
			glTexCoord2f(u,0.0f);
		...
		for(col=0;col<field.getNumColumns();++col,u+=du)
		{
			...
			if(hascolor)
			{
				glColor4fv(field.getColor(col,row).ptr());
			}
			glTexCoord2f(u,1.0f);
			glNormal3fv(normTop.ptr());
	...
	for(unsigned int row=0;row<field.getNumRows()-1;++row,vBase+=dv)
	{
		...
		if (field.getSkirtHeight()!=0.0f)
		{
			...
			if(hascolor)
			{
				colorTop = field.getColor(0,row+1);
				glColor4fv(colorTop.ptr());
			}
			glTexCoord2f(u,vTop);
			glNormal3fv(normTop.ptr());
			glVertex3fv(vertTop.ptr());
			...
			if(hascolor)
			{
				colorBase = field.getColor(0,row);
				glColor4fv(colorBase.ptr());
			}
		...
		for(unsigned int col=0;col<field.getNumColumns();++col,u+=du)
		{
			...
			if(hascolor)
			{
				colorTop = field.getColor(col,row+1);
				glColor4fv(colorTop.ptr());
			}
			glTexCoord2f(u,vTop);
			glNormal3fv(normTop.ptr());
			...
			if(hascolor)
			{
				colorBase = field.getColor(col,row);
				glColor4fv(colorBase.ptr());
			}
			glTexCoord2f(u,vBase);
			glNormal3fv(normBase.ptr());
		...
		if (field.getSkirtHeight()!=0.0f)
		{
			...
			if(hascolor) glColor4fv(colorTop.ptr());
			glTexCoord2f(u,vTop);
			glNormal3fv(normTop.ptr());
			glVertex3fv(vertTop.ptr());

			if(hascolor) glColor4fv(colorBase.ptr());
			glTexCoord2f(u,vBase);
			glNormal3fv(normBase.ptr());
			glVertex3fv(vertBase.ptr());
		}
...
void DrawShapeVisitor::apply(const crCompositeShape& group)
{
...
void crShapeDrawable::drawImplementation(CRCore::crState& state ) const
{
	if(!m_shape.valid()) 
		return;

	//glColor4fv(m_pshapeDrawable->getColor().ptr());
	state.applyColor(m_color);

	DrawShapeVisitor dsv(state,m_tessellationHints.get());
	...
	{
		char gbuf[256];
		sprintf(gbuf,"crShapeDrawable OpenGL error %s\n\0",gluErrorString(errorNo));
