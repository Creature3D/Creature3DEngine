...
HL_EXP NLboolean HL_APIENTRY nlGroupAddSocket(NLint group, NLsocket socket)
{
    ...
    if(pgroup->numsockets == pgroup->maxsockets)
    {
        ...
        if(oldmax == NL_MAX_GROUP_SOCKETS)
        {
			...
			printf("nlGroupAddSocket Error oldmax = %d, MAX_GROUP_SOCKETS = %d",oldmax,NL_MAX_GROUP_SOCKETS);
            return NL_FALSE;
        }
        pgroup->maxsockets *= 2;
    ...
    if(i == pgroup->maxsockets)
    {
        (void)htMutexUnlock(&grouplock);
        nlSetError(NL_OUT_OF_GROUP_SOCKETS);
		printf("nlGroupAddSocket Error sockets[i] = %d, numsockets = %d, maxsockets = %d", pgroup->sockets[pgroup->numsockets], pgroup->numsockets, pgroup->maxsockets);
        return NL_FALSE;
    }
    pgroup->numsockets++;
    if(htMutexUnlock(&grouplock) != 0)
