                             ...
                             NLboolean checkonly)
{
    ...
    if(sock->reclen >= NL_HEADER_LEN)
    {
		...
		NLushort    head;
		readShort(sock->inbuf, c, head);
		if(head != 0x1a7e/*0x55aa*/)
		{
			if(checkonly == NL_FALSE)
			{
				nlSetError(NL_PACKET_SYNC);
		...
		readShort(sock->inbuf, c, len);//0x1a7e
		//readInt(sock->inbuf, c, len);//0x55aa
        ...
        if(len <= (NLushort)(sock->reclen - c))
        {
            ...
            if(checkonly == NL_FALSE)
            {
                ...
                if(sock->reclen >= NL_HEADER_LEN)
                {
                    ...
                    /* read the length of the packet */
                    c = 2;
                    readShort(sock->inbuf, c, templen);//0x1a7e
					//readInt(sock->inbuf, c, templen);//0x55aa
...
static NLint sock_WritePacket(NLsocket socket, const NLvoid *buffer, NLint nbytes)
{
	...
	writeShort(temp, c, 0x1a7e);
    writeShort(temp, c, (NLushort)nbytes);

	//writeShort(temp,c,0x55aa);
	//writeInt(temp, c, (NLuint)nbytes);
...
NLint sock_PollSocket(NLsocket socket, NLenum name, NLint timeout)
{
    NLint           numselect;
    nl_socket_t     *sock = nlSockets[socket];
    fd_set          fdset;
