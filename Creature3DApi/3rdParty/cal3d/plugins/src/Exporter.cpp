...
int CExporter::getMaterialThreadId(const std::string & matName)
{//add by Îâ²Æ»ª
	// get positions of the material thread id
	//std::string::size_type openPos;
	//openPos = matName.find_last_of("[");
	//std::string::size_type closePos;
	//closePos = matName.find_last_of("]");
	...
	std::string::size_type closePos;
	closePos = matName.find_last_of("_");
	std::string::size_type openPos;
	openPos = matName.find_last_of("_", closePos-1);
	if((openPos == std::string::npos) || (closePos == std::string::npos) || (++openPos >= closePos))
	{
		theExporter.SetLastError("Invalid material thread id in material.", __FILE__, __LINE__);
		return -1;
	...
	strMaterialThreadId = matName.substr(openPos, closePos - openPos);
...
bool CExporter::ExportMaterial(const std::string& strFilename)
{
  ...
  pointPos = strFilename.find_last_of(".");

  char newStrFileName[256];
  int materialThreadId = getMaterialThreadId(pMaterialCandidate->GetName());
  sprintf(newStrFileName,"%s[%d].xrf",strFilename.substr(0,pointPos).c_str(),materialThreadId);
  ...
  if(!CalSaver::saveCoreMaterial(newStrFileName, &coreMaterial))
  {
    SetLastError(CalError::getLastErrorText(), __FILE__, __LINE__);
    return false;
  }
