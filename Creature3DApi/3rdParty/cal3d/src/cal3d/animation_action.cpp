...
bool CalAnimationAction::update(float deltaTime)
{
  ...
  if(getState() == STATE_STEADY)
  {
	...
	if(pCoreAnimation)
	{///modify by 吴财华
		if(!m_autoLock && getTime() >= pCoreAnimation->getDuration() - m_delayOut)
		{
		    setState(STATE_OUT);
		}
		// if the anim is supposed to stay locked on last keyframe, reset the time here.
		else if (m_autoLock && getTime() > pCoreAnimation->getDuration())
		{
		    setState(STATE_STOPPED);
		    setTime(pCoreAnimation->getDuration());
		}
	}
  }
  ...
  if(getState() == STATE_OUT)
  {
    // check if we are still in the OUT phase
	  ...
	  if(pCoreAnimation)
	  {///modify by 吴财华
	      if(getTime() < pCoreAnimation->getDuration())
		  {
		      setWeight((pCoreAnimation->getDuration() - getTime()) / m_delayOut * m_weightTarget);
		  }
		  else
		  {
