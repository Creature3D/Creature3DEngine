...
#undef writeInt
#define writeInt(x, y, z)     {*((NLuint *)((NLbyte *)&x[y])) = htonl((NLuint)z); y += 4;}
#undef readInt
#define readInt(x, y, z)      {z = (NLushort)ntohl(*(NLuint *)((NLbyte *)&x[y])); y += 4;}
...
#define NL_MIN_SOCKETS          NL_MAX_GROUP_SOCKETS
...
typedef struct
{
    ...
    NLint   (*PollSocket)(NLsocket socket, NLenum name, NLint timeout);
