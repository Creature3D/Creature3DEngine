...
#ifndef CRCORE_CRTHREAD_H
#define CRCORE_CRTHREAD_H
...
public:
     ...
     *  crThread::THREAD_PRIORITY_DEFAULT if
     *  init() has not been called.
     *
     *  @return the crThread::ThreadPriority of the master thread.
     */
     ...
     *  easily occur (Although the crMutex & CRCore::crBarrier
    ...
    /**
     *  Disable thread cancelation altogether. crThread::cancel() has no effect.
     *
     *  @return 0 if normal, -1 if errno set, errno code otherwise.
     */
    ...
    /**
     *  Mark the thread to cancel aysncronously on crThread::cancel().
     ...
     *  crThread::cancel() (This is the default)
     *
     *  @return 0 if normal, -1 if errno set, errno code otherwise.
     */
    ...
    /**
     *  crThread's run method.  Must be implemented by derived classes.
     *  This is where the action happens.
     */
    virtual void run() = 0;

    /**
     *  crThread's cancel cleanup routine, called upon cancel(), after the
    ...
    /** crThread's processor affinity method.  This binds a thread to a
    ...
    /** microSleep method, equivilant to the posix usleep(microsec).
      ...
      */
    static int sleep( unsigned int ms);
...
private:
    ...
    friend class ThreadPrivateActions;
      ...
      */
    crThread(const crThread &/*t*/) {};

    /**
      *  Private copy assignment, to prevent tampering.
      */
    crThread &operator=(const crThread &/*t*/) {return *(this);};
    ...
    /**
     *  Master thread's priority, set by crThread::init.
     */
