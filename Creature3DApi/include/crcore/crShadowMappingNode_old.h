...
#ifndef CRCORE_CRSHADOWMAPPINGNODE
#define CRCORE_CRSHADOWMAPPINGNODE 1
...
class CR_EXPORT crShadowMappingNode : public crGroup
{
    public :
		crShadowMappingNode();

        crShadowMappingNode(const crShadowMappingNode&,const crCopyOp& copyop=crCopyOp::SHALLOW_COPY);

        META_Node(CRCore, crShadowMappingNode);
		...
		//typedef std::pair < int, LS_CameraPair > Active_LS_CameraMapPair;
       // typedef std::map< CRCore::ref_ptr<const CRCore::crLightSource>, LightMVPS > Active_LS_LightMVPMap;
