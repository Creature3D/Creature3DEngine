...
void Matrix_implementation::setRotate(const crQuat& q)
{
    double length2 = q.length2();
    if (fabs(length2) <= std::numeric_limits<double>::min())
    {
        //m_mat[0][0] = 0.0; m_mat[1][0] = 0.0; m_mat[2][0] = 0.0;
        //m_mat[0][1] = 0.0; m_mat[1][1] = 0.0; m_mat[2][1] = 0.0;
        //m_mat[0][2] = 0.0; m_mat[1][2] = 0.0; m_mat[2][2] = 0.0;
		makeIdentity();
    }
    else
    {
...
crQuat Matrix_implementation::getRotate() const
{
    crQuat q;
    ...
    tq[0] = 1 + m_mat[0][0]+m_mat[1][1]+m_mat[2][2];
    tq[1] = 1 + m_mat[0][0]-m_mat[1][1]-m_mat[2][2];
    tq[2] = 1 - m_mat[0][0]+m_mat[1][1]-m_mat[2][2];
    tq[3] = 1 - m_mat[0][0]-m_mat[1][1]+m_mat[2][2];
    ...
    if (j==0)
    {
        ...
        QW = tq[0];
        QX = m_mat[1][2]-m_mat[2][1]; 
        QY = m_mat[2][0]-m_mat[0][2]; 
        QZ = m_mat[0][1]-m_mat[1][0]; 
    }
    else if (j==1)
    {
        QW = m_mat[1][2]-m_mat[2][1]; 
        QX = tq[1];
        QY = m_mat[0][1]+m_mat[1][0]; 
        QZ = m_mat[2][0]+m_mat[0][2]; 
    }
    else if (j==2)
    {
        QW = m_mat[2][0]-m_mat[0][2]; 
        QX = m_mat[0][1]+m_mat[1][0]; 
        QY = tq[2];
        QZ = m_mat[1][2]+m_mat[2][1]; 
    }
    else /* if (j==3) */
    {
        QW = m_mat[0][1]-m_mat[1][0]; 
        QX = m_mat[2][0]+m_mat[0][2]; 
        QY = m_mat[1][2]+m_mat[2][1]; 
        QZ = tq[3];
    }
...
crQuat Matrix_implementation::getRotate() const
{
    ...
    QW = 0.5 * sqrt( CRCore::maximum( 0.0, 1.0 + m_mat[0][0] + m_mat[1][1] + m_mat[2][2] ) );
    QX = 0.5 * sqrt( CRCore::maximum( 0.0, 1.0 + m_mat[0][0] - m_mat[1][1] - m_mat[2][2] ) );
    QY = 0.5 * sqrt( CRCore::maximum( 0.0, 1.0 - m_mat[0][0] + m_mat[1][1] - m_mat[2][2] ) );
    QZ = 0.5 * sqrt( CRCore::maximum( 0.0, 1.0 - m_mat[0][0] - m_mat[1][1] + m_mat[2][2] ) );
    ...
    QX = QX * CRCore::signOrZero(  m_mat[1][2] - m_mat[2][1]) ;
    QY = QY * CRCore::signOrZero(  m_mat[2][0] - m_mat[0][2]) ;
    QZ = QZ * CRCore::signOrZero(  m_mat[0][1] - m_mat[1][0]) ;
#else
    QX = QX * CRCore::sign(  m_mat[1][2] - m_mat[2][1]) ;
    QY = QY * CRCore::sign(  m_mat[2][0] - m_mat[0][2]) ;
    QZ = QZ * CRCore::sign(  m_mat[0][1] - m_mat[1][0]) ;
#endif
...
crQuat Matrix_implementation::getRotate() const
{
    ...
    tr = m_mat[0][0] + m_mat[1][1] + m_mat[2][2]+1.0;
    ...
    if (tr > 0.0)
    {
        ...
        QW = s / 2.0;
        s = 0.5 / s;
        QX = (m_mat[1][2] - m_mat[2][1]) * s;
        QY = (m_mat[2][0] - m_mat[0][2]) * s;
        QZ = (m_mat[0][1] - m_mat[1][0]) * s;
    }
    else
    {
        ...
        s = (value_type)sqrt ((m_mat[i][i] - (m_mat[j][j] + m_mat[k][k])) + 1.0);
        ...
        tq[3] = (m_mat[j][k] - m_mat[k][j]) * s;
        tq[j] = (m_mat[i][j] + m_mat[j][i]) * s;
        tq[k] = (m_mat[i][k] + m_mat[k][i]) * s;
...
void Matrix_implementation::setTrans( const crVector3f& v )
{
    m_mat[3][0] = v[0];
    m_mat[3][1] = v[1];
    m_mat[3][2] = v[2];
}
void Matrix_implementation::setTrans( const crVector3d& v )
{
    m_mat[3][0] = v[0];
    m_mat[3][1] = v[1];
    m_mat[3][2] = v[2];
...
void Matrix_implementation::makeScale( const crVector3f& v )
{
    makeScale(v[0], v[1], v[2] );
}

void Matrix_implementation::makeScale( const crVector3d& v )
{
    makeScale(v[0], v[1], v[2] );
}
...
void Matrix_implementation::makeTranslate( const crVector3f& v )
{
    makeTranslate( v[0], v[1], v[2] );
}

void Matrix_implementation::makeTranslate( const crVector3d& v )
{
    makeTranslate( v[0], v[1], v[2] );
}
...
void Matrix_implementation::makeRotate( const crVector3f& from, const crVector3f& to )
{
    makeIdentity();
...
void Matrix_implementation::makeRotate( const crVector3d& from, const crVector3d& to )
{
    makeIdentity();
...
void Matrix_implementation::makeRotate( value_type angle, const crVector3f& axis )
{
    makeIdentity();
...
void Matrix_implementation::makeRotate( value_type angle, const crVector3d& axis )
{
    makeIdentity();
...
void Matrix_implementation::makeRotate( const crQuat& quat )
{
    makeIdentity();
...
void Matrix_implementation::makeRotate( value_type angle1, const crVector3f& axis1, 
...
void Matrix_implementation::makeRotate( value_type angle1, const crVector3d& axis1, 
...
{
    value_type x_colMag = (rhs.m_mat[0][0] * rhs.m_mat[0][0]) + (rhs.m_mat[1][0] * rhs.m_mat[1][0]) + (rhs.m_mat[2][0] * rhs.m_mat[2][0]);
    value_type y_colMag = (rhs.m_mat[0][1] * rhs.m_mat[0][1]) + (rhs.m_mat[1][1] * rhs.m_mat[1][1]) + (rhs.m_mat[2][1] * rhs.m_mat[2][1]);
    value_type z_colMag = (rhs.m_mat[0][2] * rhs.m_mat[0][2]) + (rhs.m_mat[1][2] * rhs.m_mat[1][2]) + (rhs.m_mat[2][2] * rhs.m_mat[2][2]);
    
    if(!equivalent((double)x_colMag, 1.0) && !equivalent((double)x_colMag, 0.0))
    {
      x_colMag = sqrt(x_colMag);
      m_mat[0][0] = rhs.m_mat[0][0] / x_colMag;
      m_mat[1][0] = rhs.m_mat[1][0] / x_colMag;
      m_mat[2][0] = rhs.m_mat[2][0] / x_colMag;
    }
    else
    {
      m_mat[0][0] = rhs.m_mat[0][0];
      m_mat[1][0] = rhs.m_mat[1][0];
      m_mat[2][0] = rhs.m_mat[2][0];
    }

    if(!equivalent((double)y_colMag, 1.0) && !equivalent((double)y_colMag, 0.0))
    {
      y_colMag = sqrt(y_colMag);
      m_mat[0][1] = rhs.m_mat[0][1] / y_colMag;
      m_mat[1][1] = rhs.m_mat[1][1] / y_colMag;
      m_mat[2][1] = rhs.m_mat[2][1] / y_colMag;
    }
    else
    {
      m_mat[0][1] = rhs.m_mat[0][1];
      m_mat[1][1] = rhs.m_mat[1][1];
      m_mat[2][1] = rhs.m_mat[2][1];
    }

    if(!equivalent((double)z_colMag, 1.0) && !equivalent((double)z_colMag, 0.0))
    {
      z_colMag = sqrt(z_colMag);
      m_mat[0][2] = rhs.m_mat[0][2] / z_colMag;
      m_mat[1][2] = rhs.m_mat[1][2] / z_colMag;
      m_mat[2][2] = rhs.m_mat[2][2] / z_colMag;
    }
    else
    {
      m_mat[0][2] = rhs.m_mat[0][2];
      m_mat[1][2] = rhs.m_mat[1][2];
      m_mat[2][2] = rhs.m_mat[2][2];
    }

    m_mat[3][0] = rhs.m_mat[3][0];
    m_mat[3][1] = rhs.m_mat[3][1];
    m_mat[3][2] = rhs.m_mat[3][2];

    m_mat[0][3] = rhs.m_mat[0][3];
    m_mat[1][3] = rhs.m_mat[1][3];
    m_mat[2][3] = rhs.m_mat[2][3];
    m_mat[3][3] = rhs.m_mat[3][3];
...
bool Matrix_implementation::invert_4x3( const Matrix_implementation& mat )
{
    ...
    r00 = mat.m_mat[0][0]; r01 = mat.m_mat[0][1]; r02 = mat.m_mat[0][2];
    r10 = mat.m_mat[1][0]; r11 = mat.m_mat[1][1]; r12 = mat.m_mat[1][2];
    r20 = mat.m_mat[2][0]; r21 = mat.m_mat[2][1]; r22 = mat.m_mat[2][2];

        // Partially compute inverse of rot
    m_mat[0][0] = r11*r22 - r12*r21;
    m_mat[0][1] = r02*r21 - r01*r22;
    m_mat[0][2] = r01*r12 - r02*r11;

      // Compute determinant of rot from 3 elements just computed
    register value_type one_over_det = 1.0/(r00*m_mat[0][0] + r10*m_mat[0][1] + r20*m_mat[0][2]);
    r00 *= one_over_det; r10 *= one_over_det; r20 *= one_over_det;  // Saves on later computations

      // Finish computing inverse of rot
    m_mat[0][0] *= one_over_det;
    m_mat[0][1] *= one_over_det;
    m_mat[0][2] *= one_over_det;
    m_mat[0][3] = 0.0;
    m_mat[1][0] = r12*r20 - r10*r22; // Have already been divided by det
    m_mat[1][1] = r00*r22 - r02*r20; // same
    m_mat[1][2] = r02*r10 - r00*r12; // same
    m_mat[1][3] = 0.0;
    m_mat[2][0] = r10*r21 - r11*r20; // Have already been divided by det
    m_mat[2][1] = r01*r20 - r00*r21; // same
    m_mat[2][2] = r00*r11 - r01*r10; // same
    m_mat[2][3] = 0.0;
    m_mat[3][3] = 1.0;
...
#define d r22
    ...
    if( CRCore::square(d-1.0) > 1.0e-6 )  // Involves perspective, so we must
    {                       // compute the full inverse
    
        Matrix_implementation TPinv;
        m_mat[3][0] = m_mat[3][1] = m_mat[3][2] = 0.0;
...
#define c  r12
        ...
        a  = mat.m_mat[0][3]; b  = mat.m_mat[1][3]; c  = mat.m_mat[2][3];
        px = m_mat[0][0]*a + m_mat[0][1]*b + m_mat[0][2]*c;
        py = m_mat[1][0]*a + m_mat[1][1]*b + m_mat[1][2]*c;
        pz = m_mat[2][0]*a + m_mat[2][1]*b + m_mat[2][2]*c;
...
#define tz r12
        ...
        tx = mat.m_mat[3][0]; ty = mat.m_mat[3][1]; tz = mat.m_mat[3][2];
        ...
        TPinv.m_mat[0][0] = tx*px + 1.0;
        TPinv.m_mat[0][1] = ty*px;
        TPinv.m_mat[0][2] = tz*px;
        TPinv.m_mat[0][3] = -px * one_over_s;
        TPinv.m_mat[1][0] = tx*py;
        TPinv.m_mat[1][1] = ty*py + 1.0;
        TPinv.m_mat[1][2] = tz*py;
        TPinv.m_mat[1][3] = -py * one_over_s;
        TPinv.m_mat[2][0] = tx*pz;
        TPinv.m_mat[2][1] = ty*pz;
        TPinv.m_mat[2][2] = tz*pz + 1.0;
        TPinv.m_mat[2][3] = -pz * one_over_s;
        TPinv.m_mat[3][0] = -tx;
        TPinv.m_mat[3][1] = -ty;
        TPinv.m_mat[3][2] = -tz;
        TPinv.m_mat[3][3] = one_over_s;
...
#undef d
    ...
    else // Rightmost column is [0; 0; 0; 1] so it can be ignored
    {
        ...
        m_mat[3][0] = -(tx*m_mat[0][0] + ty*m_mat[1][0] + tz*m_mat[2][0]);
        m_mat[3][1] = -(tx*m_mat[0][1] + ty*m_mat[1][1] + tz*m_mat[2][1]);
        m_mat[3][2] = -(tx*m_mat[0][2] + ty*m_mat[1][2] + tz*m_mat[2][2]);
...
{
    if (m_mat[0][3]!=0.0 || m_mat[1][3]!=0.0 || m_mat[2][3]!=0.0 || m_mat[3][3]!=1.0) return false;

    zNear = (m_mat[3][2]+1.0) / m_mat[2][2];
    zFar = (m_mat[3][2]-1.0) / m_mat[2][2];
    
    left = -(1.0+m_mat[3][0]) / m_mat[0][0];
    right = (1.0-m_mat[3][0]) / m_mat[0][0];

    bottom = -(1.0+m_mat[3][1]) / m_mat[1][1];
    top = (1.0-m_mat[3][1]) / m_mat[1][1];
...
bool Matrix_implementation::getFrustum(double& left, double& right,
                                       ...
                                       double& zNear, double& zFar) const
{
    if (m_mat[0][3]!=0.0 || m_mat[1][3]!=0.0 || m_mat[2][3]!=-1.0 || m_mat[3][3]!=0.0)
        return false;
    ...
    double temp_near = m_mat[3][2] / (m_mat[2][2]-1.0);
    double temp_far = m_mat[3][2] / (1.0+m_mat[2][2]);

    left = temp_near * (m_mat[2][0]-1.0) / m_mat[0][0];
    right = temp_near * (1.0+m_mat[2][0]) / m_mat[0][0];

    top = temp_near * (1.0+m_mat[2][1]) / m_mat[1][1];
    bottom = temp_near * (m_mat[2][1]-1.0) / m_mat[1][1];
...
void Matrix_implementation::makeLookAt(const crVector3d& eye,const crVector3d& center,const crVector3d& up)
{
    crVector3d f(center-eye);
    f.normalize();
    crVector3d s(f^up);
    s.normalize();
    crVector3d u(s^f);
...
void Matrix_implementation::getLookAt(crVector3f& eye,crVector3f& center,crVector3f& up,value_type lookDistance) const
{
    ...
    crVector3f e = CRCore::crVector3f(0.0,0.0,0.0)*inv;
    up = transform3x3(*this,CRCore::crVector3f(0.0,1.0,0.0));
    crVector3f c = transform3x3(*this,CRCore::crVector3f(0.0,0.0,-1));
    c.normalize();
    c = e + c*lookDistance;
...
void Matrix_implementation::getLookAt(crVector3d& eye,crVector3d& center,crVector3d& up,value_type lookDistance) const
{
    Matrix_implementation inv;
    inv.invert(*this);
    ...
    // note: e and c variables must be used inside this method instead of eye and center
    // because eye and center are references and they may point to the same variable.
    crVector3d e = CRCore::crVector3d(0.0,0.0,0.0)*inv;
    up = transform3x3(*this,CRCore::crVector3d(0.0,1.0,0.0));
    crVector3d c = transform3x3(*this,CRCore::crVector3d(0.0,0.0,-1));
    c.normalize();
    c = e + c*lookDistance;
