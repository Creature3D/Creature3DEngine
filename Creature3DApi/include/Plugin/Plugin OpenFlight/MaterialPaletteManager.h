...
class MaterialPaletteManager
{
  public:
    ...
    int add(CRCore::crMaterial const* material);
  ...
  private:
    ...
    {
        MaterialRecord(CRCore::crMaterial const* m, int i)
            : crMaterial(m)
            , Index(i) { }

        CRCore::crMaterial const* crMaterial;
        int Index;

    };

    // Map to allow lookups by crMaterial pointer, and permit sorting by index
    typedef std::map<CRCore::crMaterial const*, MaterialRecord> MaterialPalette;
