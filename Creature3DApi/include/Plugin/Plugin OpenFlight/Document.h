...
class Document
{
    public:
        ...
        void setOptions(const CRIOManager::crReaderWriter::Options* options) { _options = options; }
        const CRIOManager::crReaderWriter::Options* getOptions() const { return _options.get(); }
        ...
        void setHeaderNode(CRCore::crNode* node) { _osgHeader = node; }
        CRCore::crNode* getHeaderNode() { return _osgHeader.get(); }

        // Instance definitions
        void setInstanceDefinition(int no, CRCore::crNode* node) { _instanceDefinitionMap[no] = node; }
        CRCore::crNode* getInstanceDefinition(int no);
        ...
        void setSubSurfacePolygonOffset(int level, CRCore::crPolygonOffset* po);
        CRCore::crPolygonOffset* getSubSurfacePolygonOffset(int level);

        void setSubSurfaceDepth(CRCore::crDepth* depth) { _subsurfaceDepth = depth; }
        CRCore::crDepth* getSubSurfaceDepth() { return _subsurfaceDepth.get(); }
    ...
    protected:
        ...
        // Options
        CRCore::ref_ptr<const CRIOManager::crReaderWriter::Options> _options;
        ...
        // Header data
        CRCore::ref_ptr<CRCore::crNode> _osgHeader;

        CRCore::ref_ptr<VertexPool> _vertexPool;
        ...
        CRCore::ref_ptr<LightPointAppearancePool> _lightPointAppearancePool;
        CRCore::ref_ptr<LightPointAnimationPool> _lightPointAnimationPool;
        CRCore::ref_ptr<ShaderPool> _shaderPool;

        typedef std::map<int, CRCore::ref_ptr<CRCore::crPolygonOffset> > SubSurfacePolygonOffsets;
        SubSurfacePolygonOffsets _subsurfacePolygonOffsets;
        CRCore::ref_ptr<CRCore::crDepth> _subsurfaceDepth;
        ...
        CRCore::ref_ptr<PrimaryRecord> _currentPrimaryRecord;

        typedef std::vector<CRCore::ref_ptr<PrimaryRecord> > LevelStack;
        LevelStack _levelStack;
        LevelStack _extensionStack;

        typedef std::map<int,CRCore::ref_ptr<CRCore::crNode> > InstanceDefinitionMap;
