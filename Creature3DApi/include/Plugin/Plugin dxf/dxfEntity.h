...
static inline void
getOCSMatrix(const CRCore::crVector3d& ocs, CRCore::crMatrixd& m)
{
    static const double one_64th = 1.0/64.0;
    m.makeIdentity();
    if (ocs == CRCore::crVector3d(0,0,1)) return;
    CRCore::crVector3d ax(1,0,0), ay(0,1,0), az(0,0,1);
    CRCore::crVector3d ocsaxis(ocs);
    ocsaxis.normalize();
    ...
    ay = ocsaxis ^ ax;
    ay.normalize();
    m = CRCore::crMatrixd(    ax.x(), ax.y(), ax.z(), 0, 
                        ay.x(), ay.y(), ay.z(), 0,
...
class dxfBasicEntity : public CRCore::Referenced
{
public:
...
protected:
    CRCore::crVector3d    _center;
    double    _radius;
    CRCore::crVector3d    _ocs;
};
...
public:
    ...
    virtual void drawScene(scene* sc);
protected:
    CRCore::crVector3d    _a;
    //CRCore::crVector3d    _b;
    CRCore::crVector3d    _ocs;
};
...
public:
    dxf3DFace() 
    {
        _vertices[0] = CRCore::crVector3d(0,0,0);
        _vertices[1] = CRCore::crVector3d(0,0,0);
        _vertices[2] = CRCore::crVector3d(0,0,0);
        _vertices[3] = CRCore::crVector3d(0,0,0);
    }
    virtual ~dxf3DFace() {}
    ...
    virtual void drawScene(scene* sc);
protected:
    CRCore::crVector3d _vertices[4];
};

class dxfVertex : public dxfBasicEntity
{
public:
    dxfVertex() : _vertex(CRCore::crVector3d(0,0,0)), _indice1(0), _indice2(0), _indice3(0), _indice4(0) {}
    virtual ~dxfVertex() {}
    ...
    const CRCore::crVector3d& getVertex() const { return _vertex; }
...
protected:
    ...
    std::vector<CRCore::ref_ptr<dxfVertex> >        _vertices;
    std::vector<CRCore::ref_ptr<dxfVertex> >        _indices;
    double                        _elevation;
    ...
    CRCore::crVector3d                    _ocs; //210 220 230
...
protected:
    ...
    CRCore::crVector3d                    _ocs; //210 220 230
    CRCore::crVector3d                    _lastv;
    std::vector< CRCore::crVector3d >     _vertices;
...
public:
    dxfInsert() : _block(NULL), 
                    ...
                    _rotation(0), 
                    _scale(1,1,1),
                    _point(CRCore::crVector3d(0,0,0)),
                    _ocs(CRCore::crVector3d(0,0,1)) {}
    virtual ~dxfInsert() {}
...
protected:
    ...
    CRCore::ref_ptr<dxfBlock> _block;
...
protected:
    ...
    CRCore::crVector3d        _point1;    // 10,20,30
    CRCore::crVector3d        _point2;    // 11,21,31
    CRCore::crVector3d        _ocs;       // 210,220,230
    double            _height;    // 40
    double            _xscale;    // 41
    double            _rotation;  // 50
...
class dxfEntity : public CRCore::Referenced
{
public:
    ...
    static dxfBasicEntity* getcrRegistryEntity(std::string s) {
            return _registry[s].get();
    }

protected:
    std::vector<CRCore::ref_ptr<dxfBasicEntity> > _entityList;
    static std::map<std::string, CRCore::ref_ptr<dxfBasicEntity> > _registry;
