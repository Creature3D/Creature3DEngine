...
static CRCore::buffered_object<ExtensionSet> s_glExtensionSetList;
static CRCore::buffered_object<std::string> s_glRendererList;
static CRCore::buffered_value<int> s_glInitializedList;

static CRCore::buffered_object<ExtensionSet> s_gluExtensionSetList;
static CRCore::buffered_object<std::string> s_gluRendererList;
static CRCore::buffered_value<int> s_gluInitializedList;

//static const char* envVar = getenv("CRE_GL_EXTENSION_DISABLE");
static std::string s_GLExtensionDisableString(/*envVar?envVar:*/"Nothing defined");

float CRCore::getGLVersionNumber()
{
...
bool CRCore::isExtensionInExtensionString(const char *extension, const char *extensionString)
{
...
bool CRCore::isGLExtensionSupported(unsigned int contextID, const char *extension)
{
    ...
    if (!s_glInitializedList[contextID])
    {
        ...
        //CRCore::notify(INFO)<<"OpenGL extensions supported by installed OpenGL drivers are:"<<std::endl;
        //for(ExtensionSet::iterator itr=extensionSet.begin();
        //    itr!=extensionSet.end();
        //    ++itr)
        //{
        //    CRCore::notify(INFO)<<"    "<<*itr<<std::endl;
        //}
    ...
    //if (result)
    //{
    //    if (!extensionDisabled) CRCore::notify(INFO)<<"OpenGL extension '"<<extension<<"' is supported."<<std::endl;
    //    else CRCore::notify(INFO)<<"OpenGL extension '"<<extension<<"' is supported by OpenGL\ndriver but has been disabled by CRCore::getGLExtensionDisableString()."<<std::endl;
    //}
    //else CRCore::notify(INFO)<<"OpenGL extension '"<<extension<<"' is not supported."<<std::endl;


    return result && !extensionDisabled;
}

void CRCore::setGLExtensionDisableString(const std::string& disableString)
{
    getGLExtensionDisableString() = disableString;
}

std::string& CRCore::getGLExtensionDisableString()
{
    return s_GLExtensionDisableString;
}


bool CRCore::isGLUExtensionSupported(unsigned int contextID, const char *extension)
{
    ...
    //if (result) CRCore::notify(INFO)<<"OpenGL utility library extension '"<<extension<<"' is supported."<<std::endl;
    //else CRCore::notify(INFO)<<"OpenGL utility library extension '"<<extension<<"' is not supported."<<std::endl;
...
void* CRCore::getGLExtensionFuncPtr(const char *funcName)
{
#if defined(WIN32)
