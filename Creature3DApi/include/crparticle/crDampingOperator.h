...
#ifndef CRPARTICLE_DAMPINGOPERATOR
#define CRPARTICLE_DAMPINGOPERATOR
...
*/
class crDampingOperator : public crOperator
{
public:
    crDampingOperator() : crOperator(), m_cutoffLow(0.0f), m_cutoffHigh(FLT_MAX)
    { m_damping.set(1.0f, 1.0f, 1.0f); }
    
    crDampingOperator( const crDampingOperator& copy, const CRCore::crCopyOp& copyop = CRCore::crCopyOp::SHALLOW_COPY )
    :   crOperator(copy, copyop), m_damping(copy.m_damping),
        m_cutoffLow(copy.m_cutoffLow), m_cutoffHigh(copy.m_cutoffHigh)
    {}
    
    META_Base( CRParticle, crDampingOperator );
...
protected:
    virtual ~crDampingOperator() {}
    crDampingOperator& operator=( const crDampingOperator& ) { return *this; }
...
inline void crDampingOperator::operate( crParticle* P, double dt )
{
    const CRCore::crVector3& vel = P->getVelocity();
